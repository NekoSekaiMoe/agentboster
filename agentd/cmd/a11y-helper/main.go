// a11y-helper is a small CLI bundled with agentd for desktop accessibility
// on Linux. It runs INSIDE the agentd sandbox (NOT on the host) and talks
// to the AT-SPI2 D-Bus registry there. The agentd daemon exec's it via
// sbMgr.Exec, parses the JSON envelope from stdout, and surfaces the
// result to the Web workflow as a desktop_inspect / desktop_a11y_click /
// desktop_a11y_type tool result.
//
// Usage:
//
...
func main() {
	...
	case "click":
		err = requireRefThen(cmd, args, runClick)
	case "type":
		err = requireRefText(cmd, args, runType)
	case "fill":
		err = requireRefText(cmd, args, runFill)
	case "-h", "--help", "help":
		usage()
		return
	default:
	...
	if err != nil {
		// Errors here are catastrophic (D-Bus unreachable, refs file
		// unreadable, etc.). Per-action failures are NOT returned as
		// errors — they go out as JSON envelopes with ok=false.
...
func requireRefThen(cmd string, args []string, fn func(string) error) error {
	if len(args) < 1 {
...
func requireRefText(cmd string, args []string, fn func(string, string) error) error {
	if len(args) < 2 {
