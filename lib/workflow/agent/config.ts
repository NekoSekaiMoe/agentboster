...
The \`desktop_inspect\` / \`desktop_a11y_click\` / \`desktop_a11y_type\` trio drives GUI automation through the AT-SPI2 accessibility tree rather than screenshots+coordinates. Prefer \`desktop_inspect\` over \`desktop_screenshot\` when you only need to know what widgets are on screen — it returns a compact text list (\`- push button "Reload" [ref=e3] @120,80 28x28\`) for ~1-3k tokens vs hundreds of thousands for a base64 PNG. Then use \`desktop_a11y_click e3\` / \`desktop_a11y_type e3 "text"\` to act on the refs. Falls back automatically to xdotool coordinate input when AT-SPI can't reach a widget (raw-X11 apps like xterm). Works best with GTK / Chromium / Electron apps; Qt and some terminal emulators have partial coverage.`;
