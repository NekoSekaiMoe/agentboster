...
import type { RemoteScorerConfig } from '../../config/types';
...
export class RemoteScorerProvider implements IScoringProvider {
  ...
  async evaluate(req: ScoreRequest): Promise<ScoreResponse> {
    ...
    try {
      ...
      if (!content) {
        return this.fallbackResponse('No response from remote scoring model');
      }

      return this.parseResponse(content);
    } catch (error) {
      return this.fallbackResponse(
        `Remote scoring error: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private parseResponse(content: string): ScoreResponse {
    try {
      ...
      if (!jsonMatch) {
        return this.fallbackResponse('Invalid JSON response from remote');
      }
      ...
      return this.fallbackResponse('Failed to parse remote scoring response');
    }
  }
