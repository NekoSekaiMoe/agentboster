...
节点身份持久化在 identity 包。Resolve 读 node_id_file（默认 /var/run/agentd.node_id），存在则复用，否则生成 `node-<hostname>-<unix_nano>` 并写盘。复用此文件即跨重启保持节点身份。
