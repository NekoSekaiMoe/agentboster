...
| [`computer-use-mcp/`](./computer-use-mcp/README.md) | Go 1.26.4 | standalone `go.mod` | Cross-platform MCP server for computer use: screenshots, mouse/keyboard input, accessibility tree queries. Used by the CLI desktop app. |
...
- The root `tsconfig.json` **excludes** `subpackage/`, so `yarn check:lint` from the repo root does not typecheck the CLI or agentd. Run checks inside each subdir.
