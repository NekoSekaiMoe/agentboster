...
discoverable, and run `usage_prompt` before reporting completion. The exported runner resolves the stable Runtime installed by bootstrap under `豆包用户数据目录/rpa-dev/local-preview/agent-runtime`; it never resolves runtime code through another Agent's Skill registry.
