...
async function ensurePiDeckExtension(extensionName: string): Promise<void> {
	...
	// agentboster CLI 用 ~/.agentboster/agent/ 作为 agent 配置目录
	// (subpackage/cli/packages/coding-agent/package.json: piConfig.configDir=".agentboster")。
	const extensionsDir = join(homedir, ".agentboster", "agent", "extensions");
