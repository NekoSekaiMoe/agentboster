...
export class PiProcess extends EventEmitter {
  ...
  /** 从 --version 解析出的次版本号（diagnostics 用途）。 */
  ...
  start(sessionPath?: string) {
    ...
    const args = ["--mode", "rpc"];
    // agentboster CLI supports --approve unconditionally (see
    // subpackage/cli/packages/coding-agent/src/cli/args.ts); the
    // historical pi 0.79+ version gate no longer applies.
    args.push("--approve");
    ...
    this.proc.on("exit", (code, signal) => {
      ...
      this.rpc?.close(new Error(`agentboster exited: code=${code ?? "null"}, signal=${signal ?? "null"}`));
  ...
  get client() {
    if (!this.rpc) throw new Error("agentboster process is not running");
    return this.rpc;
  }
  ...
  /**
   * 执行一次轻量 --version 检测，确认 agentboster 二进制可达。
   * 结果仅用于诊断面板，不再用于决定 --approve 是否可用（agentboster
   * CLI 始终支持该 flag，参见 subpackage/cli/packages/coding-agent/src/cli/args.ts）。
   */
  ...
  /**
   * 从 agentboster 的版本号字符串提取次版本号。
   * 格式通常为 "0.1.0"，返回 1。兼容遗留的 pi "0.79.4" 格式。
   */
