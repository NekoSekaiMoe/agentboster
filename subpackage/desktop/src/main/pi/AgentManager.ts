	...
	private async createUnlocked(input: CreateAgentInput) {
		...
		try {
			...
			if (diag) {
				...
				// pi 路径与版本检测
				lines.push(`agentboster 路径: ${diag.command}`);
				...
				if (!diag.versionCheck) {
					lines.push("1. 在终端执行 agentboster --version，确认 agentboster 是否已安装且路径正确");
					lines.push("2. 如未安装，从 agentboster 仓库构建并放到 PATH（参见 subpackage/cli/AGENTS.md）");
					lines.push("3. 安装后再次在终端执行 agentboster --version 验证");
				} else if (diag.exitCode !== 0) {
					lines.push("1. 在终端执行 agentboster --mode rpc 看是否能正常启动");
					lines.push("2. 注意终端中的错误信息，根据异常信息修复");
				} else if (!stderrText && diag.exitCode === null) {
					lines.push("1. agentboster 进程可能尚未完成初始化，可在设置页增加 RPC 超时时间");
				} else {
					lines.push("1. 在终端执行 agentboster --mode rpc 确认 agentboster 能否正常启动");
					lines.push("2. 检查设置中的 agentboster 路径是否正确");
				}
				lines.push("");
