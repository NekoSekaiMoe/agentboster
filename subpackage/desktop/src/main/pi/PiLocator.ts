...
export class PiLocator {
  ...
  resolveCommand(customPath?: string) {
    ...
    return candidates.find(candidate => existsSync(candidate)) ?? "agentboster";
  }
  ...
  private getCandidates() {
    // Binary published by @agentboster-cli/core is `agentboster` on all
    // platforms (see subpackage/cli/packages/coding-agent/package.json bin).
    // Windows shims may also appear as agentboster.cmd / agentboster.exe
    // depending on the installer (npm global, scoop, etc.).
    const names =
      process.platform === "win32"
        ? ["agentboster.cmd", "agentboster.exe", "agentboster"]
        : ["agentboster"];
