...
export class DefaultPackageManager implements PackageManager {
  ...
  private async updateConfiguredSources(
    ...
    const npmCheckResults = await this.runWithConcurrency(
      npmCheckTasks,
    ...
    if (gitCandidates.length > 0) {
      ...
      tasks.push(
        this.runWithConcurrency(gitTasks, GIT_UPDATE_CONCURRENCY).then(
          () => {},
        ),
      );
    }
  ...
  private async updateNpmBatch(
    ...
    const sourceLabel =
      sources.length === 1 ? sources[0].source : `${scope} npm packages`;
    const message =
      sources.length === 1
    ...
    for (const { pkg, scope } of sources) {
      ...
      if (parsed.type === 'npm') {
        ...
        const needsInstall =
          ...
          !(await this.installedNpmMatchesConfiguredVersion(
            parsed,
            installedPath,
          ));
        if (needsInstall) {
  ...
  private async installNpm(
    ...
    await this.runNpmCommand(
      this.getNpmInstallArgs([source.spec], installRoot),
    );
  }
  ...
  private async ensureGitRef(
    ...
    const targetHead = await this.runCommandCapture(
      'git',
      ['rev-parse', commitRef],
      {
        cwd: targetDir,
  ...
  private getPnpmGlobalPackagePath(packageName: string): string | undefined {
    ...
    const output = this.runNpmCommandSync([
      'list',
      '-g',
      '--depth',
      '0',
  ...
  private getLegacyGlobalNpmInstallPath(source: NpmSource): string | undefined {
    try {
      return (
        this.getPnpmGlobalPackagePath(source.name) ??
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const root = this.resolveManagedPath(
      getExtensionTempFolder(this.agentDir),
      prefix,
    );
  ...
  private runCommandCapture(
    ...
    return new Promise((resolvePromise, reject) => {
      ...
      child.once('close', (code, signal) => {
        ...
        reject(
          new Error(
            `${command} ${args.join(' ')} failed with ${exitStatus}: ${stderr || stdout}`,
          ),
        );
      });
    });
