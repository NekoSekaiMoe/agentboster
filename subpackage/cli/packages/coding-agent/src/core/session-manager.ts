import { type AgentMessage, uuidv7 } from '@agentboster-cli/agent';
 ...
 */
function getDefaultSessionDirPath(
  cwd: string,
...
function getMessageActivityTime(
...
export class SessionManager {
  ...
  private constructor(
    ...
    // Session files go to the OS tmpdir, not ~/.config/agentboster/. They are
    ...
    if (existsSync(this.sessionFile)) {
      ...
      if (this.fileEntries.length === 0) {
        ...
        if (statSync(explicitPath).size > 0) {
          throw new Error(
            `Session file is not a valid pi session: ${explicitPath}`,
          );
        }
        this.newSession();
    ...
    for (const msg of messages) {
      ...
      const effectiveParts: RemotePart[] = (() => {
        if (msg.metadata?.versions && msg.metadata.versions.length > 0) {
          ...
          return msg.metadata.versions[
            Math.min(idx, msg.metadata.versions.length - 1)
          ].parts as RemotePart[];
        }
  ...
  static async list(
    ...
    const filterCwd =
      sessionDir !== undefined && dir !== getDefaultSessionDirPath(cwd);
    const resolvedCwd = resolvePath(cwd);
  ...
  static async listAll(
    ...
    const customSessionDir =
      typeof sessionDirOrOnProgress === 'string'
        ? normalizePath(sessionDirOrOnProgress)
        : undefined;
    const progress =
