...
// availability).
async function patchRemoteSessionTitle(
  sessionId: string,
  title: string,
): Promise<void> {
...
async function patchRemoteModelPref(modelId: string): Promise<void> {
  const { getStoredAuth, patchUserPreferences } = await import(
    '@agentboster/adapter'
  );
  const auth = getStoredAuth();
  if (!auth) return;
...
async function patchRemoteThinkingPref(
  ...
  const auth = getStoredAuth();
  if (!auth) return;
  await patchUserPreferences(auth.url, auth.token, {
    thinkingLevel: level,
  }).catch(() => {});
}
...
export class AgentSession {
  ...
  private _pendingRegenerateIntent: {
    messageId: string;
    metadata?: unknown;
  } | null = null;
  ...
  private async _emitExtensionEvent(event: AgentEvent): Promise<void> {
    if (event.type === 'agent_start') {
      ...
      const replacement =
        await this._extensionRunner.emitMessageEnd(extensionEvent);
      if (replacement) {
  ...
  setPendingRegenerateIntent(intent: {
    messageId: string;
    metadata?: unknown;
  }): void {
  ...
  async prompt(text: string, options?: PromptOptions): Promise<void> {
    ...
    if (disableTools && this.agent.state.tools.length > 0) {
      ...
      this.agent.state.systemPrompt =
        (this._baseSystemPrompt ?? this.agent.state.systemPrompt) + planSuffix;
    }
    try {
  ...
  async addWorkflowSubagentEvent(
  ...
  async addWorkflowSubagentBatchEvent(
    event: WorkflowSubagentBatchEventDetails,
  ): Promise<void> {
    const lines = [
      `Batch: ${event.batchId}`,
      `State: ${event.event}`,
  ...
  async compact(customInstructions?: string): Promise<CompactionResult> {
    ...
    try {
      ...
      const { apiKey, headers, env } = await this._getCompactionRequestAuth(
        this.model,
      );
  ...
  async exportToHtml(outputPath?: string): Promise<string> {
    ...
    const themeName =
      configuredThemeName && getThemeByName(configuredThemeName)
        ? configuredThemeName
        : undefined;
  ...
  createReplacedSessionContext(): ReplacedSessionContext {
    const context = Object.defineProperties(
      ...
      Object.getOwnPropertyDescriptors(
        this._extensionRunner.createCommandContext(),
      ),
