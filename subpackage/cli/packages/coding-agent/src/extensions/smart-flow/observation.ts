...
// Source: packages/pi-maestro-teammate/src/public/v1/observation.ts (verbatim)
...
export interface ObservationProvider {
  ...
  ): ObservationSnapshot | Promise<ObservationSnapshot>;
  wait(
    id: string,
...
function registry(): Map<string, ObservationProvider> {
  ...
  if (existing instanceof Map)
    return existing as Map<string, ObservationProvider>;
...
function pendingObservation(
  ...
  return {
    ...
    ...(waitStatus
      ? {
          waitStatus,
          outcome: waitStatus === 'aborted' ? ('aborted' as const) : undefined,
        }
      : {}),
...
function normalizedParams(
  ...
  Pick<
    ObserveParams,
    'detail' | 'lines' | 'waitMode' | 'waitCount' | 'timeoutMs' | 'until'
  >
> {
  return {
...
function validate(params: ObserveParams): void {
  if (params.targets.length === 0)
    throw new Error('Observe requires at least one target.');
  if (
    params.targets.some((target) => !target.kind.trim() || !target.id.trim())
  ) {
  ...
  if (
    ...
    (!Number.isInteger(params.timeoutMs) || params.timeoutMs < 1)
  ) {
    throw new Error('Observe timeoutMs must be a positive integer.');
  }
  if (params.waitMode === 'count') {
    if (
      !Number.isInteger(params.waitCount) ||
      (params.waitCount ?? 0) < 1 ||
      (params.waitCount ?? 0) > params.targets.length
    ) {
      throw new Error(
        'Observe waitCount must be between 1 and the number of targets.',
      );
    }
  }
}
      ...
      params.targets.map(async (target) => {
        ...
        if (!provider)
          return unavailable(
            target,
            'not-found',
            `No observation provider for kind "${target.kind}".`,
          );
        try {
      ...
      const pendingStatus =
        reason === 'timeout'
          ...
          : reason === 'aborted'
            ? 'aborted'
            : undefined;
      const completed = params.targets.map(
        (target, index) =>
          observations[index] ?? pendingObservation(target, pendingStatus),
      );
      resolve({
        action: 'wait',
        reason,
...
async function watchTargets(
  ...
  const pollMs = Math.min(
    1_000,
    Math.max(100, Math.round(options.timeoutMs / 10)),
  );
  const deadline = startedAt + options.timeoutMs;
  ...
  const snapshotAll = async (): Promise<void> => {
    await Promise.all(
      params.targets.map(async (target, index) => {
...
export function formatObserveResult(
  ...
  for (const observation of result.observations) {
    ...
    lines.push(
      `${label}\t${observation.nativeStatus}\t${observation.summary}`.trimEnd(),
    );
