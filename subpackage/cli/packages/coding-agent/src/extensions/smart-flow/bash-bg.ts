...
// Source: packages/pi-maestro-flow/src/tools/bash-bg.ts
...
export const BashBgParams = Type.Object({
  action: Type.Union(
    ...
    {
      description:
        'run: block up to timeout then auto-background (recommended default); start: background immediately; status: snapshot; wait: block until done/timeout; kill: terminate; list: all jobs',
    },
  ),
  command: Type.Optional(
    Type.String({
      minLength: 1,
      description: 'Shell command (required for run/start)',
    }),
  ),
  jobId: Type.Optional(
    Type.String({
      description:
        'Job id returned by run/start (required for status/wait/kill)',
    }),
  ),
  cwd: Type.Optional(
    Type.String({
      description: 'Working directory for run/start (default: session cwd)',
    }),
  ),
  timeout: Type.Optional(
    Type.Integer({
      ...
      maximum: 3600,
      description:
        'run: foreground seconds before auto-backgrounding; wait: max seconds to block (default 30)',
    }),
  ),
  tail: Type.Optional(
    Type.Integer({
...
function logAccess(job: Job): string {
  return [
    `log: ${job.outFile}`,
    ...(job.logTruncated
      ? [`log retention: truncated at ${job.logLimitBytes} bytes`]
      : []),
...
function jobDetails(
  ...
  return {
    ...
    command: job.command,
    ...(truncated
      ? { logPath: job.outFile, viewCommand: viewLogCommand(job) }
      : {}),
  };
}
...
function jobOwnsLiveProcessGroup(job: Job): boolean {
  return (
    process.platform !== 'win32' && job.done && processGroupRunning(job.pid)
  );
}
...
/**
 * Background shell jobs with async semantics: `start` returns a jobId
 * immediately, and completion injects a `bash-bg-complete` message with
 * triggerTurn so the agent wakes exactly like a background notification.
 *
 * Output is captured via pipes (inherited-fd stdio breaks under jiti loaders).
 */
...
export function registerBashBg(
  pi: ExtensionAPI,
  options: RegisterBashBgOptions = {},
): void {
  const maxActiveJobs = options.maxActiveJobs ?? MAX_ACTIVE_JOBS;
  const maxRetainedCompletedJobs =
    options.maxRetainedCompletedJobs ?? MAX_RETAINED_COMPLETED_JOBS;
  ...
  const waitForObservation = (
    ...
    if (!current.found || current.phase === 'settled') {
    ...
    return new Promise((resolve) => {
      ...
      const finish = (waitStatus?: 'timeout' | 'aborted'): void => {
        ...
        resolve(
          observationSnapshot(id, options.detail, options.lines, waitStatus),
        );
      };
      const onDone = () => {
  ...
  const bashObservationProvider: ObservationProvider = {
    ...
    capabilities: observationCapabilities,
    snapshot: (id, options) =>
      observationSnapshot(id, options.detail, options.lines),
  ...
  const pruneCompletedJobs = (): void => {
    ...
    while (completed.length > maxRetainedCompletedJobs) {
      ...
      try {
        fs.rmSync(evicted.outFile, { force: true });
      } catch {
        /* best effort for externally held log handles */
      }
    }
  };
  ...
  const initializeRuntime = (): void => {
    if (!disposed) return;
    runtimeGeneration += 1;
    baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentboster-bash-bg-'));
    ...
    disposeQueryListener =
      typeof queryDisposer === 'function' ? queryDisposer : undefined;
  };
  ...
  const waitForTerminationBoundary = async (
    job: Job,
    ...
    while (Date.now() < deadline) {
      if (job.done && (!includeProcessGroup || !processGroupRunning(job.pid))) {
        return true;
      }
      await delay(
  ...
  const terminateJob = (job: Job): Promise<void> => {
    ...
    const boundaryReached =
      job.done && (!includeProcessGroup || !processGroupRunning(job.pid));
    if (boundaryReached) {
    ...
    const termination = (async () => {
      ...
      if (!terminated) {
        const boundary = includeProcessGroup
          ? `POSIX process group ${job.pid}`
          : `process ${job.pid}`;
        const timeout = taskkillTimedOut ? ' (taskkill timed out)' : '';
        throw new Error(
          `Failed to terminate bash job ${job.id}: ${boundary} is still alive${timeout}.`,
        );
      }
      job.finishTermination?.();
    })().catch((error: unknown) => {
  ...
  const startJob = (
    ...
    if (disposed) {
      throw new Error(
        'bash_bg is unavailable outside an active session runtime.',
      );
    }
    ...
    if (activeJobs >= maxActiveJobs) {
      throw new Error(
        `Too many active background jobs (${activeJobs}/${maxActiveJobs}). Wait for or stop an existing job.`,
      );
    }
    ...
    const append = (d: Buffer): void => {
      ...
      if (remainingLogBytes > 0) {
        const retained =
          d.byteLength <= remainingLogBytes
            ? d
            : d.subarray(0, remainingLogBytes);
        ws.write(retained);
    ...
    job.finishTermination = (error) => {
      ...
      finalizeOutput();
      if (outputWasFinalized && !disposed && generation === runtimeGeneration) {
        publishSnapshot();
      }
      for (const settle of [...(observationWaiters.get(job.id) ?? [])]) {
        settle();
    ...
    // Do not let the child keep the CLI's loop alive; close still fires while
  ...
  pi.registerTool({
    ...
    description:
      ...
      'NOTE: this tool always executes on the local machine — it is never routed to a remote agentd node.',
    promptSnippet:
      'Run shell commands adaptively: action=run blocks like bash then auto-backgrounds on timeout; start/status/wait/kill/list for job control, with a completion notification that triggers a new turn. Local machine only.',
    promptGuidelines: [
      'Default to the bash tool for ordinary commands — blocking for tens of seconds is fine. Reach for bash_bg when a command is unbounded (dev server, watcher, tail -f), expected to run for minutes, or you want to keep working concurrently. bash_bg always runs on the local machine.',
    ...
    async execute(
      ...
      if (params.action === 'start') {
        if (!params.command) {
          throw new Error("bash_bg start requires 'command'.");
        }
      ...
      if (params.action === 'run') {
        if (!params.command) {
          throw new Error("bash_bg run requires 'command'.");
        }
        const job = startJob(
          params.command,
      ...
      if (params.action === 'list') {
        ...
        const lines = [...jobs.values()].map(
          (j) =>
            `${j.id}\t${jobStatus(j)}${j.done ? `(exit ${j.exitCode})` : ''}\tpid ${j.pid}\t${j.command}`,
        );
        return {
      ...
      if (!params.jobId) {
        throw new Error(`bash_bg ${params.action} requires 'jobId'.`);
      }
      ...
      if (params.action === 'status') {
        ...
        return {
          content: [
            { type: 'text', text: appendLogAccess(text, job, out.truncated) },
          ],
      ...
      if (params.action === 'kill') {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `${alreadyFinished ? 'Job already finished' : job.done ? 'Stopped job' : 'Stopping job'} ${job.id} (pid ${job.pid}).`,
            },
          ],
    ...
    renderCall(args, theme, ctx) {
      ...
      const target =
        args.action === 'start' || args.action === 'run'
          ? String(args.command ?? '').slice(0, 50)
          : String(args.jobId ?? '');
      return toolCallLine(
        theme,
        'bash_bg',
  ...
  // UIs can subscribe after this tool is registered, then request the
  // authoritative in-memory snapshot via the query event.

  // Re-announce running jobs after compaction: the in-memory registry survives
  // in-process compaction, but the compacted summary may drop pending-job awareness.
  pi.on('session_compact', () => {
    const running = [...jobs.values()].filter(jobIsActive);
    if (running.length === 0) return;
    const lines = running.map(
      (job) => `- ${job.id} (pid ${job.pid}): ${job.command}`,
    );
    const body = [
    ...
    pi.sendMessage(
      {
        ...
        content: body,
        display: true,
        details: { jobs: running.map((job) => job.id) },
      },
      { deliverAs: 'nextTurn' },
    );
  });

  // Session end terminates background jobs (no orphans); completed results
  // already persist as session messages, so resume keeps their history and output.
  pi.on('session_shutdown', async () => {
    ...
    if (typeof disposeObservationProvider === 'function') {
    ...
    try {
      ...
      /* open Windows handles may finish after shutdown */
    }
  });
}
