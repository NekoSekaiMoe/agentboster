...
export async function loginWithPassword(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Login failed (HTTP ${response.status}): ${text || response.statusText}`,
    );
  }
...
export async function exchangePairCode(
  ...
  const response = await fetch(
    `${url.replace(/\/$/, '')}/api/auth/pair-exchange`,
    {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  if (!response.ok) {
    const text = await response.text().catch(() => '');
    throw new Error(
      `Pair code exchange failed (HTTP ${response.status}): ${text || response.statusText}`,
    );
  }
  const payload = (await response.json()) as {
    ok: boolean;
