...
export interface Args {
  ...
  backendUrl?: string;
  /**
   * CLI session id to register with the Web backend. Generated by the
   * embedder (e.g. Desktop) so it knows which session to attach to IM
   * or to expose in its own UI.
   */
  cliSessionId?: string;
  name?: string;
  noSession?: boolean;
...
export function parseArgs(args: string[]): Args {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (arg === '--web-session-id' && i + 1 < args.length) {
      result.cliSessionId = args[++i];
