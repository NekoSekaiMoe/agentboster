...
/**
 * Amortized O(1) FIFO queue (two-stack). Replaces Array.shift()/unshift()
 * drains, which degrade quadratically when many events buffer between
 * producer and consumer (pi #9055).
 */
class FifoQueue<T> {
  private incoming: T[] = [];
  private outgoing: T[] = [];
