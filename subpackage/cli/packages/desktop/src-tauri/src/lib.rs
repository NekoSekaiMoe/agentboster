...
/// is a `.cmd` shim. As of the build-time-shim change, this `.cmd` is
/// generated by `prepare-resources.sh` and shipped inside the bundle's
/// `resources/` dir — it is present on every normal install.
///
/// This function returns the launcher path when the launcher+`.cjs`
/// pair is usable. On Windows, if only the `.cjs` is present (e.g. an
/// older bundle built before the shim was staged, or a read-only
/// install where the staged shim is somehow missing), it falls back to
/// synthesizing a sibling `.cmd` next to the `.cjs`. The fallback is
/// best-effort: on a read-only `resources/` dir the write silently
/// fails and we return `None`, letting discovery fall through to the
/// PATH / common-location tiers — which is the right thing rather than
/// spawning a launcher that can't find its `.cjs`.
///
/// Mirrors the installer's logic in `install_cli`.
///
/// Returns the launcher path on success.
fn ensure_cli_launcher(dir: &Path, triple: &str) -> Option<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        // Normal path: prepare-resources.sh staged it at build time and
        // it ships inside `resources/`. Fallback: synthesize it from the
        // `.cjs` (older bundle / missing staged shim). The shim uses
        // `%~dp0` (the batch var for "this script's dir") so it resolves
        // the `.cjs` next to itself regardless of the install path.
...
/// Minimum Node.js major version required to run the CLI bundle.
///
/// The CLI ships as a Node bundle (`agentboster-cli.cjs`) exec'd by a
/// thin wrapper / `.cmd` shim, so the Node runtime discovered at launch
/// time must be new enough for the bundle's syntax and deps. Keep this
/// in sync with `subpackage/cli` `engines.node`.
const MIN_NODE_MAJOR: u32 = 22;

/// Build the PATH that the launcher (`PathBinary` / `SidecarBinary`)
/// will actually see when it execs `node …/agentboster-cli.cjs`.
///
/// GUI launches (macOS Finder, Windows Explorer, Linux `.desktop`) often
/// don't inherit a shell PATH, so for `SidecarBinary` we prepend the
/// sidecar's own dir before falling back to the ambient PATH — mirroring
/// `prepend_bin_dir_to_path` / `build_command`. For `PathBinary` the
/// ambient PATH is what `which` already used, so it's kept as-is.
///
/// Returns `None` if `PATH` cannot be reassembled (path entries
/// containing the platform separator), in which case callers fall back
/// to the ambient environment.
fn build_launch_path(pi: &PiProcess) -> Option<PathBuf> {
    let mut entries: Vec<PathBuf> = Vec::new();
    if let PiProcess::SidecarBinary { path } = pi
...
/// Discover and validate the Node runtime the discovered launcher will
/// exec. Two failure modes, both surfaced through the same error
/// channel as `missing_pi_cli_error`:
///
///   - Node entirely missing  → "install Node ≥ 22" actionable message
///   - Node present but < 22    → "upgrade Node" message naming the detected version
///
/// We probe via the launch PATH (`build_launch_path`) rather than the
/// ambient PATH so the check reflects the exact `node` the wrapper will
/// invoke — a GUI launch may see a different `node` than a shell does
/// (nvm/asdf installs are the common culprit).
fn verify_node_runtime(pi: &PiProcess) -> Result<(), String> {
    let mut cmd = Command::new("node");
    cmd.arg("--version");
    cmd.stdin(Stdio::null());
    cmd.stdout(Stdio::piped());
    cmd.stderr(Stdio::piped());
    if let Some(launch_path) = build_launch_path(pi) {
        cmd.env("PATH", launch_path);
    }
    // Suppress any console window on Windows during the probe.
    ...
    let output = match cmd.output() {
        Ok(o) => o,
        Err(_) => return Err(missing_node_error(None)),
    };

    if !output.status.success() {
        // `node --version` should never fail on a healthy install; treat
        // a non-zero exit as "node unusable" and report the stderr.
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(missing_node_error(Some(stderr)));
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed = parse_node_major_version(&stdout);
    match parsed {
        Some(major) if major >= MIN_NODE_MAJOR => Ok(()),
        Some(major) => Err(node_version_error(major, &stdout)),
        None => Err(node_version_error(0, &stdout)),
    }
}

/// Parse the major version out of `node --version` output (`v22.11.0`,
/// `v23.0.0`, …). Returns `None` if the format is unrecognized.
...
fn missing_node_error(detail: Option<String>) -> String {
    let mut message = String::from(
        "Could not find Node.js. The agentboster CLI requires Node 22 or newer.\n\n\
         Install it from https://nodejs.org/ (or via your package manager:\n\
         `brew install node@22`, `nvm install 22`, `winget install OpenJS.NodeJS.LTS`,\n\
         …) and restart the app.",
    );
...
fn node_version_error(detected_major: u32, raw_output: &str) -> String {
    format!(
        "Node.js {detected} is too old. The agentboster CLI requires Node {min} or newer.\n\n\
         Upgrade at https://nodejs.org/ (reported version: {raw}).",
        detected = if detected_major == 0 {
            "(unknown)".to_string()
        } else {
            format!("v{}", detected_major)
        },
        min = MIN_NODE_MAJOR,
        raw = raw_output.trim(),
    )
...
/// Discover the pi binary and verify the Node runtime it depends on.
///
/// Strategy:
...
///
/// After a launcher is resolved we also verify the Node runtime it will
/// exec — the CLI bundle (`agentboster-cli.cjs`) requires Node ≥ 22, and
/// a GUI launch may pick up a different/older `node` than a shell does.
fn discover_pi(app: &AppHandle, options: &RpcStartOptions) -> Result<PiProcess, String> {
    let pi = resolve_pi_process(app, options)?;
    verify_node_runtime(&pi)?;
    Ok(pi)
}

fn resolve_pi_process(app: &AppHandle, options: &RpcStartOptions) -> Result<PiProcess, String> {
