...
/// Remove provider credentials from ~/.agentboster/agent/auth.json when present.
#[tauri::command]
...
async fn get_cli_update_status(
    ...
    let update_command = "npm install -g @agentboster-cli/core@latest".to_string();
...
async fn update_cli_via_npm() -> Result<NpmCommandResult, String> {
    ...
    cmd.arg("install")
        .arg("-g")
        .arg("@agentboster-cli/core@latest")
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
