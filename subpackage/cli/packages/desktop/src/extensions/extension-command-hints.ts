...
function descriptionAlreadyContainsHint(
  commandName: string,
  description: string,
): boolean {
  const normalized = normalizeExtensionCommandName(commandName);
...
function shouldUseVoiceNotifyCanonicalDescription(
  commandName: string,
  description: string,
): boolean {
  const normalized = normalizeExtensionCommandName(commandName);
...
export function withExtensionCommandUsageHint(
  commandName: string,
  description: string,
): string {
  const normalizedDescription = normalizeText(description);
  if (
    shouldUseVoiceNotifyCanonicalDescription(commandName, normalizedDescription)
  ) {
  ...
  if (descriptionAlreadyContainsHint(commandName, normalizedDescription))
