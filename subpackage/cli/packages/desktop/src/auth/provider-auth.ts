...
export function isOAuthProviderId(
  provider: string,
  catalog: Map<string, OAuthProviderCatalogEntry>,
): boolean {
  const key = normalizeProviderKey(provider);
...
export function createDefaultOAuthProviderCatalog(): Map<
  string,
...
export function normalizeOAuthProviderCatalog(
  rawEntries: unknown,
): Map<string, OAuthProviderCatalogEntry> {
...
export function collectBuiltInOAuthProviderIds(
  rawEntries: unknown,
): Set<string> {
...
export function normalizeConfiguredProviderAuth(
  ...
  for (const entry of providers) {
    ...
    next.set(provider, {
      source:
        sourceRaw === 'environment' ||
        sourceRaw === 'auth_file_api_key' ||
        sourceRaw === 'auth_file_oauth'
          ? sourceRaw
          : 'auth_file_api_key',
      kind:
        kindRaw === 'api_key' || kindRaw === 'oauth' || kindRaw === 'unknown'
          ? kindRaw
          : 'unknown',
    });
  }
...
export function resolveProviderSetupCommand(
  provider: string,
  ...
  for (const command of runtimeCommands) {
    ...
    const tokenHits = providerTokens.filter((token) =>
