...
function payloadInstanceId(
  ...
  return normalizeInstanceId(
    payload.instance_id ?? payload.instanceId ?? 'default',
  );
}

function payloadGeneration(
  payload: RpcLineEventPayload | RpcClosedEventPayload,
): number | null {
...
function extractRuntimeErrorFromTextLine(line: string): string | null {
  ...
  const hasFailureWord =
    /\b(failed|failure|cannot|can't|denied|unavailable)\b/i.test(cleaned);
  const hasActionableHint = ACTIONABLE_RUNTIME_ERROR_HINTS.some((hint) =>
  ...
  if (
    ...
    (normalized.includes('requests') || normalized.includes('rate limit'))
  ) {
    return cleaned;
  }
...
export class RpcBridge {
  ...
  async start(options: RpcStartOptions): Promise<string> {
    ...
    const effectivePiPath =
      this.normalizePathOverride(options.piPath) ?? this.preferredPiPath;
    ...
    try {
      ...
      this.currentGeneration =
        ...
        Number.isFinite(result.generation)
          ? result.generation
      ...
      traceBridge(
        `started instance=${this.instanceId} generation=${this.currentGeneration ?? -1} discovery=${result.discovery}`,
      );
      ...
      this.pendingGeneration = null;
      traceBridge(
        `start-failed instance=${this.instanceId}: ${err instanceof Error ? err.message : String(err)}`,
      );
      throw err;
    }
  }
  ...
  async refreshRunningState(): Promise<boolean> {
    const running = await invoke<boolean>('rpc_is_running', {
      instanceId: this.instanceId,
    });
  ...
  onEvent(callback: RpcEventCallback): () => void {
    void this.ensureListeners().catch((err) => {
      traceBridge(
        `listener-init-failed instance=${this.instanceId}: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  ...
  async getAvailableModels(): Promise<Array<Record<string, unknown>>> {
    ...
    const data = this.getData<{ models: Array<Record<string, unknown>> }>(
      response,
    );
    return data.models;
  }
  ...
  async getMessages(): Promise<Array<Record<string, unknown>>> {
    ...
    const data = this.getData<{ messages: Array<Record<string, unknown>> }>(
      response,
    );
  ...
  async getCommands(): Promise<Array<Record<string, unknown>>> {
    ...
    const data = this.getData<{ commands: Array<Record<string, unknown>> }>(
      response,
    );
  ...
  async runPiCliCommand(
    ...
    const cliPath = this.normalizePathOverride(
      typeof options.cliPath !== 'undefined'
        ? options.cliPath
        : (this.lastStartOptions?.cliPath ?? null),
    );
  ...
  async checkRpcCompatibility(): Promise<RpcCompatibilityReport> {
    ...
    if (!this.isConnected) {
      return {
        ...
        checks,
        error:
          'No active RPC runtime. Open a project/session before running compatibility checks.',
        checkedAt: Date.now(),
      };
    }
    try {
  ...
  private matchesPayloadGeneration(
    payload: RpcLineEventPayload | RpcClosedEventPayload,
  ): boolean {
  ...
  private emitToListeners(event: Record<string, unknown>): void {
    for (const listener of this.eventListeners) {
      try {
        ...
        traceBridge(
          `listener-error instance=${this.instanceId}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
  }
  ...
  private async ensureListeners(): Promise<void> {
    ...
    this.listenersReadyPromise = (async () => {
      ...
      try {
        unlistenEventLocal = await listen<RpcLineEventPayload>(
          'rpc-event',
          (event) => {
        ...
        unlistenClosedLocal = await listen<RpcClosedEventPayload>(
          'rpc-closed',
          (event) => {
            const payload = event.payload;
            if (payloadInstanceId(payload) !== this.instanceId) return;
            ...
            this._isConnected = false;
            traceBridge(
              `closed instance=${this.instanceId} generation=${payload.generation ?? -1} reason=${typeof payload.reason === 'string' ? payload.reason : 'RPC process closed'}`,
            );
        ...
        unlistenStderrLocal = await listen<RpcLineEventPayload>(
          'rpc-stderr',
          (event) => {
            const payload = event.payload;
            if (payloadInstanceId(payload) !== this.instanceId) return;
            ...
            traceBridge(
              `stderr-error instance=${this.instanceId} message=${runtimeError.slice(0, 180)}`,
            );
            this.emitToListeners({
              type: 'error',
              source: 'stderr',
  ...
  private handleLine(line: string): void {
    ...
    try {
      ...
      if (
        this.parseFailureCount <= 5 ||
        sanitized.includes('extension_ui_request') ||
        sanitized.includes('notify')
      ) {
        traceBridge(
          `parse-failed instance=${this.instanceId} sample=${sanitized.slice(0, 180)}`,
        );
      }
      const runtimeError = extractRuntimeErrorFromTextLine(line);
      if (runtimeError) {
        traceBridge(
          `stdout-text-error instance=${this.instanceId} message=${runtimeError.slice(0, 180)}`,
        );
        this.emitToListeners({
          type: 'error',
          source: 'stdout_text',
    ...
    if (
      ...
      traceBridge(
        `response instance=${this.instanceId} id=${data.id} command=${String(data.command ?? '-')} success=${data.success === false ? 'no' : 'yes'}`,
      );
  ...
  private timeoutMsForCommand(command: Record<string, unknown>): number {
    const type =
      typeof command.type === 'string' ? command.type.trim().toLowerCase() : '';
    switch (type) {
      case 'compact':
        return 5 * 60_000;
      default:
  ...
  private async send(
    ...
    return new Promise((resolve, reject) => {
      ...
      traceBridge(
        `send instance=${this.instanceId} id=${id} command=${String(command.type)} timeoutMs=${timeoutMs}`,
      );
      const timeout = setTimeout(() => {
        this.pendingRequests.delete(id);
        traceBridge(
          `timeout instance=${this.instanceId} id=${id} command=${String(command.type)} timeoutMs=${timeoutMs}`,
        );
        reject(
          new Error(`Timeout waiting for response to ${String(command.type)}`),
        );
      }, timeoutMs);
      ...
      invoke('rpc_send', {
        ...
        this.pendingRequests.delete(id);
        traceBridge(
          `send-failed instance=${this.instanceId} id=${id} command=${String(command.type)}: ${String(err)}`,
        );
  ...
  private getData<T = Record<string, unknown>>(
    response: Record<string, unknown>,
  ): T {
...
class ActiveRpcBridgeProxy {
  ...
  setActiveBridge(bridge: RpcBridge): void {
    ...
    for (const listener of listeners) {
      this.listenerUnsubscribers.set(
        listener,
        this.activeBridge.onEvent(listener),
      );
    }
  }
  ...
  onEvent(callback: RpcEventCallback): () => void {
    ...
    this.listenerUnsubscribers.set(
      callback,
      this.activeBridge.onEvent(callback),
    );
    return () => {
