...
export type SlashPaletteSection =
  | 'CLI'
  | 'Extensions'
  | 'Prompts'
  | 'Skills'
...
export function normalizeRuntimeSlashCommand(
  ...
  if (!name) return null;
  const description = withRuntimeCommandUsageHint(
    name,
    normalizeText(raw.description) || `Run /${name}`,
  );
  return {
    name,
    description,
...
export function normalizeRuntimeSlashCommands(
  rawCommands: Array<Record<string, unknown>>,
): RuntimeSlashCommand[] {
  ...
  for (const raw of rawCommands) {
    ...
    if (!parsed) continue;
    const sourceKey =
      parsed.source === 'other' ? parsed.rawSource || 'other' : parsed.source;
...
export function createSlashPaletteItems(
  runtimeCommands: RuntimeSlashCommand[],
): SlashPaletteItem[] {
  const builtinItems: SlashPaletteItem[] = BUILTIN_SLASH_COMMANDS.map(
    (command) => ({
...
export function filterSlashPaletteItemsByQuery(
  ...
  for (const item of items) {
    if (
      ...
      continue;
    if (
      item.commandName.startsWith(query) ||
...
export function findSlashPaletteItemByName(
  items: SlashPaletteItem[],
  commandName: string,
