...
export function resolveProviderHintFromModelArg(
  rawArg: string,
  providerPool: ModelOption[],
): string | null {
  const arg = rawArg.trim().replace(/^\/+/, '');
  if (!arg) return null;

  const byDelim = arg.includes('/')
    ? arg.split('/')[0]
    : arg.includes('::')
      ? arg.split('::')[0]
      : arg;
  ...
  const partial = providers.find((provider) =>
    provider.toLowerCase().includes(token),
  );
  if (partial) return partial;

  const fuzzy = providerPool.filter((model) =>
    `${model.provider}/${model.id}`.toLowerCase().includes(token),
  );
  if (fuzzy.length > 0) {
...
export function resolvePreferredModelPickerProvider(
  rawPreferredProvider: string,
  providerPool: ModelOption[],
): string {
  const preferred = rawPreferredProvider.trim().toLowerCase();
  if (!preferred) return '';
  const exact = providerPool.find(
    (model) => model.provider.toLowerCase() === preferred,
  )?.provider;
  if (exact) return exact;
  const partial = providerPool.find((model) =>
    model.provider.toLowerCase().includes(preferred),
  )?.provider;
  return partial ?? '';
}

export function resolveModelCandidateFromArg(
  rawArg: string,
  ...
  const exactById = availableModels.find(
    (model) => model.id.toLowerCase() === lower,
  );
  if (exactById) return exactById;
  const fuzzy = availableModels.filter((model) =>
