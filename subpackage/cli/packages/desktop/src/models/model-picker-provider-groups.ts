...
export type ModelPickerAuthSource =
  | PiAuthProviderStatus['source']
  | 'runtime'
  | 'missing';
...
export function buildModelPickerProviderGroups({
  ...
  for (const model of combinedByKey.values()) {
    ...
    const selectable =
      availableByKey.has(modelKey(model.provider, model.id)) ||
  ...
  return Array.from(groupedByProvider.values())
    .map((group) => {
      ...
      const isDefaultOAuthProvider = isOAuthProviderId(
        authKey,
        oauthProviderCatalog,
      );
      const authConfigured =
        !forcedLoggedOut &&
        (providerAuthConfigured.has(authKey) ||
          (!isDefaultOAuthProvider && hasSelectableModel));
      ...
      return {
        ...
        models: [...group.models].sort((a, b) =>
          formatModelDisplayName(a.id).localeCompare(
            formatModelDisplayName(b.id),
            undefined,
            { sensitivity: 'base' },
          ),
        ),
...
export function resolveActiveModelPickerProvider(
  ...
  if (
    providerGroups.some(
      (group) => group.providerKey === modelPickerActiveProvider,
    )
  ) {
