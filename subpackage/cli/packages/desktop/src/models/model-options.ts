...
export function formatModelDisplayName(modelId: string): string {
  ...
  value = value.replace(
    /^(openai|anthropic|google|xai|openrouter|ollama|lmstudio)[:/]/i,
    '',
  );
...
export function parseListModelsCatalog(output: string): ModelOption[] {
  ...
  mapped.sort((a, b) => {
    const providerCompare = formatProviderDisplayName(a.provider).localeCompare(
      formatProviderDisplayName(b.provider),
      undefined,
      {
        sensitivity: 'base',
      },
    ...
    return formatModelDisplayName(a.id).localeCompare(
