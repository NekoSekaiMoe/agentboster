...
function pickPreferredRemoteCandidate(
  candidates: RemoteBranchCandidate[],
): RemoteBranchCandidate {
  const unique = new Map<string, RemoteBranchCandidate>();
  for (const candidate of candidates) {
    if (!unique.has(candidate.fullName))
      unique.set(candidate.fullName, candidate);
  }
...
export function findGitBranchEntryByQuery(
  query: string,
  entries: GitBranchEntry[],
): GitBranchEntry | null {
  ...
  const exactLocalByName = entries.find(
    (entry) => entry.scope === 'local' && entry.name === normalized,
  );
  if (exactLocalByName) return exactLocalByName;

  const exactByFullName = entries.find(
    (entry) => entry.fullName === normalized,
  );
  ...
  const caseInsensitiveLocal = entries.find(
    (entry) => entry.scope === 'local' && entry.name.toLowerCase() === needle,
  );
  if (caseInsensitiveLocal) return caseInsensitiveLocal;

  const caseInsensitiveByFullName = entries.find(
    (entry) => entry.fullName.toLowerCase() === needle,
  );
