...
import {
  deriveForkSessionName,
  buildForkEntryIdByMessageId,
  resolveForkEntryId,
} from './chat-view/history-fork-utils.js';
import {
...
import type {
  ...
  HistoryTreeRow,
  HistoryViewerRole,
} from './chat-view/history-viewer-types.js';
...
const THINKING_LEVEL_CYCLE_ORDER: ThinkingLevel[] = [
  'off',
  'minimal',
  'low',
  'medium',
...
export class ChatView {
  ...
  private onOpenTerminal: ((command?: string) => void | Promise<void>) | null =
    null;
  ...
  private onOpenExtensionConfig:
    | ((commandName: string, args: string) => boolean | Promise<boolean>)
    | null = null;
  private onOpenProviderConfig:
    | ((provider: string) => boolean | Promise<boolean>)
    | null = null;
  private onBeginRenameCurrentSession:
    | (() => boolean | Promise<boolean>)
    | null = null;
  private onRenameCurrentSession:
    | ((nextName: string) => boolean | Promise<boolean>)
    | null = null;
  private onCreateFreshSession: (() => boolean | Promise<boolean>) | null =
    null;
  ...
  private providerAuthById = new Map<
    string,
    Pick<PiAuthProviderStatus, 'source' | 'kind'>
  >();
  ...
  private unsupportedThinkingLevelsByModel = new Map<
    string,
    Set<ThinkingLevel>
  >();
  ...
  private runningProviderAuthAction: {
    provider: string;
    action: 'login' | 'logout';
  } | null = null;
  ...
  private readonly welcomeHeadlines = [
    'Ready when you are',
    'Your move when you’re back',
    'Come back when you want, I’m here',
    'I’m waiting for you',
  ];
  ...
  setOnOpenExtensionConfig(
    cb: (commandName: string, args: string) => boolean | Promise<boolean>,
  ): void {
    this.onOpenExtensionConfig = cb;
  }

  setOnOpenProviderConfig(
    cb: (provider: string) => boolean | Promise<boolean>,
  ): void {
  ...
  setOnRenameCurrentSession(
    cb: (nextName: string) => boolean | Promise<boolean>,
  ): void {
  ...
  setWelcomeProjects(
    ...
    this.welcomeProjects = projects
      .filter(
        (entry) =>
          Boolean(entry?.id) && Boolean(entry?.name) && Boolean(entry?.path),
      )
  ...
  setProjectPath(path: string | null): void {
    ...
    if (!path) {
      ...
      this.welcomeHeadlineIndex =
        (this.welcomeHeadlineIndex + 1) % this.welcomeHeadlines.length;
  ...
  prepareForSessionSwitch(
    ...
    this.providerAuthForcedLoggedOut.clear();
    this.resetRunActivityState();
    this.bindingStatusText = projectPath
  ...
  private parseComposerSkillDraftFromCommand(
    commandText: string,
  ): ComposerSkillDraft | null {
  ...
  private parseSlashInput(
    value: string,
  ): { commandText: string; commandName: string; args: string } | null {
  ...
  private async ensureSlashCommandsLoaded(force = false): Promise<void> {
    ...
    if (
      !force &&
      this.slashRuntimeCommands.length > 0 &&
      Date.now() - this.slashCommandsUpdatedAt < 15_000
    )
      return;
    ...
    try {
      ...
      this.slashRuntimeCommands = normalizeRuntimeSlashCommands(
  ...
  private getSlashPaletteItems(): SlashPaletteItem[] {
    ...
    return filterSlashPaletteItemsByQuery(
      this.buildAllSlashPaletteItems(),
      this.slashPaletteQuery,
    );
  }

  private findSlashPaletteItemByName(
    commandName: string,
  ): SlashPaletteItem | null {
    return findSlashPaletteItemByName(
      this.buildAllSlashPaletteItems(),
      commandName,
    );
  }
  ...
  private isUnknownRuntimeModel(provider: string, modelId: string): boolean {
    ...
    return (
      providerKey === 'unknown' &&
      (modelKey.length === 0 || modelKey === 'unknown')
    );
  }

  private currentModelSelection(
    state: RpcSessionState | null | undefined = this.state,
  ...
  private async loadOAuthProviderCatalog(force = false): Promise<void> {
    ...
    const stale =
      Date.now() - this.oauthProviderCatalogLoadedAt >
      MODEL_PICKER_AUTH_CACHE_MS;
  ...
  private async loadProviderAuthStatus(force = false): Promise<void> {
    ...
    const stale =
      Date.now() - this.providerAuthLoadedAt > MODEL_PICKER_AUTH_CACHE_MS;
  ...
  private async loadModelCatalog(force = false): Promise<void> {
    ...
    const stale =
      Date.now() - this.modelCatalogLoadedAt > MODEL_PICKER_CATALOG_CACHE_MS;
    ...
    try {
      ...
      if (result.exit_code !== 0) {
        throw new Error(
          result.stderr ||
            result.stdout ||
            `pi --list-models failed with exit ${result.exit_code}`,
        );
      }
  ...
  private resolveProviderSetupCommand(provider: string): string | null {
    return resolveProviderSetupCommandForProvider(
      provider,
      this.slashRuntimeCommands,
    );
  }
  ...
  private async handleProviderAuthAction(
    provider: string,
    action: 'login' | 'logout',
  ): Promise<void> {
    const providerKey = this.providerKey(provider);
    ...
    try {
      if (action === 'login') {
        ...
        if (this.isOAuthProviderId(providerKey)) {
          ...
          if (this.onOpenTerminal) {
            ...
            this.pushNotice(
              `Opened terminal and started /login for ${providerLabel}.`,
              'info',
            );
            return;
          }
      ...
      if (result.removed) {
        ...
        this.pushNotice(
          `${providerLabel} is configured via environment variable; remove env var to fully log out.`,
          'info',
        );
      } else {
        this.providerAuthForcedLoggedOut.add(providerKey);
        ...
        this.recomputeProviderAuthConfigured();
        this.pushNotice(
          `No stored auth.json credentials found for ${providerLabel}`,
          'info',
        );
      }
      ...
      console.error(
        `Provider auth action failed (${action}:${providerKey}):`,
        err,
      );
      this.pushNotice(
        err instanceof Error ? err.message : 'Provider auth action failed',
        'error',
      );
    } finally {
      this.runningProviderAuthAction = null;
  ...
  private async switchAwayFromLoggedOutProvider(
    providerKey: string,
  ): Promise<void> {
    ...
    const fallback =
      this.availableModels.find(
        (model) => this.providerKey(model.provider) !== providerKey,
      ) ?? null;
    if (!fallback) {
      this.pushNotice(
        'No other authenticated models available. Log in to another provider.',
        'info',
      );
      return;
    }
  ...
  private async pickSessionExportPathFromDialog(): Promise<string | null> {
    try {
      ...
      const basePath = this.projectPath
        ? `${this.projectPath.replace(/\\/g, '/')}/session.html`
        : 'session.html';
  ...
  private async executeSlashCommandFromComposer(): Promise<void> {
    ...
    if (
      ...
      this.pushNotice(
        'Slash commands cannot be sent with pending attachments',
        'info',
      );
      return;
    }
    ...
    if (parsed) {
      ...
      if (exact) {
        if (
          exact.source === 'skill' &&
          this.stageSkillSlashCommand(parsed.commandText)
        )
          return;
    ...
    if (this.slashPaletteOpen && liveItems.length > 0) {
      const picked =
        liveItems[
          Math.max(0, Math.min(this.slashPaletteIndex, liveItems.length - 1))
        ];
      const pickedArgs =
        parsed && parsed.commandName === picked.commandName ? parsed.args : '';
      const commandText = `/${picked.commandName}${pickedArgs ? ` ${pickedArgs}` : ''}`;
      if (picked.source === 'skill' && this.stageSkillSlashCommand(commandText))
        return;
  ...
  private async runSlashCommand(
    ...
    try {
      if (item.source === 'builtin') {
        ...
        await this.executeRuntimeSlashCommand(
          trimmedCommandText,
          item.source,
          item.commandName,
          args,
  ...
  private async executeRuntimeSlashCommand(
    ...
    const options = this.currentIsStreaming()
  ...
  private openModelPicker(options: { preferredProvider?: string } = {}): void {
    ...
    const preferred = resolvePreferredModelPickerProvider(
      normalizeText(options.preferredProvider),
      providerPool,
    );
    if (preferred) {
  ...
  private resolveProviderHintFromModelArg(rawArg: string): string | null {
    return resolveProviderHintFromModelArg(rawArg, [
  ...
  private async executeBuiltinSlashCommand(
    ...
    await executeBuiltinSlashCommandView({
      ...
      resolveModelCandidateFromArg:
        this.resolveModelCandidateFromArg.bind(this),
      resolveProviderHintFromModelArg:
        this.resolveProviderHintFromModelArg.bind(this),
      setModel: this.setModel.bind(this),
      unwrapQuotedArg: this.unwrapQuotedArg.bind(this),
      pickSessionExportPathFromDialog:
        this.pickSessionExportPathFromDialog.bind(this),
      ...
      loadPiAgentChangelogMarkdown:
        this.loadPiAgentChangelogMarkdown.bind(this),
      extractLatestChangelogSections:
        this.extractLatestChangelogSections.bind(this),
  ...
  private previewSlashPaletteItem(item: SlashPaletteItem): void {
    const parsed = this.parseSlashInput(this.inputText);
    const args =
      parsed && parsed.commandName === item.commandName ? parsed.args : '';
  ...
  private selectSlashPaletteItem(item: SlashPaletteItem): void {
    ...
    const args =
      parsed && parsed.commandName === item.commandName ? parsed.args : '';
    const commandText = `/${item.commandName}${args ? ` ${args}` : ''}`;
    if (item.source === 'skill' && this.stageSkillSlashCommand(commandText))
      return;
  ...
  private async bindNativeFileDropListener(): Promise<void> {
    ...
    const handler = (event: { payload?: unknown }) => {
      ...
      if (
        this.handleDroppedPathCandidates(nativePaths, {
      ...
      if (sidebarFallbackPaths.length > 0) {
        const handledFromSidebarFallback = this.handleDroppedPathCandidates(
          sidebarFallbackPaths,
          {
            quietImageReadFailure: true,
          },
        );
  ...
  private bindModelPickerGlobalListeners(): void {
    if (this.modelPickerGlobalListenersBound || typeof document === 'undefined')
      return;
    document.addEventListener(
      'pointerdown',
      this.onGlobalPointerDownForModelPicker,
      true,
    );
    document.addEventListener(
      'mousedown',
    ...
    document.addEventListener(
      'keydown',
      this.onGlobalEscapeForModelPicker,
      true,
    );
  ...
  private unbindModelPickerGlobalListeners(): void {
    if (
      !this.modelPickerGlobalListenersBound ||
  ...
  connect(): void {
    ...
    this.unsubscribeEvents = rpcBridge.onEvent((event) =>
  ...
  async refreshFromBackend(
    ...
    try {
      const [state, backendMessages] = await Promise.all([
        rpcBridge.getState(),
        rpcBridge.getMessages(),
      ]);
      if (requestInstanceId !== rpcBridge.getInstanceId()) {
        push?.(
          `chat:refreshFromBackend stale instance=${requestInstanceId} active=${rpcBridge.getInstanceId()}`,
        );
        return;
      }
      this.isConnected = rpcBridge.isConnected;
      ...
      if (this.compactionInsertIndex !== null) {
        this.compactionInsertIndex = Math.max(
          0,
          Math.min(this.compactionInsertIndex, this.messages.length),
        );
      }
      this.expandedToolWorkflowIds.clear();
      this.expandedToolGroupByWorkflowId.clear();
      ...
      this.lastAssistantContextTokens =
        this.deriveLatestAssistantContextTokens(backendMessages);
      if (state.isStreaming) {
        ...
        const streamWindow =
          lastUserIndex >= 0
            ? backendMessages.slice(lastUserIndex + 1)
            : backendMessages;
        this.runHasAssistantText = streamWindow.some((entry) => {
          if ((entry.role as string) !== 'assistant') return false;
          return (
            this.extractText((entry as Record<string, unknown>).content).trim()
              .length > 0
          );
        });
        ...
        this.runSawToolActivity =
          this.runSawToolActivity || sawToolInStreamWindow;
        if (this.runSawToolActivity) {
          this.keepWorkflowExpandedUntilAssistantText =
            !this.runHasAssistantText;
        }
      } else {
        this.runHasAssistantText = false;
        this.runSawToolActivity = false;
      ...
      if (
        !this.oauthProviderCatalogLoading &&
        this.oauthProviderCatalogLoadedAt === 0
      ) {
        void this.loadOAuthProviderCatalog();
      }
      if (!this.loadingModelCatalog && this.modelCatalog.length === 0) {
      ...
      push?.(
        `chat:refreshFromBackend ok session=${state.sessionFile ?? '-'} messages=${backendMessages.length}`,
      );
    } catch (err) {
      console.error('Failed to refresh chat state:', err);
      this.lastBackendRefreshError =
  ...
  private extractAssistantPartialContent(
  ...
  private async loadAvailableModels(): Promise<void> {
    ...
    push?.(
      `chat:loadModels start instance=${rpcBridge.getInstanceId()} seq=${requestSeq}`,
    );
    ...
    try {
      ...
      if (requestInstanceId !== rpcBridge.getInstanceId()) {
        push?.(
          `chat:loadModels stale instance=${requestInstanceId} active=${rpcBridge.getInstanceId()}`,
        );
        return;
      }
      ...
      if (this.availableModels.length === 0) {
        this.pushNotice(
          `Could not load models right now: ${truncate(this.lastModelLoadError, 120)}`,
          'info',
        );
      }
    } finally {
  ...
  private markThinkingLevelUnsupported(
    level: ThinkingLevel,
    state: RpcSessionState | null | undefined = this.state,
  ): void {
    ...
    if (!key) return;
    const existing =
      this.unsupportedThinkingLevelsByModel.get(key) ??
      new Set<ThinkingLevel>();
    existing.add(level);
    this.unsupportedThinkingLevelsByModel.set(key, existing);
  }

  private clearThinkingLevelUnsupported(
    level: ThinkingLevel,
    state: RpcSessionState | null | undefined = this.state,
  ): void {
    const key = this.thinkingLevelModelKey(state);
  ...
  private unsupportedThinkingLevelsForCurrentModel(): Set<ThinkingLevel> {
    ...
    return (
      this.unsupportedThinkingLevelsByModel.get(key) ?? new Set<ThinkingLevel>()
    );
  }
  ...
  private async setThinkingLevel(
    ...
    try {
      ...
      if (requestedLevel === 'xhigh' && this.state?.thinkingLevel !== 'xhigh') {
        this.pushNotice(
          `xhigh is not available for this model (using ${this.state?.thinkingLevel || 'high'})`,
          'info',
        );
      }
      void this.refreshSessionStats(true);
  ...
  private async cycleThinkingLevel(direction: 1 | -1 = 1): Promise<void> {
    ...
    let cursor = Math.max(
      0,
      order.indexOf((this.state?.thinkingLevel ?? 'off') as ThinkingLevel),
    );
    ...
    for (let attempt = 0; attempt < order.length; attempt += 1) {
      ...
      for (let step = 1; step <= order.length; step += 1) {
        const nextIndex =
          (cursor + step * direction + order.length * 2) % order.length;
      ...
      const appliedIndex = order.indexOf(
        (applied ?? candidate) as ThinkingLevel,
      );
      if (appliedIndex >= 0) {
        cursor = appliedIndex;
      }
  ...
  private resolveContextWindow(raw?: Record<string, unknown>): number | null {
    const stateWindow =
      typeof this.state?.model?.contextWindow === 'number' &&
      Number.isFinite(this.state.model.contextWindow)
    ...
    if (provider && modelId) {
      const fromCatalog = this.availableModels.find(
        (m) => m.provider === provider && m.id === modelId,
      )?.contextWindow;
      if (
        typeof fromCatalog === 'number' &&
        Number.isFinite(fromCatalog) &&
        fromCatalog > 0
      ) {
        return fromCatalog;
      }
  ...
  private deriveLatestAssistantContextTokens(
  ...
  private async refreshSessionStats(force = false): Promise<void> {
    ...
    try {
      const raw = (await rpcBridge.getSessionStats()) as Record<
        string,
        unknown
      >;
  ...
  private sessionStatsLines(): string[] {
    ...
    if (this.sessionStats.tokens !== null) {
      parts.push(
        `Context tokens: ${Math.round(this.sessionStats.tokens).toLocaleString()}`,
      );
    }
    if (this.sessionStats.contextWindow) {
      parts.push(
        `Context window: ${Math.round(this.sessionStats.contextWindow).toLocaleString()}`,
      );
    }
    ...
    if (this.sessionStats.lifetimeTokens !== null) {
      parts.push(
        `Session tokens total: ${Math.round(this.sessionStats.lifetimeTokens).toLocaleString()}`,
      );
    }
  ...
  private extractRuntimeErrorMessage(
    ...
    if (nestedError && typeof nestedError === 'object') {
      return (
        pickString(nestedError as Record<string, unknown>, [
          'message',
          'error',
          'detail',
          'reason',
  ...
  private extractAssistantMessageError(
    message: Record<string, unknown> | null | undefined,
  ): string {
    if (!message || typeof message !== 'object') return '';
    const stopReason =
      pickString(message, ['stopReason', 'stop_reason', 'reason'])
        ?.trim()
        .toLowerCase() ?? '';
    ...
    if (stopReason === 'aborted') {
      if (
        errorMessage &&
        errorMessage.toLowerCase() !== 'request was aborted'
      ) {
        return errorMessage;
      }
  ...
  private appendSystemMessage(
    ...
    const isCollapsible = Boolean(
      options.collapsibleTitle && options.collapsibleTitle.trim().length > 0,
    );
    this.messages.push({
      ...
      collapsibleExpanded: isCollapsible
  ...
  private pushRuntimeNotice(
    ...
    if (
      ...
      now - this.lastRuntimeNoticeAt < dedupeMs
    ) {
      return;
    }
    this.lastRuntimeNoticeSignature = normalized;
  ...
  private maybePushExtensionCompatibilityHint(
    event: Record<string, unknown>,
    errorMessage: string,
  ): void {
    const normalizedError = errorMessage.trim().toLowerCase();
    if (!normalizedError.includes('modelregistry.getapikey is not a function'))
      return;
    const extensionPath = pickString(event, [
      'extensionPath',
      'extension',
      'path',
    ]);
  ...
  private handleEvent(event: Record<string, unknown>): void {
    ...
    if (
      handleMessageStreamEvent(type, event, {
        promoteQueuedMessageFromUserEvent:
          this.promoteQueuedMessageFromUserEvent.bind(this),
        getLastMessage: () => this.messages[this.messages.length - 1] ?? null,
        ensureStreamingAssistantMessage:
          this.ensureStreamingAssistantMessage.bind(this),
        extractText: this.extractText.bind(this),
        extractAssistantMessageError:
          this.extractAssistantMessageError.bind(this),
        ...
        findMostRecentRunningToolByName:
          this.findMostRecentRunningToolByName.bind(this),
        ...
        extractAssistantPartialContent:
          this.extractAssistantPartialContent.bind(this),
        mergeStreamingText: this.mergeStreamingText.bind(this),
        scheduleStreamingUiReconcile:
          this.scheduleStreamingUiReconcile.bind(this),
        createId: uid,
      })
    ) {
      return;
    ...
    if (
      handleRuntimeStatusEvent(type, event, {
        ...
        maybePushExtensionCompatibilityHint:
          this.maybePushExtensionCompatibilityHint.bind(this),
        render: this.render.bind(this),
        scrollToBottom: this.scrollToBottom.bind(this),
        refreshFromBackend: this.refreshFromBackend.bind(this),
        loadAvailableModels: this.loadAvailableModels.bind(this),
  ...
  private attachOrphanToolResult(
    toolName: string,
    output: string,
    isError: boolean,
  ): void {
  ...
  private formatDroppedPathToken(path: string): string {
    ...
    const projectRoot = this.projectPath
      ? this.normalizeDroppedPath(this.projectPath).replace(/\/+$/, '')
      : '';
  ...
  private appendDroppedPathReferences(paths: string[]): void {
    const seen = new Set<string>(
      this.pendingFileReferences.map((entry) => entry.token.toLowerCase()),
    );
  ...
  private handleDroppedPathCandidates(
    ...
    const signatureNames = normalizedPaths.map((path) =>
      this.fileNameFromPath(path),
    );
    if (
      signatureNames.length > 0 &&
      this.shouldIgnoreDuplicateDrop(signatureNames)
    ) {
      return true;
    }
    const imagePaths = normalizedPaths.filter((path) =>
      this.isImageName(this.fileNameFromPath(path)),
    );
    const filePaths = normalizedPaths.filter(
      (path) => !this.isImageName(this.fileNameFromPath(path)),
    );
  ...
  private pathFromDroppedFile(file: File): string {
    ...
    const pathValue =
      typeof maybePath.path === 'string' && maybePath.path.trim().length > 0
        ? maybePath.path
        : typeof maybePath.webkitRelativePath === 'string' &&
            maybePath.webkitRelativePath.trim().length > 0
  ...
  private handleDroppedDataTransfer(dataTransfer: DataTransfer | null): void {
    ...
    const droppedPathsFromPayload = this.dedupeDroppedPaths([
      ...customPaths,
      ...customJsonPaths,
      ...uriPaths,
    ]);
    ...
    const imagePathsFromPayload = droppedPathsFromPayload.filter((path) =>
      this.isImageName(this.fileNameFromPath(path)),
    );
    const filePathsFromPayload = droppedPathsFromPayload.filter(
      (path) => !this.isImageName(this.fileNameFromPath(path)),
    );
    ...
    const fallbackSidebarPaths = !hasPayloadCandidates
      ? this.dedupeDroppedPaths(activeSidebarPaths)
      : [];
    const fallbackImagePaths = fallbackSidebarPaths.filter((path) =>
      this.isImageName(this.fileNameFromPath(path)),
    );
    const fallbackFilePaths = fallbackSidebarPaths.filter(
      (path) => !this.isImageName(this.fileNameFromPath(path)),
    );

    const imagePaths =
      imagePathsFromPayload.length > 0
        ? imagePathsFromPayload
        : fallbackImagePaths;
  ...
  private async prepareImages(files: FileList | File[]): Promise<void> {
    ...
    for (const file of list) {
      ...
      const pathHint =
        rawPathHint && rawPathHint !== safeName ? rawPathHint : undefined;
      let base64 = '';
    ...
    if (failed > 0) {
      this.pushNotice(
        `Attached ${next.length} image${next.length === 1 ? '' : 's'}; ${failed} failed`,
        'info',
      );
    }
  }
  ...
  private removePendingFileReference(id: string): void {
    this.pendingFileReferences = this.pendingFileReferences.filter(
      (entry) => entry.id !== id,
    );
    this.render();
  }
  ...
  private composedPromptText(rawText: string): string {
    ...
    const fileTokens = this.pendingFileReferences
  ...
  private currentIsStreaming(): boolean {
    return (
      Boolean(this.state?.isStreaming) ||
  ...
  private ensureStreamingAssistantMessage(seed?: {
    text?: string;
    errorText?: string;
  }): UiMessage {
  ...
  private promoteQueuedMessageFromUserEvent(
    ...
    if (eventText.length > 0) {
      matchIndex = this.queuedComposerMessages.findIndex(
        (entry) => normalize(entry.text) === eventText,
      );
    } else {
      matchIndex = this.queuedComposerMessages.findIndex(
        (entry) => normalize(entry.text).length === 0,
      );
    ...
    this.pushUserEcho(
      queued.text,
      'followUp',
      this.cloneImages(queued.attachments),
    );
    return true;
  }

  private enqueueComposerQueueMessage(
    text: string,
    attachments: PendingImage[],
  ): string {
    ...
    this.queuedComposerMessages = [...this.queuedComposerMessages, entry].slice(
      -6,
    );
    return entry.id;
  }
  ...
  private syncComposerQueueFromState(
    ...
    if (pendingCount > 0 && this.queuedComposerMessages.length > pendingCount) {
      this.queuedComposerMessages = this.queuedComposerMessages.slice(
        this.queuedComposerMessages.length - pendingCount,
      );
    }
  }
  ...
  private rememberComposerHistoryEntry(rawText: string): void {
    ...
    if (!text) return;
    const last =
      this.composerInputHistory[this.composerInputHistory.length - 1] ?? '';
    if (last !== text) {
      this.composerInputHistory.push(text);
      if (this.composerInputHistory.length > 120) {
        this.composerInputHistory.splice(
          0,
          this.composerInputHistory.length - 120,
        );
      }
    }
    this.resetComposerHistoryNavigation();
  ...
  private shouldHandleComposerHistoryKey(
    event: KeyboardEvent,
  ...
  async sendMessage(
    ...
    await sendMessageFlow({
      ...
      selectedSkillCommandText:
        this.selectedSkillDraft?.commandText?.trim() ?? '',
      pendingImages: [...this.pendingImages],
      slashQueryFromInput: () =>
        this.pendingFileReferences.length > 0
          ? null
          : this.slashQueryFromInput(),
      executeSlashCommandFromComposer:
        this.executeSlashCommandFromComposer.bind(this),
      rememberComposerHistoryEntry:
        this.rememberComposerHistoryEntry.bind(this),
  ...
  private clearStreamingUiState(): void {
    ...
    if (this.compactionCycle?.status === 'running') {
      ...
      this.compactionCycle.details.push(
  ...
  async compactNow(customInstructions?: string): Promise<boolean> {
    ...
    this.compactionCycle = {
      ...
      details: normalizedInstructions
        ? [`Custom instructions: ${truncate(normalizedInstructions, 180)}`]
    ...
    try {
      ...
      const summary =
        pickString(result as Record<string, unknown>, ['summary']) ||
        'Compaction complete';
      const tokensBefore = pickNumber(result as Record<string, unknown>, [
        'tokensBefore',
        'tokens_before',
      ]);
      const firstKeptEntry = pickString(result as Record<string, unknown>, [
        'firstKeptEntryId',
        'first_kept_entry_id',
      ]);
      if (this.compactionCycle) {
        ...
        if (typeof tokensBefore === 'number' && Number.isFinite(tokensBefore)) {
          this.compactionCycle.details.push(
            `Context before compaction: ${Math.round(tokensBefore).toLocaleString()} tokens`,
          );
        }
        if (firstKeptEntry) {
          this.compactionCycle.details.push(
            `First kept entry: ${truncate(firstKeptEntry, 48)}`,
          );
        }
      ...
      if (
        this.compactionCycle &&
        typeof this.sessionStats.tokens === 'number' &&
        Number.isFinite(this.sessionStats.tokens)
      ) {
        this.compactionCycle.details.push(
          `Context after compaction: ${Math.round(this.sessionStats.tokens).toLocaleString()} tokens`,
        );
      }
      ...
      if (this.compactionCycle) {
        ...
        this.compactionCycle.errorMessage =
          err instanceof Error
            ? truncate(err.message, 220)
  ...
  private async forkFrom(entryId: string): Promise<void> {
    const sourceSessionName =
      this.historyViewerSessionLabel.trim() ||
      this.state?.sessionName?.trim() ||
      '';
    ...
    try {
      ...
      await this.refreshFromBackend();
      this.pushNotice(
        result.cancelled ? 'Fork cancelled' : 'Fork ready in editor',
        'success',
      );
  ...
  openHistoryViewerForFork(options?: {
    loading?: boolean;
    ...
    this.historyViewerSessionLabel =
      options?.sessionName?.trim() || this.state?.sessionName?.trim() || '';
    this.historyQuery = options?.query?.trim() ?? '';
    this.historyRoleFilter = 'all';
    this.forkOptions = [];
    this.render();
  ...
  private async loadForkTargetsForHistory(): Promise<void> {
    ...
    try {
      ...
      this.forkEntryIdByMessageId = buildForkEntryIdByMessageId(
        this.messages,
        options,
      );
    } catch (err) {
  ...
  private revealMessage(messageId: string): void {
    const escaped = (window as any).CSS?.escape
      ? (window as any).CSS.escape(messageId)
      : messageId;
  ...
  private updateComposerOffset(): void {
    ...
    if (!chatRoot) return;
    const composer =
      this.container.querySelector<HTMLElement>('.composer-shell');
    ...
    const apply = () => {
      const measured = Math.max(
        140,
        Math.ceil(composer.getBoundingClientRect().height) + 18,
      );
  ...
  private currentWorkingPhrase(): string {
    const candidate =
      this.workingStatusPhrases[this.workingStatusPhraseIndex] ?? 'working';
  ...
  private currentWorkingLabel(): string {
    ...
    const count = Math.max(
      1,
      Math.min(this.workingStatusCharCount, phrase.length),
    );
  ...
  private stepWorkingStatusText(): void {
    ...
    if (this.workingStatusPhase === 'typing') {
      this.workingStatusCharCount = Math.min(
        phrase.length,
        this.workingStatusCharCount + 1,
      );
      ...
      this.workingStatusPhase = 'typing';
      this.workingStatusPhraseIndex =
        (this.workingStatusPhraseIndex + 1) % this.workingStatusPhrases.length;
      this.workingStatusCharCount = 0;
      nextDelay = 920;
      this.render();
    }
  ...
  private renderThinking(msg: UiMessage): TemplateResult | typeof nothing {
    ...
    const thinkingText = this.normalizeThinkingText(
      msg.thinking.replace(/^\s+/, ''),
    );
  ...
  private toggleToolWorkflowExpanded(
    workflowId: string,
    autoExpanded = false,
  ...
  private collectAssistantWorkflow(
    startIndex: number,
  ): AssistantWorkflowCandidate | null {
  ...
  private renderAssistantWorkflow(workflow: AssistantWorkflow): TemplateResult {
    return renderAssistantWorkflowView({
      ...
      formatDuration,
      isWorkflowThinkingExpanded: (thinkingId) =>
        this.isWorkflowThinkingExpanded(thinkingId),
      toggleWorkflowThinkingExpanded: (thinkingId) =>
        this.toggleWorkflowThinkingExpanded(thinkingId),
      isToolGroupExpanded: (workflowId, groupId) =>
        this.isToolGroupExpanded(workflowId, groupId),
      toggleToolGroupExpanded: (workflowId, groupId) =>
        this.toggleToolGroupExpanded(workflowId, groupId),
      toggleToolWorkflowExpanded: (
        workflowId,
        autoExpanded,
        currentlyExpanded,
      ) =>
        this.toggleToolWorkflowExpanded(
          workflowId,
          autoExpanded,
          currentlyExpanded,
        ),
  ...
  private renderMessageTimeline(): TemplateResult[] {
    return renderMessageTimelineRows({
      ...
      renderAssistantWorkflow: (workflow) =>
        this.renderAssistantWorkflow(workflow),
      renderUserMessage: (message) => this.renderUserMessage(message),
      hasRenderableAssistantContent: (message) =>
        this.hasRenderableAssistantContent(message),
  ...
  private renderAssistantMessage(msg: UiMessage): TemplateResult {
    return renderAssistantMessageRow({
      ...
      isStandaloneCodeBlockMarkdown: (value) =>
        this.isStandaloneCodeBlockMarkdown(value),
  ...
  private async refreshWelcomeDashboard(force = false): Promise<void> {
    ...
    try {
      const inventory = await loadWelcomeDashboardInventory();
      this.welcomeDashboard = {
        loading: false,
  ...
  private renderCenteredWelcome(): TemplateResult {
    ...
    const activeProject =
      this.welcomeProjects.find(
        (project) => project.id === this.welcomeActiveProjectId,
      ) ??
      this.welcomeProjects.find(
        (project) =>
          normalizeComparablePath(project.path) === comparableProjectPath,
      ) ??
      null;
    ...
    const projectLabel =
      activeProject?.name ??
      (this.projectPath
        ? this.fileNameFromPath(this.projectPath)
        : 'Add project');
    const welcomeHeadline =
      this.welcomeHeadlines[this.welcomeHeadlineIndex] ??
      this.welcomeHeadlines[0];
    ...
    return renderCenteredWelcomeView({
      ...
      onSelectProject: (projectId) => {
        ...
        if (projectId !== activeProject?.id)
          this.onSelectWelcomeProject?.(projectId);
      },
      onAddProject: () => {
        this.welcomeProjectMenuOpen = false;
        this.onAddProject?.();
  ...
  private renderComposerControls(
    ...
    const { provider: currentProvider, modelId: currentModelId } =
      this.currentModelSelection();
    const currentModelValue =
      currentProvider && currentModelId
        ? `${currentProvider}::${currentModelId}`
        : '';
    const currentModelDisplay = currentModelId
      ? formatModelDisplayName(currentModelId)
      : 'Select model';
    const currentProviderDisplay = currentProvider
      ? this.displayProviderLabel(currentProvider)
      : '';
    const currentModelTitle =
      currentProvider && currentModelId
        ? `${currentProviderDisplay} / ${currentModelId}`
        : 'Select model';
    ...
    const activeProviderGroup =
      providerGroups.find(
        (group) => group.providerKey === resolvedActiveProvider,
      ) ?? null;
    ...
    return renderComposerControlsView({
      ...
      runningProviderAuthActionProvider:
        this.runningProviderAuthAction?.provider ?? null,
      ...
      onToggleModelPicker: (preferredProvider) =>
        this.toggleModelPicker(preferredProvider),
      onSetModelPickerActiveProvider: (provider) =>
        this.setModelPickerActiveProvider(provider),
      onProviderAuthAction: (provider, action) =>
        this.handleProviderAuthAction(provider, action),
  ...
  private ensureActiveSlashItemVisible(): void {
    if (
      !this.slashPaletteOpen ||
      this.slashPaletteNavigationMode !== 'keyboard'
    )
      return;
    requestAnimationFrame(() => {
      const menu = this.container.querySelector<HTMLElement>(
        '.composer-slash-menu',
      );
      const activeItem = this.container.querySelector<HTMLElement>(
  ...
  private handleSlashPaletteMouseMove(event: MouseEvent): void {
    ...
    const target =
      event.target instanceof Element
        ? (event.target.closest('.composer-slash-item') as HTMLElement | null)
        : null;
    if (!target) return;
  ...
  private handleComposerDragOver(
    event: DragEvent,
    interactionLocked: boolean,
  ): void {
  ...
  private handleComposerDrop(
    ...
    handleComposerDropEvent({
      event,
      interactionLocked,
      onHandleDroppedDataTransfer: (dataTransfer) =>
        this.handleDroppedDataTransfer(dataTransfer),
    });
  }

  private handleComposerKeyDown(
    event: KeyboardEvent,
    interactionLocked: boolean,
    isStreaming: boolean,
  ): void {
    handleComposerKeyDownEvent({
      ...
      shouldHandleComposerHistoryKey: (ev, textarea, direction) =>
        this.shouldHandleComposerHistoryKey(ev, textarea, direction),
      onNavigateComposerHistory: (direction) =>
        this.navigateComposerHistory(direction),
      ...
      onExecuteSlashCommandFromComposer: () =>
        this.executeSlashCommandFromComposer(),
      onSendMessage: (mode) => this.sendMessage(mode),
    });
  }

  private handleComposerFilePickerChange(
    event: Event,
    interactionLocked: boolean,
  ): void {
    handleComposerFilePickerChangeEvent({
  ...
  private renderComposer(): TemplateResult {
    ...
    const canSendBase =
      ...
      (this.inputText.trim().length > 0 ||
        this.pendingImages.length > 0 ||
        this.pendingFileReferences.length > 0 ||
        Boolean(this.selectedSkillDraft));
    ...
    const connectivityStatus =
      ...
      (!this.isConnected && this.projectPath ? 'RPC disconnected' : '');
  ...
  private async loadSessionTreeForHistory(): Promise<void> {
    ...
    try {
      ...
      this.historyTreeRows = parseSessionTreeRows({
        ...
        currentSessionEntryIds: this.messages
          .map((message) => message.sessionEntryId ?? '')
  ...
  private renderHistoryViewer(): TemplateResult | typeof nothing {
    return renderHistoryViewerView<UiMessage>({
      ...
      resolveForkEntryId: (messages, index) =>
        resolveForkEntryId(messages, index, this.forkEntryIdByMessageId),
  ...
  private doRender(): void {
    ...
    const showWorkingIndicator =
      hasProject && this.shouldShowWorkingIndicator();
    if (
      !hasProject &&
      !this.welcomeDashboard.loading &&
      this.welcomeDashboard.updatedAt === 0
    ) {
    ...
    const template = html`
          ...
          if (
            this.slashPaletteOpen &&
            !target.closest('.composer-slash-menu') &&
          ...
          if (
            ...
            !target.closest('.welcome-project-wrap')
          ) {
            this.welcomeProjectMenuOpen = false;
            changed = true;
          }
