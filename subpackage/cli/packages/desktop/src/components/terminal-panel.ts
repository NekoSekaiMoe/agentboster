...
function compactPath(path: string | null): string {
  ...
  const home = (typeof globalHome === 'string' ? globalHome : '')
    .replace(/\\/g, '/')
    .replace(/\/+$/, '');
...
export class TerminalPanel {
  ...
  private onCommandComplete:
    | ((event: TerminalCommandCompleteEvent) => void | Promise<void>)
    | null = null;
  ...
  setOnCommandComplete(
    cb: (event: TerminalCommandCompleteEvent) => void | Promise<void>,
  ): void {
  ...
  private applyTheme(): void {
    ...
    const panelRoot = this.container.querySelector<HTMLElement>(
      '.terminal-panel-root',
    );
    const panelStyles = panelRoot ? getComputedStyle(panelRoot) : styles;
    const background =
      panelStyles.getPropertyValue('--terminal-panel-bg').trim() ||
  ...
  private ensureTerminal(): void {
    const viewport =
      this.container.querySelector<HTMLElement>('#terminal-viewport');
    if (!viewport) return;
  ...
  private buildPiInteractiveBridgeCommand(dimensions?: {
    cols: number;
    rows: number;
  }): string | null {
    const platform = navigator.platform.toLowerCase();
  ...
  private async ensureShellProfile(): Promise<ShellProfile> {
    ...
    this.resolvingShellProfile = (async () => {
      ...
      for (const profile of profiles) {
        try {
          const probe = await Command.create(
            profile.name,
            this.buildShellProbeArgs(profile),
            {
  ...
  private isSpawnScopeError(error: unknown): boolean {
    ...
    return (
      message.includes('allow-spawn') ||
      message.includes('configured shell scope') ||
      message.includes('program not allowed')
    );
  }
  ...
  private async executeShellViaExecute(
    ...
    const output = await Command.create(
      profile.name,
      this.buildShellArgs(profile, command),
      {
        cwd: this.cwd || undefined,
      },
    ).execute();
  ...
  private async executeShell(
    ...
    const initialInput = Array.isArray(options.initialInput)
      ? options.initialInput
      : [];
    const outputRaw = options.rawOutput === true;
    const allowExecuteFallback = options.allowExecuteFallback ?? !outputRaw;
    const initialInputStartDelayMs = Math.max(
      0,
      Number(options.initialInputStartDelayMs ?? 0),
    );
    const initialInputInterChunkDelayMs = Math.max(
      0,
      Number(options.initialInputInterChunkDelayMs ?? 140),
    );
    ...
    return await new Promise<TerminalExecResult>((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        if (
          this.runningChild &&
          spawnedChild &&
          this.runningChild.pid === spawnedChild.pid
        ) {
      ...
      const settleWithError = (error: unknown) => {
        ...
        cleanup();
        reject(
          error instanceof Error ? error : new Error(normalizeText(error)),
        );
      };
      ...
      const attemptExecuteFallback = () => {
        ...
        if (!allowExecuteFallback) {
          settleWithError(
            new Error(
              'Spawn unavailable for interactive terminal command. Restart app and verify shell capabilities.',
            ),
          );
          return;
        }
        this.writeInfo('Spawn unavailable; falling back to execute mode.');
        void this.executeShellViaExecute(profile, command, streamOutput)
      ...
      const sendInitialInput = async (child: Child): Promise<void> => {
        ...
        if (initialInputStartDelayMs > 0) {
          await new Promise((resolve) =>
            setTimeout(resolve, initialInputStartDelayMs),
          );
        }
        for (let i = 0; i < initialInput.length; i += 1) {
          ...
          if (
            i < initialInput.length - 1 &&
            initialInputInterChunkDelayMs > 0
          ) {
            await new Promise((resolve) =>
              setTimeout(resolve, initialInputInterChunkDelayMs),
            );
          }
        }
      };
      ...
      const onStdout = (payload: unknown) => {
        const text = outputRaw
          ? this.decodeOutputChunk(payload, stdoutDecoder)
      ...
      const onStderr = (payload: unknown) => {
        const text = outputRaw
          ? this.decodeOutputChunk(payload, stderrDecoder)
          : normalizeText(payload);
        if (!text) return;
        stderr += text;
        if (streamOutput) {
  ...
  render(): void {
    const shellLabel =
      this.shellProfile?.label ??
      shellProfilesForPlatform()[0]?.label ??
      'shell';
    const headerTitle = this.running
      ? `Terminal ${shellLabel} · running`
      : `Terminal ${shellLabel}`;
