...
function roleFromSessionEntry(roleRaw: string): HistoryViewerRole {
  ...
  if (normalized === 'custom' || normalized === 'custom_message')
    return 'custom';
  return 'system';
}
...
function mapSessionTreeEntry(
  ...
  switch (type) {
    case 'message': {
      ...
      if (messageRole === 'user') {
        ...
        entryLabel = 'user';
        preview =
          extractText(messageRecord?.content).replace(/\s+/g, ' ').trim() ||
          '(empty message)';
      ...
      if (messageRole === 'assistant') {
        ...
        const contentPreview = extractText(messageRecord?.content)
          .replace(/\s+/g, ' ')
          .trim();
        const stopReason =
          pickString(messageRecord ?? {}, ['stopReason', 'stop_reason']) ?? '';
        const errorMessage =
          pickString(messageRecord ?? {}, ['errorMessage', 'error_message']) ??
          '';
        preview =
          contentPreview ||
          (stopReason === 'aborted'
      ...
      if (messageRole === 'toolresult') {
        ...
        entryLabel = 'tool';
        const toolName =
          pickString(messageRecord ?? {}, ['toolName', 'tool_name']) ?? 'tool';
        const toolOutputRaw = extractToolOutput(
    ...
    case 'custom_message': {
      role = 'custom';
      const customType =
        pickString(record, ['customType', 'custom_type']) ?? 'custom';
      entryLabel = customType;
      preview =
        extractText(record.content).replace(/\s+/g, ' ').trim() || '(empty)';
    ...
    case 'branch_summary': {
      ...
      preview =
        (pickString(record, ['summary']) ?? extractText(record.content))
          .replace(/\s+/g, ' ')
          .trim() || '(empty)';
    ...
    case 'compaction': {
      ...
      const tokensBefore = pickNumber(record, [
        'tokensBefore',
        'tokens_before',
      ]);
      preview = (pickString(record, ['summary']) ?? 'compaction entry')
        .replace(/\s+/g, ' ')
        .trim();
      const tokensBadge =
    ...
    case 'thinking_level_change': {
      ...
      entryLabel = 'thinking';
      const level =
        pickString(record, ['thinkingLevel', 'thinking_level']) ?? 'updated';
      preview = level;
    ...
    case 'custom': {
      ...
      entryLabel = customType;
      preview =
        extractText(record.data).replace(/\s+/g, ' ').trim() || 'custom entry';
    ...
    default: {
      role = 'system';
      preview =
        extractText(record.content).replace(/\s+/g, ' ').trim() || entryLabel;
  ...
  return {
    ...
    displayText:
      ...
      `${entryLabel}: ${normalizedPreview || '(empty)'}`,
    canFork,
  };
}
...
export function compactTreeLinePrefix(prefix: string, depth: number): string {
  ...
  const tail = normalized.slice(
    Math.max(0, normalized.length - maxVisibleDepth * charsPerLevel),
  );
  return `… ${tail}`;
}
...
export function parseSessionTreeRows({
  ...
  for (const entry of entries) {
    const parentId =
      entry.parentId && entriesById.has(entry.parentId) ? entry.parentId : null;
    if (!parentId) {
      roots.push(entry);
      continue;
    }
  ...
  const byIndex = (
    a: SessionTreeEntryRecord,
    b: SessionTreeEntryRecord,
  ...
  const currentLeafId = resolveCurrentTreeLeafId(
    entriesById,
    entries,
    currentSessionEntryIds,
  );
  ...
  const buildPrefix = (
    ...
    const parts: string[] = ancestorHasNext.map((hasNext) =>
      hasNext ? '│  ' : '   ',
    );
    if (depth > 0) {
      parts.push(isLast ? '└─ ' : '├─ ');
