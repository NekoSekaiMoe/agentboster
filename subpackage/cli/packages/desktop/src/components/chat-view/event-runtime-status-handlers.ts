...
export function handleRuntimeStatusEvent(
  ...
  switch (type) {
    ...
    case 'agent_end': {
      ...
      if (runError && !(last?.role === 'assistant' && last.errorText)) {
        context.pushRuntimeNotice(
          `Run failed: ${context.truncate(runError, 180)}`,
          'error',
          2600,
        );
      }
    ...
    case 'error': {
      const errorMessage =
        context.extractRuntimeErrorMessage(event) || 'Unknown runtime error';
      const source = pickString(event, [
        'source',
        'phase',
        'stage',
        'provider',
      ...
      if (source === 'stderr' || source === 'stdout_text') {
        ...
        context.pushRuntimeNotice(
          `${prefix}: ${context.truncate(errorMessage, 180)}`,
          'error',
          2600,
        );
      }
    ...
    case 'extension_error': {
      const error =
        context.extractRuntimeErrorMessage(event) || 'Unknown extension error';
      ...
      const prefix = source
        ? `Extension error (${extensionLabel}:${source})`
        : `Extension error (${extensionLabel})`;
      context.pushRuntimeNotice(
        `${prefix}: ${context.truncate(error, 180)}`,
        'error',
        2600,
      );
    ...
    case 'rpc_connected': {
      ...
      context.setBindingStatusText(
        context.projectPath ? 'Loading session…' : null,
      );
    ...
    case 'rpc_disconnected': {
      ...
      context.cancelStreamingUiReconcile();
      context.setBindingStatusText(
        context.projectPath ? 'Reconnecting session…' : null,
      );
