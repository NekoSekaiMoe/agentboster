...
export function deriveLatestAssistantContextTokens(
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    const usageInput = pickNumber(message, [
      'usage.input',
      'usage.inputTokens',
      'usage.input_tokens',
    ]);
    const usageOutput = pickNumber(message, [
      'usage.output',
      'usage.outputTokens',
      'usage.output_tokens',
    ]);
    const usageCacheRead = pickNumber(message, [
      'usage.cacheRead',
      'usage.cache_read',
    ]);
    const usageCacheWrite = pickNumber(message, [
      'usage.cacheWrite',
      'usage.cache_write',
    ]);
    const components = [
