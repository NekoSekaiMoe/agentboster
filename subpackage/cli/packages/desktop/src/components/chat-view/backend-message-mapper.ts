...
export function mapBackendMessages({
  ...
  for (const raw of backendMessages) {
    ...
    switch (role) {
      ...
      case 'assistant': {
        ...
        for (const part of content) {
          ...
          if (type === 'toolCall') {
            ...
            if (existing) {
              existing.name =
                typeof p.name === 'string' ? p.name : existing.name;
              existing.args =
                (p.arguments as Record<string, unknown>) ?? existing.args;
            ...
            const toolCall: ToolCallBlockLike = {
              ...
              isExpanded: false,
              startedAt:
                pickNumber(p, ['startedAt', 'startTime', 'timestamp', 'ts']) ??
                undefined,
        ...
        if (toolCallId && toolCallMap.has(toolCallId)) {
          ...
          if (target) {
            target.toolCalls.push({
              ...
              name: (typeof raw.toolName === 'string' &&
      ...
      case 'custom': {
        const customType =
          typeof raw.customType === 'string' ? raw.customType : 'custom';
