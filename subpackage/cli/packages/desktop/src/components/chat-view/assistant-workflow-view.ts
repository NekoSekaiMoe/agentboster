...
export function renderAssistantWorkflowView({
  ...
  const { total, running, autoExpanded, expanded } =
    resolveWorkflowExpansionState(
      workflow.id,
      workflow.toolCalls,
      workflow.isTerminal,
    );
  const failed = workflow.toolCalls.filter(
  ...
  const durationMs =
    workflow.startedAt > 0
      ? (running > 0
          ? Date.now()
          : Math.max(workflow.endedAt, workflow.startedAt)) - workflow.startedAt
      : 0;
  ...
  if (summaryParts.length === 0 && total > 0)
    summaryParts.push(`${total} complete`);
  ...
  for (const message of workflow.messages) {
    const normalizedThinking = normalizeThinkingText(
      (message.thinking ?? '').replace(/^\s+/, ''),
    );
    if (normalizedThinking) {
      let displayThinking = normalizedThinking;
      if (lastThinkingFull) {
        if (normalizedThinking.startsWith(lastThinkingFull)) {
          displayThinking = normalizedThinking
            .slice(lastThinkingFull.length)
            .replace(/^\s+/, '')
            .trim();
      ...
      if (!displayThinking) {
        if (previous && previous.kind === 'thinking') {
          previous.animating =
            previous.animating || Boolean(message.isThinkingStreaming);
        }
      } else if (previous && previous.kind === 'thinking') {
        previous.animating =
          previous.animating || Boolean(message.isThinkingStreaming);
        if (
          displayThinking === previous.text ||
          previous.text.startsWith(displayThinking)
        ) {
    ...
    for (const toolCall of message.toolCalls) {
      ...
      if (
        previous &&
        previous.kind === 'group' &&
        previous.group.toolName === toolCall.name &&
        previous.group.preview === preview
      ) {
            ...
            expanded
              ? html`
                  ...
                  if (entry.kind === 'thinking') {
                    const thinkingExpanded = isWorkflowThinkingExpanded(
                      entry.id,
                    );
                  ...
                  const groupRunning = group.calls.some(
                    (toolCall) => toolCall.isRunning,
                  );
                  const groupFailed = group.calls.some(
                    (toolCall) => toolCall.isError,
                  );
                  const groupExpanded = isToolGroupExpanded(
                    workflow.id,
                    group.id,
                  );
                  const output =
                    [...group.calls]
                      .reverse()
                      .map((call) =>
                        (call.streamingOutput ?? call.result ?? '').trim(),
                      )
                      .find((value) => value.length > 0) ?? '';
                  const statusLabel = groupRunning
                    ? 'running'
                    : groupFailed
                      ? 'failed'
                      : 'success';
