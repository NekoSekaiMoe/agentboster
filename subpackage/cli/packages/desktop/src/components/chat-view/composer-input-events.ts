...
export function handleComposerDragOverEvent({
  event,
  interactionLocked,
}: HandleComposerDragOverEventParams): void {
  event.preventDefault();
...
export function handleComposerKeyDownEvent({
  ...
  const canHistoryUp =
    event.key === 'ArrowUp' &&
    shouldHandleComposerHistoryKey(event, textarea, 'up');
  const canHistoryDown =
    event.key === 'ArrowDown' &&
    shouldHandleComposerHistoryKey(event, textarea, 'down');
  ...
  if (slashPaletteOpen && liveSlashItems.length > 0) {
    ...
    if (event.key === 'ArrowUp') {
      ...
      onSetSlashPaletteNavigationMode('keyboard');
      const nextIndex =
        (getSlashPaletteIndex() - 1 + liveSlashItems.length) %
        liveSlashItems.length;
      onSetSlashPaletteIndex(nextIndex);
      const item = liveSlashItems[nextIndex];
      if (item) onPreviewSlashPaletteItem(item);
      onRender();
