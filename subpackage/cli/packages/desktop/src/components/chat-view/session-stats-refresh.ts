...
export function computeSessionStatsFromRaw({
  ...
  const contextUsageRecord =
    raw.contextUsage && typeof raw.contextUsage === 'object'
      ? (raw.contextUsage as Record<string, unknown>)
      : null;
  const contextUsageHasTokensKey = Boolean(
    contextUsageRecord && Object.hasOwn(contextUsageRecord, 'tokens'),
  );
  const contextUsageHasPercentKey = Boolean(
    contextUsageRecord && Object.hasOwn(contextUsageRecord, 'percent'),
  );
  const contextUsageTokensExplicitNull =
    contextUsageHasTokensKey && contextUsageRecord?.tokens === null;
  const contextUsagePercentExplicitNull =
    contextUsageHasPercentKey && contextUsageRecord?.percent === null;
  ...
  const contextTokens =
    ...
    (contextUsageExplicitlyUnknown ? null : lastAssistantContextTokens);
  const usageRatio =
    rawUsageRatio ??
  ...
  const normalizedContextTokens =
    contextTokens ??
    (usageRatio !== null && contextWindow && contextWindow > 0
      ? usageRatio * contextWindow
      : null);
