...
interface HandleMessageStreamEventContext {
  ...
  ensureStreamingAssistantMessage: (
    options?: EnsureStreamingAssistantMessageOptions,
  ) => MessageLike;
  ...
  switch (type) {
    case 'message_start': {
      ...
      if (role === 'toolResult') {
        ...
        const output = context.extractToolOutput(
          message.content ?? message.result ?? message,
        );
    ...
    case 'message_update': {
      ...
      if (subtype === 'error') {
        const streamError =
          context.extractRuntimeErrorMessage(assistantEvent) ||
          context.extractRuntimeErrorMessage(event);
        const assistant = context.ensureStreamingAssistantMessage(
          streamError ? { errorText: streamError } : undefined,
        );
      ...
      if (subtype === 'text_delta') {
        const assistant = context.ensureStreamingAssistantMessage();
        const partialText = context.extractAssistantPartialContent(
          assistantEvent,
          'text',
        );
        assistant.text = context.mergeStreamingText(
          assistant.text,
          partialText,
      ...
      if (
        ...
        subtype.includes('thinking') ||
        subtype.includes('reason')
      ) {
        const assistant = context.ensureStreamingAssistantMessage();
        const partialThinking = context.extractAssistantPartialContent(
          assistantEvent,
          'thinking',
        );
        const currentThinking = assistant.thinking || '';
        assistant.thinking = context.mergeStreamingText(
          currentThinking,
          partialThinking,
          assistantEvent.delta,
        );
          ...
          if (existing) {
            existing.name =
              ...
              toolCall.name.trim().length > 0
                ...
                : existing.name;
            existing.args =
              ((toolCall.arguments ?? existing.args) as Record<
                string,
                unknown
              >) || existing.args;
    ...
    case 'turn_end': {
      ...
      const turnRole =
        typeof turnMessage?.role === 'string' ? turnMessage.role : '';
    ...
    case 'tool_execution_update': {
      ...
      if (partialText) {
        ...
        tool.streamingOutput = context.mergeStreamingText(
          currentOutput,
          partialText,
          partialResult.delta,
        );
  ...
  switch (type) {
    ...
    case 'auto_compaction_progress': {
      ...
      const detail =
        ...
        context
          .extractToolOutput(event.detail ?? event.payload ?? event)
          .trim();
      if (detail) {
        const cleaned = context.truncate(
          detail.replace(/\s+/g, ' ').trim(),
          220,
        );
        if (
          cleaned &&
          compactionCycle.details[compactionCycle.details.length - 1] !==
            cleaned
        ) {
    ...
    case 'auto_compaction_end': {
      ...
      if (aborted) {
        ...
        compactionCycle.details.push(
          'Compaction was aborted before completion.',
        );
        ...
        compactionCycle.details.push(
          `Failure: ${context.truncate(errorMessage, 220)}`,
        );
        context.pushRuntimeNotice(
          `Auto-compaction failed: ${context.truncate(errorMessage, 180)}`,
          'error',
          2600,
        );
      } else {
        ...
        const tokensBefore = pickNumber(event, [
          'result.tokensBefore',
          'tokensBefore',
          'tokens_before',
        ]);
    ...
    case 'auto_retry_start': {
      ...
      const maxAttempts =
        typeof event.maxAttempts === 'number' ? event.maxAttempts : 1;
    ...
    case 'auto_retry_end': {
      ...
      if (!success) {
        const finalError =
          context.extractRuntimeErrorMessage(event) || 'Unknown retry failure';
        context.pushRuntimeNotice(
          `Retry failed: ${context.truncate(finalError, 180)}`,
          'error',
          2600,
        );
      } else {
        context.appendSystemMessage(
          attempt ? `Retry succeeded on attempt ${attempt}` : 'Retry succeeded',
          { idPrefix: 'runtime' },
        );
      }
      context.render();
