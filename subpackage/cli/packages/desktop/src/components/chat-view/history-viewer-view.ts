...
export function renderHistoryViewerView<Message extends HistoryViewerMessage>({
  ...
  const filteredTreeRows: HistoryTreeRow[] = forkMode
    ...
    : historyTreeRows.filter((row) => {
        if (historyRoleFilter !== 'all' && row.role !== historyRoleFilter)
          return false;
        if (!query) return true;
        const haystack =
          `${row.role} ${row.entryLabel} ${row.preview} ${row.displayText} ${row.entryId}`.toLowerCase();
        return haystack.includes(query);
      });

  const filteredBrowseRows: Array<{ msg: Message; sourceIndex: number }> =
    forkMode || useTreeRows
      ? []
      : sourceMessages
          .map((msg, sourceIndex) => ({ msg, sourceIndex }))
          .filter(({ msg }) => {
            if (historyRoleFilter !== 'all' && msg.role !== historyRoleFilter)
              return false;
            if (!query) return true;
            const haystack =
              `${msg.role} ${msg.label || ''} ${messagePreview(msg)}`.toLowerCase();
            return haystack.includes(query);
          });
                  ...
                  : useTreeRows
                    ? filteredTreeRows.map((row, idx) => {
                        const visibleMessageId =
                          sessionMessageIdByEntryId.get(row.entryId) ?? null;
                        const canJump = Boolean(visibleMessageId);
                        const title = canJump
                          ? 'Jump to this entry'
                          : 'Entry is outside the active branch';
                        const compactPrefix = compactTreeLinePrefix(
                          row.linePrefix,
                          row.depth,
                        );
                        const rowText =
                          row.displayText.trim() || row.preview || '(entry)';
                    ...
                    : filteredBrowseRows.map(({ msg, sourceIndex }, idx) => {
                        const forkEntryId = resolveForkEntryId(
                          sourceMessages,
                          sourceIndex,
                        );
                        const canFork =
