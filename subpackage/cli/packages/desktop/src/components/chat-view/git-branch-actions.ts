...
export async function switchGitBranchAction({
  ...
  try {
    ...
    if (!hasCommit) {
      ...
      if (!switched.ok) {
        pushNotice(
          switched.error || `Failed to switch branch: ${branch}`,
          'error',
        );
        return;
      }
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (fallback.exitCode === 0) {
        closeGitMenu();
        pushNotice(
          `Switched to ${localBranch} (tracking ${remoteRef})`,
          'success',
        );
        await refreshGitSummary(true);
        return;
      ...
      pushNotice(
        result.stderr.trim() ||
          result.stdout.trim() ||
          `Failed to switch branch: ${remoteRef}`,
        'error',
      );
      return;
    }
...
export async function createAndCheckoutBranchAction({
  ...
  if (refCheck.exitCode !== 0) {
    pushNotice(
      refCheck.stderr.trim() || refCheck.stdout.trim() || 'Invalid branch name',
      'error',
    );
    return;
  }
