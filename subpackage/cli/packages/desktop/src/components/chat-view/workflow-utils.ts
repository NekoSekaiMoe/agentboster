...
export function summarizeToolCall(
  ...
  const command = pickToolArg(toolCall.args, [
    'command',
    'cmd',
    'shell',
    'script',
  ]);
  const path = pickToolArg(toolCall.args, [
    'path',
    'filePath',
    'targetPath',
    'from',
  ...
  const query = pickToolArg(toolCall.args, [
    'query',
    'pattern',
    'glob',
    'name',
  ...
  if ((name === 'read' || name === 'readfile') && path)
    return `Read ${truncateText(path, 74)}`;
  if ((name === 'write' || name === 'writefile') && path)
    return `Wrote ${truncateText(path, 74)}`;
...
function buildToolCallGroups(
  ...
  for (const toolCall of toolCalls) {
    ...
    if (
      previous &&
      previous.toolName === toolCall.name &&
...
function isThinkingOnlyAssistantMessage(
  message: WorkflowMessage | undefined,
): boolean {
...
export function collectAssistantWorkflow({
  ...
  const thinkingParts = grouped
    .map((entry) =>
      normalizeThinkingText((entry.thinking ?? '').replace(/^\s+/, '')),
    )
    .filter(Boolean);
  const dedupedThinkingParts = thinkingParts.filter(
    (part, index) => index === 0 || part !== thinkingParts[index - 1],
  );
...
export function resolveWorkflowExpansionState({
  ...
  const autoExpanded =
    isTerminal &&
    keepWorkflowExpandedUntilAssistantText &&
    (running > 0 || runSawToolActivity || total === 0);
  const expanded =
    (autoExpanded && !collapsedAutoWorkflowIds.has(workflowId)) ||
    manualExpanded;
  return {
    total,
    running,
