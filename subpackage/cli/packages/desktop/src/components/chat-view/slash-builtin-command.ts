...
function getSessionMessageBreakdown(messages: SessionInfoMessageLike[]): {
  ...
  for (const message of messages) {
    ...
    toolResults += message.toolCalls.filter(
      (call) =>
...
export function formatSessionInfoBlock(params: {
  ...
  const { user, assistant, toolCalls, toolResults } =
    getSessionMessageBreakdown(messages);
  const totalMessages = state?.messageCount ?? sessionStats.messageCount;
  const pendingMessages =
    state?.pendingMessageCount ?? sessionStats.pendingCount;
  ...
  lines.push('Tokens');
  lines.push(
    `Context: ${sessionStats.tokens !== null ? Math.round(sessionStats.tokens).toLocaleString() : '—'}`,
  );
  lines.push(
    `Context window: ${sessionStats.contextWindow !== null ? Math.round(sessionStats.contextWindow).toLocaleString() : '—'}`,
  );
  lines.push(
    `Usage: ${sessionStats.usageRatio !== null ? `${(sessionStats.usageRatio * 100).toFixed(1)}%` : '—'}`,
  );
  lines.push(
    `Session tokens total: ${sessionStats.lifetimeTokens !== null ? Math.round(sessionStats.lifetimeTokens).toLocaleString() : '—'}`,
  );
  lines.push(
    `Cost: ${sessionStats.costUsd !== null ? formatUsd(sessionStats.costUsd) : '—'}`,
  );
  return lines.join('\n');
}
...
export async function executeBuiltinSlashCommand({
  ...
  switch (commandName) {
    ...
    case 'model': {
      ...
      if (!candidate) {
        const providerHint =
          resolveProviderHintFromModelArg(rawArg) ?? undefined;
    ...
    case 'changelog': {
      ...
      const markdown = showAll
        ? markdownFull
        : extractLatestChangelogSections(markdownFull, 2);
