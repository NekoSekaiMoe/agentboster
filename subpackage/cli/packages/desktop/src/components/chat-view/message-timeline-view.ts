...
export function renderAssistantMessageRow<Message extends TimelineMessage>({
  ...
  const canCopy = Boolean(
    errorLine.length > 0 || (trimmedText.length > 0 && !standaloneCodeBlock),
  );
  const formattedErrorLine = errorLine
    ? /^error\b[:\s-]*/i.test(errorLine)
      ? errorLine
      : `Error: ${errorLine}`
    : '';
...
export function renderSystemMessageRow<Message extends TimelineMessage>({
  message,
}: RenderSystemMessageRowParams<Message>): TemplateResult {
  const isInline =
    message.label === 'share' ||
    message.label === 'auth' ||
    message.label === 'models';
  return html`
...
export function renderCompactionCycleRow({
  ...
  const elapsedSeconds = Math.max(
    1,
    Math.round((completed - cycle.startedAt) / 1000),
  );
...
export function renderMessageTimelineRows<Message extends TimelineMessage>({
  ...
  const compactionInsertAt = compactionCycle
    ? Math.max(
        0,
        Math.min(compactionInsertIndex ?? messages.length, messages.length),
      )
    : null;
