...
export function renderGitRepoControlView({
  ...
  const branchEntries = summary.branchEntries.filter((entry) => {
    ...
    const haystack =
      `${entry.name} ${entry.fullName} ${entry.remote ?? ''} ${entry.scope}`.toLowerCase();
    return haystack.includes(query);
  });
  const filesLabel = summary.dirtyFiles === 1 ? 'file' : 'files';
  const matchingEntry =
    gitBranchQuery.trim().length > 0
      ? resolveGitBranchSelection(gitBranchQuery)
      : null;
          ...
          ? html`
                ...
                branchEntries.length === 0
                  ...
                  : branchEntries.map((entry) => {
                      const active =
                        entry.scope === 'local' && entry.name === currentBranch;
                      const disabled =
                        active || switchingGitBranch || fetchingGitRemotes;
                      const label =
                        entry.scope === 'remote' ? entry.fullName : entry.name;
                      return html`
											<button
