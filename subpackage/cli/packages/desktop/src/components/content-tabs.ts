...
export class ContentTabs {
  ...
  setTabs(tabs: MainContentTab[], activeId: string | null): void {
    ...
    if (
      this.contextTabKey &&
      !this.tabs.some((tab) => this.tabKey(tab) === this.contextTabKey)
    ) {
    ...
    if (
      this.draggingTabKey &&
      !this.tabs.some((tab) => this.tabKey(tab) === this.draggingTabKey)
    ) {
      this.clearDragState();
    }
  ...
  private isTabPinned(tab: MainContentTab): boolean {
    ...
    if (Object.hasOwn(this.tabPins, key)) {
  ...
  private applyStoredLayoutToTabs(tabs: MainContentTab[]): MainContentTab[] {
    const withPins = tabs.map((tab) => {
      const key = this.tabKey(tab);
      const storedPinned = Object.hasOwn(this.tabPins, key)
  ...
  private persistTabColors(): void {
    try {
      localStorage.setItem(
        TAB_COLORS_STORAGE_KEY,
        JSON.stringify(this.tabColors),
      );
    } catch {
      // ignore
    }
      ...
      if (parsed && typeof parsed === 'object') {
        if (parsed.pins && typeof parsed.pins === 'object') {
          this.tabPins = Object.fromEntries(
            Object.entries(parsed.pins).map(([key, value]) => [
              key,
              Boolean(value),
            ]),
          );
        ...
        if (Array.isArray(parsed.order)) {
          this.tabOrder = parsed.order.filter(
  ...
  private openContext(tab: MainContentTab, e: MouseEvent): void {
    ...
    const pad = 10;
    this.contextX = Math.min(
      Math.max(pad, e.clientX + 6),
      Math.max(pad, window.innerWidth - menuWidth - pad),
    );
    this.contextY = Math.min(
      Math.max(pad, e.clientY + 6),
  ...
  private getContextTab(): MainContentTab | null {
    ...
    return (
      this.tabs.find((tab) => this.tabKey(tab) === this.contextTabKey) ?? null
    );
  }
  ...
  private toggleContextTabPinned(): void {
    ...
    this.tabs = this.applyStoredLayoutToTabs(
      this.tabs.map((entry) =>
        this.tabKey(entry) === key ? { ...entry, pinned: nextPinned } : entry,
      ),
    );
    this.persistTabLayout();
    this.closeContext();
  ...
  private beginTabPointerInteraction(
    tab: MainContentTab,
    event: PointerEvent,
  ): void {
  ...
  private activateTabDrag(tabKey: string): void {
    const tab =
      this.tabs.find((entry) => this.tabKey(entry) === tabKey) ?? null;
    if (!tab) return;
    const element = this.container.querySelector<HTMLElement>(
      `.content-tab[data-tab-key="${tabKey}"]`,
    );
    const rect = element?.getBoundingClientRect();
  ...
  private updatePreviewOrderFromPointer(): void {
    ...
    const draggedTab = previewTabs.find(
      (tab) => this.tabKey(tab) === draggedKey,
    );
    if (!draggedTab) return;

    const draggedPinned = this.isTabPinned(draggedTab);
    const sameGroupTabs = previewTabs.filter(
      (tab) => this.isTabPinned(tab) === draggedPinned,
    );
    const otherTabs = sameGroupTabs.filter(
      (tab) => this.tabKey(tab) !== draggedKey,
    );
    ...
    const pinnedGroup = draggedPinned
      ? reorderedGroup
      : previewTabs.filter((tab) => this.isTabPinned(tab));
    const unpinnedGroup = draggedPinned
      ? previewTabs.filter((tab) => !this.isTabPinned(tab))
      : reorderedGroup;
  ...
  private finishTabPointerInteraction(): void {
    ...
    const finalIds =
      previewTabs?.map((tab) => this.tabKey(tab)).join('|') ?? initialIds;
    const orderChanged = Boolean(
      draggingKey && previewTabs && finalIds !== initialIds,
    );
  ...
  private getDragGhostLeft(): number {
    const scroll = this.container.querySelector<HTMLElement>(
      '.content-tabs-scroll',
    );
    if (!scroll) return 0;
    const rect = scroll.getBoundingClientRect();
    return (
      this.dragCurrentX - this.dragGrabOffsetX - rect.left + scroll.scrollLeft
    );
  }
  ...
  private scheduleOverflowMeasurement(): void {
    ...
    this.overflowMeasureFrame = requestAnimationFrame(() => {
      ...
      const titleEls =
        this.container.querySelectorAll<HTMLElement>('.content-tab-title');
  ...
  private renderTab(
    ...
    const pinnedBoundary = Boolean(
      pinned && (!nextTab || !this.isTabPinned(nextTab)),
    );
    return html`
			<div
          ...
          placeholder
            ...
            : html`
              ...
              this.onSelect?.(tab.id);
						...
						${
              tab.closable
                ? html`<button class="content-tab-close" @click=${(
                    event: Event,
                  ) => {
  ...
  render(): void {
    ...
    const renderedTabs = this.getRenderedTabs();
    const draggingTab =
      this.tabs.find((tab) => this.tabKey(tab) === this.draggingTabKey) ?? null;
