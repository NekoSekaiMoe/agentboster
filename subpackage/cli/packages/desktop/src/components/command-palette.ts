...
export class CommandPalette {
  ...
  private onRunSlashCommand:
    | ((commandText: string) => boolean | Promise<boolean>)
    | null = null;
  ...
  setOnRunSlashCommand(
    callback: ((commandText: string) => boolean | Promise<boolean>) | null,
  ): void {
  ...
  private async loadCommands(): Promise<void> {
    ...
    try {
      ...
      rpcCommands = Array.isArray(result)
    ...
    const slashCommands: PaletteCommand[] = normalizeRuntimeSlashCommands(
      rpcCommands,
    ).map((command) => {
  ...
  private filterCommands(): void {
    ...
    this.filteredCommands = this.commands
      .filter((cmd) =>
        `${cmd.name} ${cmd.description}`.toLowerCase().includes(query),
      )
      .slice(0, 30);
