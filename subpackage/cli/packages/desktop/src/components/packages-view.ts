...
import {
  normalizeRecommendedSource,
  RECOMMENDED_PACKAGES,
  type RecommendedPackageDefinition,
...
import {
  ...
  withExtensionCommandUsageHint,
} from '../extensions/extension-command-hints.js';
import {
  getBundledThemesStatus,
  isBundledThemeId,
  removeBundledThemes,
  restoreBundledThemes,
} from '../theme/bundled-themes.js';
...
const DESKTOP_THEMES_DOC_URL =
  'https://github.com/gustavonline/pi-desktop/blob/dev/docs/THEMES_DESKTOP_MAPPING.md';
const AUTO_RENAME_PRIMARY_SOURCE = normalizeRecommendedSource(
  'npm:@byteowlz/pi-auto-rename',
);
const AUTO_RENAME_LEGACY_SOURCE = normalizeRecommendedSource(
...
const SMART_NOTIFY_PRIMARY_SOURCE = normalizeRecommendedSource(
  'npm:pi-smart-voice-notify',
);
const SMART_NOTIFY_LEGACY_SOURCE = normalizeRecommendedSource(
...
function parsePiListOutput(output: string): {
  user: InstalledPackageItem[];
  project: InstalledPackageItem[];
} {
...
function npmPackageNameFromSource(source: string): string {
  ...
  if (/^[@a-z0-9][\w./-]*$/i.test(source) && !source.includes(':'))
    return source.trim().toLowerCase();
  return '';
}

function pathMatchesNpmPackage(
...
function commandLikelyNeedsModelArg(command: PackageConfigCommand): boolean {
  ...
  return (
    /(^|\b)(model|provider)(\b|$)/.test(haystack) ||
    haystack.includes('provider/model')
  );
}

function normalizeAutoRenameConfigDraft(
  raw: Record<string, unknown>,
): AutoRenameConfigDraft {
  ...
  return {
    ...
    modelSelection:
      readString(raw.modelSelection) === 'cheapest' ? 'cheapest' : 'current',
    model,
    fallbackModel,
    fallbackDeterministic:
      ...
      readString(raw.fallbackDeterministic) === 'readable-id'
        ? (readString(
            raw.fallbackDeterministic,
          ) as AutoRenameConfigDraft['fallbackDeterministic'])
    ...
    readableIdSuffix:
      readBoolean(raw.readableIdSuffix) ?? defaults.readableIdSuffix,
...
function readModelRefFromConfigObject(
  parsed: Record<string, unknown>,
): string | null {
  const provider =
    readString(parsed.provider) ||
    readString(parsed.providerId) ||
    readString(parsed.provider_id);
  const id =
    readString(parsed.id) ||
    readString(parsed.modelId) ||
    readString(parsed.model_id);
...
function parseSkillDoc(content: string): ParsedSkillDoc {
  ...
  let title = attributes.name
    ? toTitleFromSlug(attributes.name)
    : 'Skill details';
  ...
  if (sections.length === 0 && body.trim()) {
    sections.push({
      heading: null,
      paragraphs: [normalizeDocLine(body.trim())],
    });
  }
...
function sourceKindLabel(
  kind: 'npm' | 'git' | 'url' | 'local' | 'unknown',
): string {
  switch (kind) {
    case 'npm':
      return 'npm';
...
function inferSourceKindFromSource(
  source: string,
): 'npm' | 'git' | 'url' | 'local' | 'unknown' {
  const value = source.trim();
  if (!value) return 'unknown';
  if (value.startsWith('npm:') || /^[@a-z0-9][\w./-]*$/i.test(value))
    return 'npm';
...
function readCommandSourceInfo(raw: Record<string, unknown>): {
  ...
  const info = (raw.sourceInfo ?? raw.source_info) as
    | Record<string, unknown>
    | undefined;
  const path =
    readString(raw.path) || readString(raw.location) || readString(info?.path);
...
export class PackagesView {
  ...
  private onInsertPromptTemplate:
    | ((commandText: string) => void | Promise<void>)
    | null = null;
  ...
  private refreshAutoRenameProjectOptions(): void {
    const providerItems = this.projectOptionsProvider
      ? this.projectOptionsProvider()
      : [];
    const merged = uniqueBy(
      [
        ...
        ...(this.currentProjectPath
          ? [
              {
                id: `current:${normalizeFsPath(this.currentProjectPath)}`,
                name: pathBaseName(this.currentProjectPath),
    ...
    if (this.autoRenameConfigSaveScope === 'project') {
      const selected =
        this.autoRenameProjectOptions.find(
          (item) => item.id === this.autoRenameConfigTargetProjectId,
        ) ??
        this.autoRenameProjectOptions.find(
          (item) =>
            normalizeFsPath(item.path) ===
            normalizeFsPath(this.autoRenameConfigTargetProjectPath),
        ) ??
        (this.currentProjectPath
          ? this.autoRenameProjectOptions.find(
              (item) =>
                normalizeFsPath(item.path) ===
                normalizeFsPath(this.currentProjectPath),
            )
          : null) ??
      ...
      const currentMatch = this.autoRenameProjectOptions.find(
        (item) =>
          normalizeFsPath(item.path) ===
          normalizeFsPath(this.currentProjectPath),
      );
  ...
  setOnInsertPromptTemplate(
    cb: ((commandText: string) => void | Promise<void>) | null,
  ): void {
  ...
  async open(): Promise<void> {
    await Promise.all([
      this.loadCatalog(this.catalogItems.length === 0),
      this.loadConfig(),
      this.refreshBundledThemesStatus(),
    ]);
  ...
  private updateBundledThemeInstallStateFromResources(
    resources: DiscoveredThemeItem[],
  ): void {
    const bundledResources = resources.filter((item) =>
      isBundledThemeId(item.id),
    );
    if (bundledResources.length === 0) return;
    this.desktopThemesInstalledCount = Math.max(
      this.desktopThemesInstalledCount,
      bundledResources.length,
    );
  ...
  private canEditResource(item: DiscoveredResourceItem): boolean {
    ...
    return (
      this.isResourcePathWithin(item.path, globalRoot) ||
      this.isResourcePathWithin(item.path, projectRoot)
    );
  }

  private normalizeResourceListItem(
  ...
  private async loadConfig(): Promise<void> {
    ...
    try {
      const userListResult = await rpcBridge.runPiCliCommand(['list'], {
        cwd: '/',
      });
      const parsedUser = parsePiListOutput(userListResult.stdout ?? '');

      this.installedUser = uniqueBy(
        [...parsedUser.user],
        (item) => item.source,
      );
      ...
      if (migratedNotifyPackage) {
        const refreshedListResult = await rpcBridge.runPiCliCommand(['list'], {
          cwd: '/',
        });
        const refreshedParsed = parsePiListOutput(
          refreshedListResult.stdout ?? '',
        );
        this.installedUser = uniqueBy(
      ...
      if (
        this.activePackageConfigSource &&
        !this.isNormalizedSourceInstalled(this.activePackageConfigSource)
      ) {
  ...
  private async migrateLegacyNotifyPackageIfNeeded(): Promise<boolean> {
    ...
    const legacyInstalled =
      this.installedUser.find(
        (item) =>
          normalizeRecommendedSource(item.source) ===
          SMART_NOTIFY_LEGACY_SOURCE,
      ) ?? null;
    if (!legacyInstalled) return false;
    this.smartNotifyMigrationAttempted = true;

    const hasPreferred = this.installedUser.some(
      (item) =>
        normalizeRecommendedSource(item.source) === SMART_NOTIFY_PRIMARY_SOURCE,
    );
    let changed = false;

    if (!hasPreferred) {
      const installOk = await this.executePackageCommand(
        ['install', SMART_NOTIFY_INSTALL_SOURCE],
        {
          scope: 'global',
    ...
    const removeOk = await this.executePackageCommand(
      ['remove', removeSource],
      {
        scope: 'global',
        appendLocalFlag: false,
  ...
  private async refreshPackageConfigCommands(): Promise<void> {
    ...
    for (const raw of commands) {
      ...
      const normalizedSource = normalizeRecommendedSource(
        matchedPackage.source,
      );
  ...
  private matchesInstalledItemPath(
    path: string,
    ...
    const locationMatches = normalizedLocation
      ...
      : false;
    const npmFallbackMatches =
      !locationMatches && pathMatchesNpmPackage(normalizedPath, item.source);
  ...
  private findInstalledItemForCommand(
    ...
    const normalizedSourceHint =
      meta.sourceHint && meta.sourceHint.toLowerCase() !== 'auto'
    ...
    if (normalizedSourceHint) {
      const bySource =
        installedItems.find(
          (item) =>
            normalizeRecommendedSource(item.source) === normalizedSourceHint,
        ) ?? null;
  ...
  private toDiscoveredResourceItem(
    raw: Record<string, unknown>,
    installedItems: InstalledDisplayItem[],
  ): DiscoveredResourceItem | null {
    ...
    const sourceInfoSource =
      sourceInfo.source && sourceInfo.source.toLowerCase() !== 'auto'
        ? sourceInfo.source
        : '';
    ...
    const packageDisplayName =
      ...
      (packageSource ? this.getDisplayName(packageSource) : null);
    const commandText = name.startsWith('/') ? name : `/${name}`;
    const idBase =
      `${kind}:${name}:${path || packageSource || sourceInfo.origin || 'runtime'}`.toLowerCase();
    return {
      id: idBase,
      kind,
      name,
      ...
      sourceKind: packageSource
        ? inferSourceKindFromSource(packageSource)
        : 'unknown',
    };
  }

  private async listDirSafe(path: string): Promise<
    Array<{
      name: string;
      isDirectory: boolean;
      isFile: boolean;
  ...
  private async scanPromptResources(
    scope: 'global' | 'project',
  ...
  private async scanSkillResources(
    scope: 'global' | 'project',
  ): Promise<DiscoveredResourceItem[]> {
  ...
  private mergeDiscoveredResources(
    ...
    const merged = localResources.map((item) =>
      this.normalizeResourceListItem(item),
    );
    for (const item of commandResources) {
      ...
      let matchIndex = normalizedPath
        ? merged.findIndex(
            (existing) => normalizeFsPath(existing.path) === normalizedPath,
          )
        : -1;
      ...
      merged[matchIndex] = {
        ...
        packageDisplayName:
          normalized.packageDisplayName ?? existing.packageDisplayName,
  ...
  private async refreshDiscoveredResources(): Promise<void> {
    ...
    try {
      ...
      const merged = this.mergeDiscoveredResources(
        commandResources,
        localResources,
      );
  ...
  private getPackageConfigCommandArg(
    source: string,
    commandName: string,
  ): string {
    ...
    for (const variant of variants) {
      ...
      if (this.packageConfigCommandArgs.has(key))
        return (this.packageConfigCommandArgs.get(key) || '').trim();
    }
        ...
        if (stored && stored.trim()) {
          ...
          for (const syncVariant of variants) {
            this.packageConfigCommandArgs.set(
              this.packageConfigCommandKey(source, syncVariant),
              trimmed,
            );
          }
          return trimmed;
  ...
  private setPackageConfigCommandArg(
    source: string,
    commandName: string,
    value: string,
  ): void {
    ...
    if (!trimmed) {
      for (const variant of variants) {
        this.packageConfigCommandArgs.delete(
          this.packageConfigCommandKey(source, variant),
        );
        try {
          localStorage.removeItem(
            this.packageConfigStorageKey(source, variant),
          );
        } catch {
          // ignore
        }
  ...
  private seedModelArgsForSource(
    source: string,
    commands: PackageConfigCommand[],
  ): void {
    if (this.configModels.length === 0) return;
    const fallback = modelRef(
      this.configModels[0].provider,
      this.configModels[0].id,
    );
  ...
  private async ensureConfigModelsLoaded(force = false): Promise<void> {
    ...
    try {
      ...
      this.configModels = this.mapModelOptions(
  ...
  private isNormalizedSourceInstalled(normalizedSource: string): boolean {
    return this.getInstalledItems(false).some(
      (item) => normalizeRecommendedSource(item.source) === normalizedSource,
    );
  }
  ...
  private isAutoRenameSource(source: string): boolean {
    const normalized = normalizeRecommendedSource(source);
    return (
      normalized === AUTO_RENAME_PRIMARY_SOURCE ||
      normalized === AUTO_RENAME_LEGACY_SOURCE
    );
  }

  private isAutoRenameConfigCommand(
    source: string,
  ...
  private resolveAutoRenameCommandForSource(
    source: string,
  ): PackageConfigCommand | null {
  ...
  private resolveProjectOptionForPath(
    path: string,
  ): PackageProjectOption | null {
    const normalized = normalizeFsPath(path);
    if (!normalized) return null;
    const sorted = [...this.autoRenameProjectOptions].sort(
      (a, b) => normalizeFsPath(b.path).length - normalizeFsPath(a.path).length,
    );
  ...
  private async findExistingProjectAutoRenameConfig(
    projectPath: string,
  ): Promise<{ path: string; scopeLabel: string } | null> {
    const normalizedProjectPath = projectPath.trim();
    if (!normalizedProjectPath) return null;
    const { exists } = await import('@tauri-apps/plugin-fs');
    const option = this.resolveProjectOptionForPath(normalizedProjectPath);
    const projectLabel =
      option?.name || pathBaseName(normalizedProjectPath) || 'Project';
    const candidates = [
  ...
  private async resolveProjectAutoRenameConfigPath(
    projectPath: string,
  ): Promise<{ path: string; scopeLabel: string }> {
    const normalizedProjectPath = projectPath.trim();
    if (!normalizedProjectPath) return { path: '', scopeLabel: '' };
    const existing = await this.findExistingProjectAutoRenameConfig(
    ...
    return {
      path: joinFsPath(
        joinFsPath(normalizedProjectPath, '.pi'),
        'auto-rename.json',
      ),
  ...
  private applyAutoRenameScopeFromPath(path: string): void {
    const normalized = normalizeFsPath(path);
    const globalPath = normalizeFsPath(
      this.resolveGlobalAutoRenameConfigPath(),
    );
    ...
    const projectOption =
      this.resolveProjectOptionForPath(path) ??
      this.autoRenameProjectOptions.find(
        (item) =>
          normalizeFsPath(item.path) ===
          normalizeFsPath(this.currentProjectPath),
  ...
  private async resolveAutoRenameConfigPathPreference(options?: {
    ...
    if (requestedScope === 'project') {
      ...
      if (!projectPath) {
        throw new Error(
          'No project selected for project-scoped auto-rename settings.',
        );
      }
    ...
    for (const projectPath of preferredProjectPaths) {
      const existing =
        await this.findExistingProjectAutoRenameConfig(projectPath);
      if (existing) return existing;
    }
    ...
    if (preferredProjectPaths.length > 0) {
      return await this.resolveProjectAutoRenameConfigPath(
        preferredProjectPaths[0],
      );
    }
    if (globalPath) {
      return { path: globalPath, scopeLabel: 'Global' };
  ...
  private async ensureAutoRenameConfigLoaded(
    ...
    if (
      ...
      normalizeFsPath(this.autoRenameConfigPath) ===
        normalizeFsPath(preferred.path)
    ) {
      return;
    }
    ...
    try {
      ...
      if (options?.scope === 'project' && options.projectPath?.trim()) {
        const selected = this.resolveProjectOptionForPath(
  ...
  private async setAutoRenameSaveScope(
    source: string,
    scope: 'global' | 'project',
  ): Promise<void> {
    ...
    if (scope === 'project') {
      ...
      if (!selected) {
        this.autoRenameConfigError =
          'No opened project is available for project-scoped save.';
        this.render();
        return;
      }
      this.autoRenameConfigTargetProjectId = selected.id;
      this.autoRenameConfigTargetProjectPath = selected.path;
      await this.ensureAutoRenameConfigLoaded(source, true, {
        scope: 'project',
  ...
  private async setAutoRenameTargetProject(
    source: string,
    projectId: string,
  ): Promise<void> {
    const selected =
      this.autoRenameProjectOptions.find((item) => item.id === projectId) ??
      null;
    if (!selected) return;
    this.autoRenameConfigTargetProjectId = selected.id;
    this.autoRenameConfigTargetProjectPath = selected.path;
  ...
  private setAutoRenameConfigField<K extends keyof AutoRenameConfigDraft>(
    key: K,
    value: AutoRenameConfigDraft[K],
  ): void {
  ...
  private async saveAutoRenameConfig(source: string): Promise<void> {
    if (
      this.autoRenameConfigSaving ||
      this.runningConfigCommand ||
      this.runningCommand
    )
      return;
    ...
    try {
      ...
      const targetProjectPath =
        targetScope === 'project' ? this.autoRenameConfigTargetProjectPath : '';
      ...
      const primaryModel = primaryModelValue
        ? splitModelRef(primaryModelValue)
        : null;
      const fallbackModel = fallbackModelValue
        ? splitModelRef(fallbackModelValue)
        : null;
      ...
      const nextDoc: Record<string, unknown> = {
        ...this.autoRenameConfigRaw,
        $schema:
          readString(this.autoRenameConfigRaw.$schema) ||
          AUTO_RENAME_SCHEMA_RELATIVE,
        model: primaryModel
          ? { provider: primaryModel.provider, id: primaryModel.id }
          : null,
        fallbackModel: fallbackModel
          ? { provider: fallbackModel.provider, id: fallbackModel.id }
          : null,
        ...
        prefixCommand: this.autoRenameConfig.prefixCommand.trim()
          ? this.autoRenameConfig.prefixCommand.trim()
          : null,
  ...
  private async findInstalledItemForExtensionCommand(
    commandName: string,
  ): Promise<InstalledDisplayItem | null> {
  ...
  private ensureSyntheticExtensionConfigCommand(
    ...
    const currentCommands = [
      ...(this.packageConfigCommands.get(normalizedSource) ?? []),
    ];
    if (
      !currentCommands.some((command) =>
        commandNamesMatch(command.name, normalizedCommand),
      )
    ) {
    ...
    if (args.trim()) {
      this.setPackageConfigCommandArg(
        normalizedSource,
        normalizedCommand,
        args.trim(),
      );
  ...
  async openExtensionConfigByCommand(
    commandName: string,
    args = '',
  ): Promise<boolean> {
    const normalizedCommand = normalizeCommandNameForMatch(commandName);
    ...
    const installed =
      await this.findInstalledItemForExtensionCommand(normalizedCommand);
    if (installed) {
      const source = normalizeRecommendedSource(installed.source);
      this.ensureSyntheticExtensionConfigCommand(
        source,
        normalizedCommand,
        args,
        installed,
    ...
    const heuristicMatches = this.getInstalledItems(false).filter((item) => {
      const normalizedSource = normalizeRecommendedSource(item.source);
      const sourceName = normalizedSource.startsWith('npm:')
        ? normalizedSource.slice(4)
        : normalizedSource;
      const sourceTail =
        sourceName.split('/').filter(Boolean).pop() || sourceName;
      const display = item.displayName.toLowerCase();
      return (
        display.includes(normalizedCommand) ||
        sourceName.includes(normalizedCommand) ||
        sourceTail.includes(normalizedCommand)
      );
    });
  ...
  async openExtensionConfigByProvider(provider: string): Promise<boolean> {
    ...
    const tokenParts = providerToken
      .split(/[-_.]/)
  ...
  private async loadPackageConfigFromPackage(
    source: string,
    commands: PackageConfigCommand[],
  ): Promise<void> {
    ...
    try {
      ...
      await this.ensureHomePath();
      const { exists, readTextFile, readDir, stat } = await import(
      ...
      for (const filePath of candidates) {
        try {
          ...
          for (const command of commands) {
            ...
            const key = command.name.startsWith('/')
      ...
      if (loadedFromPath) {
        if (loadedModelValue) {
          this.packageConfigLoadedModelBySource.set(
            normalizeRecommendedSource(source),
            loadedModelValue,
          );
        }
        this.packageConfigStatus = `Loaded package model ${loadedModelValue || '(unknown)'} from ${loadedFromPath}.`;
      } else if (
        commands.some((command) => commandLikelyNeedsModelArg(command))
      ) {
        this.packageConfigLoadedModelBySource.delete(
          normalizeRecommendedSource(source),
        );
        this.packageConfigStatus =
          'No saved model found on disk. Using package default.';
      }
      this.render();
    } catch {
      // ignore errors
  ...
  private async runPackageConfigCommand(
    ...
    try {
      ...
      if (
        ...
        this.packageConfigLoadedModelBySource.set(
          normalizeRecommendedSource(this.activePackageConfigSource),
          trimmedArgs,
        );
      }
      ...
      if (
        ...
        try {
          ...
          const installed = this.findInstalledItemForSource(
            normalized,
            'global',
          );
          const basePath =
            installed?.location?.trim() || command.path?.trim() || '';
          const key = command.name.startsWith('/')
            ? command.name.slice(1)
            : command.name;
          ...
          await this.ensureHomePath();
          const { exists, readTextFile, writeTextFile, stat, mkdir } =
          ...
          if (parsedModel && this.homePath) {
            try {
              const extensionsDir = joinFsPath(
                joinFsPath(joinFsPath(this.homePath, '.pi'), 'agent'),
                'extensions',
              );
              const extensionConfigPath = joinFsPath(
                extensionsDir,
                `${pkgName}.json`,
              );
              ...
              if (await exists(extensionConfigPath)) {
                try {
                  const raw = JSON.parse(
                    await readTextFile(extensionConfigPath),
                  ) as unknown;
  ...
  private renderPackageConfigCommandEditor(
    source: string,
    command: PackageConfigCommand,
  ): TemplateResult {
    ...
    const isSmartNotifyCommand = SMART_NOTIFY_COMMAND_NAMES.has(
      normalizedCommandName,
    );
    const sourceLoadedModel =
      this.packageConfigLoadedModelBySource.get(
        normalizeRecommendedSource(source),
      ) || '';
    const currentArgs =
      this.getPackageConfigCommandArg(source, command.name) ||
      (supportsModelPicker ? sourceLoadedModel : '');
    const selectedModel = this.configModels.some(
      (model) => modelRef(model.provider, model.id) === currentArgs,
    )
      ? currentArgs
      : '';
    const modelSelectValue = selectedModel || currentArgs || '';
    const title = command.description?.trim() || 'Package setting';
    const defaultDescription = supportsModelPicker
  ...
  private renderAutoRenameConfigEditor(
    source: string,
    command: PackageConfigCommand,
  ): TemplateResult {
    const busy =
    ...
    const usageHint =
      extensionCommandUsageHint(command.name) ||
      'Args: config, test, init, regen, <name>';
    const supportsProjectScope = Boolean(this.currentProjectPath);
    const configScopeLabel =
      this.autoRenameConfigScopeLabel ||
      (supportsProjectScope ? 'Project (.pi)' : 'Global');
    ...
    const primaryInOptions = modelOptions.some(
      (model) => modelRef(model.provider, model.id) === modelValue,
    );
    const fallbackInOptions = modelOptions.some(
      (model) => modelRef(model.provider, model.id) === fallbackModelValue,
    );
    ...
    const modeControlsDisabled =
      busy || extensionDisabled || prefixOnlyActive || hasExplicitModels;
    const basicControlsDisabled = busy || extensionDisabled;
    const targetProjectMissing =
      saveScope === 'project' && !activeProjectTarget;
    const saveDisabled =
      busy || this.autoRenameConfigLoading || targetProjectMissing;
            ...
            : html`
						...
						<div class="packages-config-field-grid">
							<div class="packages-config-field">
								...
								<select
									...
									@change=${(event: Event) => {
                    const value =
                      (event.target as HTMLSelectElement).value === 'cheapest'
                        ? 'cheapest'
                        : 'current';
                    ...
                    this.setAutoRenameConfigField('prefix', value);
                      ...
                      const value = (event.target as HTMLSelectElement)
                        .value as AutoRenameConfigDraft['fallbackDeterministic'];
                      this.setAutoRenameConfigField(
                        'fallbackDeterministic',
                        value,
                      );
                    }}
				...
				<div class="packages-config-command-actions">
					<div class="packages-config-save-inline">
						...
						<select
							...
							@change=${(event: Event) => {
                const value =
                  (event.target as HTMLSelectElement).value === 'project'
                    ? 'project'
                    : 'global';
                if (value === 'project' && !canUseProjectScope) {
                  this.autoRenameConfigError =
                    'No opened projects available for project-scoped save.';
                  this.render();
                  return;
                }
  ...
  private resolveSourceUrl(source: string): string | null {
    ...
    if (normalizeRecommendedSource(source) === DESKTOP_THEMES_PACKAGE_SOURCE)
      return DESKTOP_THEMES_DOC_URL;
    if (/^https?:\/\//i.test(source)) return source;
    if (source.startsWith('npm:'))
      return `https://www.npmjs.com/package/${source.slice(4)}`;
    if (source.startsWith('github:'))
      return `https://github.com/${source.slice(7).replace(/^\/+/, '')}`;
    ...
    if (/^[@a-z0-9][\w./-]*$/i.test(source))
      return `https://www.npmjs.com/package/${source}`;
    return null;
  }
  ...
  private getInstalledItems(applyQuery = true): InstalledDisplayItem[] {
    ...
    return unique.filter((item) =>
      `${item.displayName} ${item.source} ${item.location}`
        .toLowerCase()
        .includes(q),
    );
  }
  ...
  private filteredThemeResources(): DiscoveredThemeItem[] {
    ...
    return this.themeResources.filter((item) => {
      const haystack =
        `${item.name} ${item.description} ${item.variant} ${item.path}`.toLowerCase();
      return haystack.includes(q);
    });
  }

  private parseThemeRgb(
    color: string,
  ...
  private inferThemeVariantFromBackground(
    background: string,
  ): 'light' | 'dark' {
  ...
  private resolveThemeColorValue(
  ...
  private async scanThemeResources(): Promise<DiscoveredThemeItem[]> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const colors =
          (parsed.colors as Record<string, unknown> | undefined) ?? {};
        const accent =
          this.resolveThemeColorValue(parsed, colors.accent) ?? '#7A818F';
  ...
  private isRecommendedSkillInstalled(
    ...
    const normalizedPackageSource = normalizeRecommendedSource(
      definition.packageSource,
    );
    const isLocalRecommendedSkill =
      normalizedPackageSource.startsWith('local:');
    if (isLocalRecommendedSkill) return true;

    const resourceSource = (resource.packageSource || '').trim();
    if (!resourceSource) return false;
    return this.sourceMatchesInstalled(
      definition.packageSource,
      resourceSource,
    );
  }
  ...
  private buildRecommendedSkillItems(): RecommendedSkillSurfaceItem[] {
    return RECOMMENDED_SKILLS.map((definition) => {
      ...
      const packageInstalled = this.isSourceInstalledForScope(
        definition.packageSource,
        'global',
      );
      const installed = this.isRecommendedSkillInstalled(
        definition,
        resource,
        packageInstalled,
      );
  ...
  private recommendedSkillNameSet(
    items: RecommendedSkillSurfaceItem[],
  ): Set<string> {
    return new Set(
      items.map((item) => item.definition.skillName.toLowerCase()),
    );
  }
  ...
  private async copyFilePreservingMode(
    sourcePath: string,
    targetPath: string,
  ): Promise<void> {
    ...
    try {
      ...
      if (
        typeof sourceInfo.mode === 'number' &&
        Number.isFinite(sourceInfo.mode)
      ) {
  ...
  private async resolveSkillDirectoryFromPackage(
    basePath: string,
    skillName: string,
  ): Promise<string | null> {
    ...
    try {
      ...
      while (queue.length > 0) {
        ...
        for (const entry of entries) {
          if (!entry.isDirectory || this.shouldSkipPackagedSkillDir(entry.name))
            continue;
          const fullPath = joinFsPath(next.path, entry.name);
          const normalizedName = entry.name.trim().toLowerCase();
          if (
            normalizedName === normalizedSkill &&
            (await exists(joinFsPath(fullPath, 'SKILL.md')))
          ) {
            return fullPath;
          }
          queue.push({ path: fullPath, depth: next.depth + 1 });
  ...
  private async materializeRecommendedSkillFromPackage(
    item: RecommendedSkillSurfaceItem,
  ): Promise<string | null> {
    if (item.definition.sourceKind === 'local')
      return item.resource?.path || null;
    await this.ensureHomePath();
    ...
    const installedPackage = this.findInstalledItemForSource(
      item.definition.packageSource,
      'global',
    );
    const basePath = installedPackage?.location?.trim();
    if (!basePath) return null;

    const sourceDir = await this.resolveSkillDirectoryFromPackage(
      basePath,
      item.definition.skillName,
    );
    ...
    if (!(await exists(targetSkillPath)) && (await exists(sourceSkillPath))) {
      ...
      await writeTextFile(targetSkillPath, sourceSkillContent, {
        append: false,
        create: true,
      });
    }
    ...
    if (!(await exists(targetSkillPath))) {
      throw new Error(
        `Missing SKILL.md for ${item.definition.skillName} after install.${copyError ? ` ${copyError}` : ''}`,
      );
    }
    return targetSkillPath;
  }

  private async resolvePackageSkillContentPath(
    skillName: string,
    packageSource: string | null,
  ): Promise<string | null> {
    ...
    const skillDir = await this.resolveSkillDirectoryFromPackage(
      basePath,
      skillName,
    );
  ...
  private async resolveRecommendedSkillContentPath(
    item: RecommendedSkillSurfaceItem,
  ): Promise<string | null> {
    if (item.resource?.path) return item.resource.path;
    return this.resolvePackageSkillContentPath(
      item.definition.skillName,
      item.definition.packageSource,
    );
  }

  private async loadActiveSkillContent(
    path: string | null,
  ...
  private async runSkill(item: DiscoveredResourceItem): Promise<void> {
    await this.stageResourceCommandInChat(
      item.commandText,
      `Prepared ${item.commandText} in chat. Press Enter to run.`,
    );
  }

  private async installRecommendedSkill(
    item: RecommendedSkillSurfaceItem,
  ): Promise<void> {
    ...
    try {
      materializedSkillPath =
        await this.materializeRecommendedSkillFromPackage(item);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
    ...
    await this.refreshDiscoveredResources();
    const resolvedSkill = this.findSkillResourceByName(
      item.definition.skillName,
    );
    if (!resolvedSkill) {
    ...
    const setupSuffix = item.definition.setupHint
      ? ` Setup: ${item.definition.setupHint}`
      : '';
    const skillPath = materializedSkillPath || resolvedSkill.path;
    const locationSuffix = skillPath
      ? ` Skill files at ${pathDirName(skillPath)}.`
      : '';
    await this.stageResourceCommandInChat(
      commandText,
      `Prepared ${commandText} in chat. Press Enter to run.${locationSuffix}${setupSuffix}`,
    );
  ...
  private async triggerCreatorSkillInChat(): Promise<void> {
    const creatorItem =
      this.buildRecommendedSkillItems().find(
        (item) => item.definition.skillName === RESOURCE_CREATOR_SKILL_NAME,
      ) ?? null;
    if (creatorItem) {
    ...
    await this.stageResourceCommandInChat(
      commandText,
      `Prepared ${commandText} in chat. Press Enter to run.`,
    );
  }
  ...
  private resourceCreatorSkillPath(): string | null {
    if (!this.homePath) return null;
    const skillsRoot = joinFsPath(
      joinFsPath(joinFsPath(this.homePath, '.pi'), 'agent'),
      'skills',
    );
    return joinFsPath(
      joinFsPath(skillsRoot, RESOURCE_CREATOR_SKILL_NAME),
      'SKILL.md',
    );
  }
...
`;
    ...
    try {
      ...
      try {
        ...
        const assetUrl = new URL(
          '../../assets/skills/creatorskill/SKILL.md',
          import.meta.url,
        ).href;
  ...
  private resolveResourceCreatorLoadedCommand(): string | null {
    const entry = this.skillResources.find(
      (item) => item.name === RESOURCE_CREATOR_SKILL_NAME && item.loaded,
    );
...
Execute the required file creation/edits directly, then summarize exactly which files were created.`;
  ...
  private async runResourceCreator(): Promise<void> {
    ...
    try {
      ...
      const commandPrefix =
        loadedCommand || `/skill:${RESOURCE_CREATOR_SKILL_NAME}`;
  ...
  private renderResourceCreatorModal(): TemplateResult | typeof nothing {
    ...
    const globalTarget = this.homePath
      ...
      : '~/.pi/agent';
    const creatingSkill =
      this.resourceCreatorKindLocked && this.resourceCreatorKind === 'skill';
    return html`
                  ...
                  ? html`
									<div class="packages-config-field">
										...
										<select
											...
											@change=${(event: Event) => {
                        this.resourceCreatorKind = (
                          event.target as HTMLSelectElement
                        ).value as 'auto' | 'prompt' | 'skill';
                        this.render();
                      }}
										>
  ...
  private statusTone(): 'info' | 'success' | 'error' {
    ...
    if (
      text.startsWith('installed') ||
      text.startsWith('removed') ||
      text.startsWith('updated') ||
      text.startsWith('refreshed')
    ) {
      return 'success';
    }
    return 'info';
      ...
      return lines
        .filter((line) => {
          ...
          return (
            ...
            lower.includes('notify-action') ||
            lower.includes('notify-target') ||
            lower.includes('extension_ui_request') ||
            lower.includes('rpc:event') ||
  ...
  private sourceMatchesInstalled(
    ...
    const bareSource = normalizedSource.startsWith('npm:')
      ? normalizedSource.slice(4)
      : normalizedSource;
    const bareInstalled = normalizedInstalled.startsWith('npm:')
      ? normalizedInstalled.slice(4)
      : normalizedInstalled;
    return bareSource === bareInstalled;
  }

  private isSourceInstalledForScope(
    source: string,
    _scope: 'global' | 'local',
  ): boolean {
    return this.installedUser.some((item) =>
      this.sourceMatchesInstalled(source, item.source),
    );
  }
  ...
  private extensionInstallState(source: string): {
    global: boolean;
    project: boolean;
  } {
    if (normalizeRecommendedSource(source) === DESKTOP_THEMES_PACKAGE_SOURCE) {
  ...
  private findInstalledItemForSource(
    source: string,
    _scope: 'global' | 'local',
  ): InstalledDisplayItem | null {
    if (normalizeRecommendedSource(source) === DESKTOP_THEMES_PACKAGE_SOURCE) {
  ...
  private buildExtensionSurfaceItems(
    ...
    const all = [...bySource.values()].sort((a, b) =>
      a.displayName.localeCompare(b.displayName),
    );
    const installedOnly = all.filter(
      (item) => item.installState.global || item.installState.project,
    );
    const galleryOnly = all.filter(
      (item) => !(item.installState.global || item.installState.project),
    );
  ...
  private async openPackagesItemModal(
    modal: ActivePackagesModal,
  ): Promise<void> {
    ...
    if (modal.kind === 'extension') {
      ...
      if (commands.length === 0 && this.isAutoRenameSource(source)) {
        this.ensureSyntheticExtensionConfigCommand(
          ...
          'auto-rename',
          'config',
          modal.item.installedItemForScope ?? null,
        );
      ...
      if (autoRenameCommand) {
        await this.ensureAutoRenameConfigLoaded(source, false, {
          scope: 'global',
        }).catch(() => {});
      }
      this.seedModelArgsForSource(source, commands);
    ...
    if (modal.kind === 'skill') {
      ...
      const fallbackPath = directPath
        ? null
        : await this.resolvePackageSkillContentPath(
            modal.item.name,
            modal.item.packageSource,
          );
    ...
    const contentPath = await this.resolveRecommendedSkillContentPath(
      modal.item,
    );
    ...
    try {
      ...
      if (resolved === '~' || resolved.startsWith('~/')) {
        try {
          ...
          resolved =
            resolved === '~' ? home : joinFsPath(home, resolved.slice(2));
        } catch {
  ...
  private async installExtensionItem(
    item: ExtensionSurfaceItem,
  ): Promise<void> {
    if (
      normalizeRecommendedSource(item.source) === DESKTOP_THEMES_PACKAGE_SOURCE
    ) {
      if (this.runningCommand || this.runningConfigCommand) return;
      this.runningCommand = true;
  ...
  private async uninstallExtensionItem(
    item: ExtensionSurfaceItem,
  ): Promise<void> {
    if (this.runningCommand || this.runningConfigCommand) return;
    this.closePackagesItemModal();
    ...
    const installed =
      item.installedItemForScope ??
      this.findInstalledItemForSource(item.source, 'global');
    if (!installed) return;
    await this.removePackage(
      this.resolveInstalledRemoveSource(installed),
      'global',
    );
    await this.refreshPackages(false);
  }
  ...
  private renderSkillContentBlock(): TemplateResult {
    ...
    if (this.activeSkillContent) {
      ...
      const modalTitle =
        this.activePackagesModal?.kind === 'recommended-skill'
      ...
      const sections = doc.sections.map((section, index) => {
        if (
          index === 0 &&
          section.heading &&
          normalizedModalTitle &&
          section.heading.trim().toLowerCase() === normalizedModalTitle
        ) {
  ...
  private renderPackagesItemModal(): TemplateResult | typeof nothing {
    ...
    if (modal.kind === 'extension') {
      ...
      return html`
                ...
                {
                  label: 'Status',
                  value: selectedScopeInstalled ? 'Installed' : 'Not installed',
                },
    ...
    if (modal.kind === 'theme') {
      ...
      return html`
                ...
                {
                  label: 'Type',
                  value: `${item.variant === 'dark' ? 'Dark' : 'Light'} theme`,
                },
    ...
    if (modal.kind === 'recommended-skill') {
      ...
      const commandText =
        resource?.commandText ?? `/skill:${definition.skillName}`;
      const skillPath = this.activeSkillContentPath;
      const skillOpenUrl =
        definition.openUrl ??
        (resource?.packageSource
          ? this.resolveSourceUrl(resource.packageSource)
          : null);
      ...
      return html`
                ...
                {
                  label: 'Path',
                  value: skillPath ?? 'Install to view location',
                },
              ])}
							${skillContent}
							${
                !this.activeSkillContent &&
                !this.activeSkillContentLoading &&
    ...
    const skillOpenUrl = item.packageSource
      ? this.resolveSourceUrl(item.packageSource)
      : null;
  ...
  render(): void {
    ...
    const themeResources = this.filteredThemeResources().filter(
      (item) => !isBundledThemeId(item.id),
    );
    const query = this.normalizeQuery();
    const hasQuery = query.length > 0;
    const queryLabel = this.query.trim();
    const recommendedSkills = this.buildRecommendedSkillItems().filter(
      (item) => {
        if (!query) return true;
        return `${item.definition.name} ${item.definition.description}`
          .toLowerCase()
          .includes(query);
      },
    );
    const recommendedSkillNames =
      this.recommendedSkillNameSet(recommendedSkills);
    const visibleSkillResources = skillResources.filter(
      (item) => !recommendedSkillNames.has(item.name.toLowerCase()),
    );
    const recommendedExtensions = this.buildRecommendedExtensionItems().filter(
      (item) => {
        if (!query) return true;
        return `${item.displayName} ${item.description} ${item.note ?? ''}`
          .toLowerCase()
          .includes(query);
      },
    );
    ...
    const recommendedPackages = RECOMMENDED_PACKAGES.filter((item) => {
      if (!query) return true;
      return `${item.name} ${item.description} ${item.installSourceHint}`
        .toLowerCase()
        .includes(query);
    });
    const extensionSurfaces = this.buildExtensionSurfaceItems(
      recommendedPackages,
      catalogItems,
      installedItems,
    );
    ...
    const extensionDiscover = hasQuery
      ? extensionSurfaces.gallery.filter(
          (item) =>
            !recommendedExtensionSources.has(
              normalizeRecommendedSource(item.source),
            ),
        )
      : [];
    ...
    const installedRecommendedSkills = recommendedSkills.filter(
      (item) => item.installed,
    );
    const availableRecommendedSkills = recommendedSkills.filter(
      (item) => !item.installed,
    );
    const availableRecommendedExtensions = recommendedExtensions.filter(
      (item) => !item.installState.global,
    );
    ...
    for (const item of installedRecommendedSkills) {
      const note =
        item.resource?.path ||
        `Package: ${this.getDisplayName(item.definition.packageSource)}`;
      addInstalledRow(item.definition.name.toLowerCase(), {
        ...
        onTitleClick: () =>
          void this.openPackagesItemModal({ kind: 'recommended-skill', item }),
      ...
      addInstalledRow(item.name.toLowerCase(), {
        ...
        note:
          ...
          (item.packageDisplayName
            ? `Package: ${item.packageDisplayName}`
            : 'Runtime command'),
        iconName: 'skill',
        ...
        onTitleClick: () =>
          void this.openPackagesItemModal({ kind: 'skill', item }),
    ...
    for (const item of themeResources) {
      addInstalledRow(`theme:${item.name.toLowerCase()}`, {
        ...
        onTitleClick: () =>
          void this.openPackagesItemModal({ kind: 'theme', item }),
        titleAttr: item.path,
      });
    }
    ...
    const sortedInstalled = installedRowsData.sort((a, b) => {
      const typeOrder = (opt: { sortKey: string; options: RowOptions }) =>
        opt.options.iconName === 'skill'
          ? 0
          : opt.options.iconName === 'extension'
            ? 1
            : 2;
    ...
    const installedRows = sortedInstalled.map((entry) =>
      this.renderPackageRow(entry.options),
    );

    const discoverRows = discoverRowsData
      .sort(
        (a, b) => a.priority - b.priority || a.sortKey.localeCompare(b.sortKey),
      )
    ...
    const discoverEmptyMessage = hasQuery
      ? 'No matches found.'
    ...
    const hasDiagnostics =
      ...
      notifyDebugLines.length > 0 ||
      this.commandOutput.trim().length > 0;
    const diagnosticsOpen =
      this.runningCommand || this.statusTone() === 'error';
    const loadingPackagesInitial =
      (this.loadingConfig || this.loadingResources) &&
      installedCount === 0 &&
      !this.configError &&
      !this.resourcesError;
