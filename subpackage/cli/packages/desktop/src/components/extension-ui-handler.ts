...
import {
  type Options as DesktopNotificationOptions,
  isPermissionGranted,
  onAction,
  requestPermission,
  sendNotification,
...
export function isSupportedExtensionUiMethod(
  value: unknown,
): value is UiMethod {
  return (
    typeof value === 'string' &&
    (SUPPORTED_EXTENSION_UI_METHODS as readonly string[]).includes(value)
  );
}
...
export function normalizeExtensionUiRequest(
...
function shouldSuppressUiStatusText(text: string): boolean {
  ...
  if (
    /^(?:💰|\$|usd|eur|dkk|kr|€|£|¥)?\s*\$?\s*\d+(?:[.,]\d+)?\s*(?:usd|eur|dkk|kr)?$/i.test(
      text,
    )
  ) {
    return true;
  ...
  if (
    /(?:↑\s*\d|↓\s*\d|(?:^|\s)r\d|(?:^|\s)w\d|\(sub\)|\(auto\)|\/\d+[km]|\bthinking\b)/i.test(
      normalized,
    )
  ) {
    return true;
...
export class ExtensionUiHandler {
  ...
  private onNotificationActionTarget:
    | ((target: NotificationActionTarget) => void)
    | null = null;
  private appWindowFocused =
  ...
  setNotificationActionHandler(
    handler: ((target: NotificationActionTarget) => void) | null,
  ): void {
  ...
  private appendNotificationContext(
    body: string,
  ...
  private formatDesktopNotificationTitle(request: ExtensionUiRequest): string {
    const cleanedTitle = this.sanitizeDesktopNotificationText(
      request.title?.trim() || '',
    );
  ...
  private formatDesktopNotificationBody(
    request: ExtensionUiRequest,
    contextSuffix: string,
  ): string {
    const cleanedMessage = this.sanitizeDesktopNotificationText(
  ...
  private async resolveDesktopNotificationIconPath(): Promise<
    string | undefined
  > {
    if (this.desktopNotificationIconPath !== undefined)
      return this.desktopNotificationIconPath || undefined;
    try {
  ...
  private shouldThrottleDesktopNotification(
    request: ExtensionUiRequest,
  ): boolean {
  ...
  private buildNotificationActionTarget(
    request: ExtensionUiRequest,
  ...
  private extractNotificationActionTarget(
    notification: DesktopNotificationOptions,
  ): NotificationActionTarget | null {
    const extra = notification.extra as Record<string, unknown> | undefined;
    if (!extra) return null;
    const workspaceId =
      typeof extra.notifyTargetWorkspaceId === 'string'
        ? extra.notifyTargetWorkspaceId.trim()
        : '';
    const tabId =
      typeof extra.notifyTargetTabId === 'string'
        ? extra.notifyTargetTabId.trim()
        : '';
    const sessionPath =
      typeof extra.notifyTargetSessionPath === 'string'
        ? extra.notifyTargetSessionPath.trim()
        : '';
    const workspaceLabel =
      typeof extra.notifyTargetWorkspaceLabel === 'string'
        ? extra.notifyTargetWorkspaceLabel.trim()
        : '';
    const sessionLabel =
      typeof extra.notifyTargetSessionLabel === 'string'
        ? extra.notifyTargetSessionLabel.trim()
        : '';
    if (!workspaceId && !tabId && !sessionPath) return null;
    return {
      workspaceId: workspaceId || undefined,
  ...
  private async ensureDesktopNotificationPermission(
    prompt = false,
  ): Promise<boolean> {
    try {
      const grantedInitially = await isPermissionGranted();
      this.trace(
        `notify:permission-check prompt=${prompt ? 'yes' : 'no'} granted=${grantedInitially ? 'yes' : 'no'}`,
      );
      if (grantedInitially) {
        return true;
      }
      ...
      this.trace(
        `notify:permission-check-failed ${err instanceof Error ? err.message : String(err)}`,
      );
      return false;
    }
  }
  ...
  private ensureDesktopNotificationActionListener(): void {
    ...
    void onAction(async (notification: DesktopNotificationOptions) => {
      ...
      this.trace(
        `notify:action-target workspace=${target.workspaceId ?? '-'} tab=${target.tabId ?? '-'} session=${target.sessionPath ?? '-'}`,
      );
  ...
  private createContainers(): void {
    ...
    this.statusContainer.className =
      'hidden fixed bottom-[92px] left-[278px] right-4 z-40 pointer-events-none';
    ...
    this.widgetAboveContainer.className =
      'hidden fixed bottom-[132px] left-[278px] right-4 z-30';
    ...
    this.widgetBelowContainer.className =
      'hidden fixed bottom-3 left-[278px] right-4 z-30';
  ...
  async handleRequest(request: ExtensionUiRequest): Promise<void> {
    switch (request.method) {
      ...
      case 'set_editor_text':
        ...
        break;
      case 'computer_use':
        void this.handleComputerUse(request);
        break;
    }
  }
  ...
  private async showDesktopNotification(
    ...
    if (!granted) {
      this.trace(
        `notify:skipped permission-missing message=${request.message ?? ''}`,
      );
      return false;
    }
    ...
    const options: DesktopNotificationOptions = {
      ...
      extra: {
        ...
        ...(actionTarget?.workspaceId
          ? { notifyTargetWorkspaceId: actionTarget.workspaceId }
          : {}),
        ...(actionTarget?.tabId
          ? { notifyTargetTabId: actionTarget.tabId }
          : {}),
        ...(actionTarget?.sessionPath
          ? { notifyTargetSessionPath: actionTarget.sessionPath }
          : {}),
        ...(actionTarget?.workspaceLabel
          ? { notifyTargetWorkspaceLabel: actionTarget.workspaceLabel }
          : {}),
        ...(actionTarget?.sessionLabel
          ? { notifyTargetSessionLabel: actionTarget.sessionLabel }
          : {}),
      },
    };
    ...
    try {
      ...
      this.trace(
        `notify:native-dispatched type=${request.notifyType ?? 'info'} sound=${sound ?? 'none'} delivery=unverified`,
      );
      return true;
    } catch (err) {
      this.trace(
        `notify:native-failed ${err instanceof Error ? err.message : String(err)}`,
      );
      try {
        sendNotification({
          title,
        ...
        this.trace(
          `notify:native-dispatched fallback=minimal type=${request.notifyType ?? 'info'}`,
        );
        return true;
      } catch (fallbackErr) {
        this.trace(
          `notify:native-failed-fallback ${fallbackErr instanceof Error ? fallbackErr.message : String(fallbackErr)}`,
        );
        return false;
      }
    }
  ...
  private async showNotification(request: ExtensionUiRequest): Promise<void> {
    ...
    this.trace(
      `notify:request type=${request.notifyType ?? 'info'} ${this.describeNotificationContext(backgrounded)}`,
    );
    if (!backgrounded) {
    ...
    if (!desktopShown) {
      this.trace(
        `notify:desktop-missed message=${request.message ?? request.title ?? ''}`,
      );
    }
    this.trace(
      `notify:dispatch backgrounded=${backgrounded ? 'yes' : 'no'} desktop=${desktopShown ? 'yes' : 'no'}`,
    );
  }
  ...
  private setWidget(request: ExtensionUiRequest): void {
    const container =
      request.widgetPlacement === 'belowEditor'
        ? this.widgetBelowContainer
        : this.widgetAboveContainer;
    if (!container) return;
  ...
  private async setTitle(request: ExtensionUiRequest): Promise<void> {
    ...
    try {
      ...
      this.trace(
        `setTitle:rename-failed ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  private async handleComputerUse(request: ExtensionUiRequest): Promise<void> {
    await this.sendResponse(request.id, {
      error:
        ...
        'Update the CLI to a version that includes MCP-based computer-use support.',
    });
  }
  ...
  private async sendResponse(
    ...
    await rpcBridge.sendExtensionUiResponse({
