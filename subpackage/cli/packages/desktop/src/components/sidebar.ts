...
function fileIconKind(name: string): string {
  ...
  if (['png', 'jpg', 'jpeg', 'webp', 'gif', 'bmp', 'ico', 'heic'].includes(ext))
    return 'image';
...
export class Sidebar {
  ...
  private workspaceRenameDraft: { workspaceId: string; value: string } | null =
    null;
  ...
  private sessionRenameDraft: {
    projectId: string;
    sessionPath: string;
    value: string;
  } | null = null;
  private fileRenameDraft: {
    projectId: string;
    filePath: string;
    value: string;
  } | null = null;
  ...
  private onWorkspaceCreate:
    ...
    | null = null;
  private onWorkspaceEmoji:
    | ((workspaceId: string, emoji: string | null) => void)
    | null = null;
  private onWorkspaceReorder: ((orderedIds: string[]) => void) | null = null;
  private onWorkspaceRename:
  ...
  private onProjectSelect:
    ...
    | null = null;
  private onSessionSelect:
    | ((projectId: string, sessionPath: string, sessionName?: string) => void)
    | null = null;
  private onSessionRename:
    | ((
        projectId: string,
        sessionPath: string,
        currentName: string,
  ...
  private onSessionDelete:
    | ((projectId: string, sessionPath: string) => void)
    | null = null;
  private onSessionFork:
    | ((projectId: string, sessionPath: string, sessionName?: string) => void)
    | null = null;
  private onSessionMarkUnread:
    | ((projectId: string, sessionPath: string, sessionName?: string) => void)
    | null = null;
  private onNewSessionInProject:
    | ((project: { id: string; name: string; path: string }) => void)
    | null = null;
  private onNewFileInProject:
    | ((project: {
        id: string;
        name: string;
        path: string;
    ...
    | null = null;
  private newFilePlacementHint: {
    projectId: string;
    anchorPath: string;
    newPath: string;
    expiresAt: number;
  } | null = null;
  private onFileOpen: ((projectId: string, filePath: string) => void) | null =
    null;
  private onFileDelete: ((projectId: string, filePath: string) => void) | null =
    null;
  ...
  setOnWorkspaceCreate(
    cb: (workspace?: { title?: string; emoji?: string | null }) => void,
  ): void {
    this.onWorkspaceCreate = cb;
  }

  setOnWorkspaceEmoji(
    cb: (workspaceId: string, emoji: string | null) => void,
  ): void {
  ...
  setOnWorkspaceRename(
    cb: (workspaceId: string, nextTitle: string) => void,
  ): void {
  ...
  setWorkspaces(
    ...
    const nextActive =
      activeWorkspaceId &&
      next.some((workspace) => workspace.id === activeWorkspaceId)
        ? activeWorkspaceId
        : (next[0]?.id ?? null);
    ...
    if (
      ...
      !next.some((workspace) => workspace.id === this.activeWorkspaceId)
    ) {
      this.workspaceMenuOpen = false;
    }
    if (
      (this.pendingWorkspaceDragId &&
        !next.some(
          (workspace) => workspace.id === this.pendingWorkspaceDragId,
        )) ||
      (this.draggingWorkspaceId &&
        !next.some((workspace) => workspace.id === this.draggingWorkspaceId)) ||
      (this.workspaceDragOverId &&
        !next.some((workspace) => workspace.id === this.workspaceDragOverId))
    ) {
      this.cancelWorkspacePointerDrag(false);
    }
    if (
      this.emojiPickerWorkspaceId &&
      !next.some((workspace) => workspace.id === this.emojiPickerWorkspaceId)
    ) {
    ...
    if (
      ...
      !next.some(
        (workspace) => workspace.id === this.workspaceRenameDraft?.workspaceId,
      )
    ) {
      this.workspaceRenameDraft = null;
    }
    ...
    if (
      this.workspaceRenameDraft &&
      this.workspaceRenameDraft.workspaceId === this.activeWorkspaceId
    ) {
  ...
  setOnProjectSelect(
    cb: (project: { id: string; name: string; path: string } | null) => void,
  ): void {
    this.onProjectSelect = cb;
  }

  setOnSessionSelect(
    cb: (projectId: string, sessionPath: string, sessionName?: string) => void,
  ): void {
  ...
  setOnSessionDelete(
    cb: (projectId: string, sessionPath: string) => void,
  ): void {
  ...
  setOnSessionMarkUnread(
    cb: (projectId: string, sessionPath: string, sessionName?: string) => void,
  ): void {
    this.onSessionMarkUnread = cb;
  }

  setOnNewSessionInProject(
    cb: (project: { id: string; name: string; path: string }) => void,
  ): void {
    this.onNewSessionInProject = cb;
  }

  setOnNewFileInProject(
    cb: (project: {
      id: string;
      name: string;
      path: string;
  ...
  private readonly onWindowContextMenuPointerDown = (
    event: PointerEvent,
  ): void => {
  ...
  private closeContextMenu(shouldRender = true): void {
    ...
    window.removeEventListener(
      'pointerdown',
      this.onWindowContextMenuPointerDown,
      true,
    );
    window.removeEventListener(
      'mousedown',
      this.onWindowContextMenuMouseDown,
      true,
    );
  ...
  private openContextMenu(e: MouseEvent, target: SidebarContextTarget): void {
    ...
    const maxX = Math.min(
      ...
      Math.floor(bounds.right - menuWidth - padding),
    );
    const minY = Math.max(padding, Math.floor(bounds.top + padding));
    const maxY = Math.min(
      window.innerHeight - menuHeight - padding,
      Math.floor(bounds.bottom - menuHeight - padding),
    );
  ...
  private findSession(
    projectId: string,
    sessionPath: string,
  ): { project: Project; session: SidebarSession } | null {
    const project = this.projects.find((p) => p.id === projectId);
    if (!project) return null;
    const session = project.sessions.find(
      (entry) => normalizePath(entry.path) === normalizePath(sessionPath),
    );
  ...
  setMode(mode: SidebarMode): void {
    ...
    this.closeContextMenu(false);
    if (mode === 'files') {
      const expandedProjects = this.projects.filter(
        (project) => project.expanded,
      );
  ...
  getProjectByPath(
    ...
    const project =
      this.projects.find((entry) => normalizePath(entry.path) === normalized) ??
      null;
    return project
      ? { id: project.id, name: project.name, path: project.path }
      : null;
  ...
  setAttentionSessions(
    entries: Array<{ path: string; message?: string | null }>,
  ): void {
  ...
  setTransientSessionDraft(
    ...
    const same =
      ...
      normalizePath(this.transientSessionDraft?.path) ===
        normalizePath(next?.path) &&
    ...
    const sessions = this.sortedSessions(
      project.sessions.filter((session) => {
        ...
        return (
          ...
          !this.suppressedSessionPaths.has(normalizedPath)
        );
      }),
    );
  ...
  upsertSession(
    ...
    const nextName =
      session.name?.trim() || existing?.name || 'Untitled session';
    if (existing) {
  ...
  setNewFilePlacementHint(
    ...
    if (!normalizedProjectId || !normalizedNewPath || !normalizedAnchorPath)
      return;
  ...
  private async commitFileRename(
    projectId: string,
    node: FileNode,
  ): Promise<void> {
    if (node.isDirectory) return;
    ...
    if (
      ...
      draft.filePath !== normalizePath(node.path)
    )
      return;
  ...
  private commitSessionRename(project: Project, session: SidebarSession): void {
    ...
    if (
      ...
      draft.sessionPath !== normalizePath(session.path)
    )
      return;
  ...
  private async deleteSession(
    ...
    try {
      ...
      project.sessions = project.sessions.filter(
        (entry) => normalizePath(entry.path) !== normalizePath(session.path),
      );
  ...
  private handleSessionContextMenu(
    e: MouseEvent,
    project: Project,
    session: SidebarSession,
  ): void {
  ...
  private runSessionContextAction(
    action: 'rename' | 'delete' | 'fork' | 'markUnread' | 'togglePin',
  ): void {
  ...
  private runFileContextAction(action: 'newFile' | 'rename' | 'delete'): void {
    ...
    if (action === 'newFile') {
      const project =
        this.projects.find((entry) => entry.id === target.projectId) ?? null;
      if (!project) {
        this.render();
        return;
      }
      const baseDir = target.isDirectory
        ? target.filePath
        : parentFsPath(target.filePath);
  ...
  private runWorkspaceContextAction(
    ...
    this.closeContextMenu();
    const workspace =
      this.workspaces.find((entry) => entry.id === target.workspaceId) ?? null;
    if (!workspace) {
      this.render();
      return;
    }
  ...
  private isWorkspaceHydrationCurrent(hydrationToken?: number): boolean {
    return (
      typeof hydrationToken !== 'number' ||
      hydrationToken === this.workspaceHydrationToken
    );
  }
  ...
  private async hydrateProjects(
    ...
    await Promise.all(
      this.projects.map((project) =>
        this.refreshProjectPathStatus(project.id, hydrationToken),
      ),
    );
    if (!this.isWorkspaceHydrationCurrent(hydrationToken)) return;
  ...
  private async refreshProjectPathStatus(
    projectId: string,
  ...
  private async relinkProject(projectId: string): Promise<void> {
    ...
    try {
      ...
      const duplicate = this.projects.find(
        (p) => p.id !== projectId && normalizePath(p.path) === normalized,
      );
      if (duplicate) {
  ...
  private async loadSessionsForProject(
    projectId: string,
    options?: { silent?: boolean; hydrationToken?: number },
  ): Promise<void> {
    ...
    const run = (async () => {
      ...
      if (
        silent &&
        project.sessionsLoaded &&
        now - project.lastSessionsLoadedAt < 2200
      ) {
        return;
      }
      ...
      try {
        ...
        const byProject = sessions.filter((s) => {
          ...
          return (
            sessionPath.includes(projectPath) ||
            sessionPath.includes(normalizePath(project.name))
          );
        });

        const visibleProjectSessions = byProject.filter(
          (session) =>
            !this.suppressedSessionPaths.has(normalizePath(session.path)),
        );
        const scannedSessions = visibleProjectSessions.slice(0, 40).map(
          (s) =>
            ({
              id: s.id,
        ...
        const scannedByPath = new Map<string, SidebarSession>(
          scannedSessions.map(
            (entry) => [normalizePath(entry.path), entry] as const,
          ),
        );
        ...
        const preservedOptimistic = project.sessions.filter((entry) => {
          ...
          const normalizedPath = normalizePath(entry.path);
          if (!normalizedPath || scannedByPath.has(normalizedPath))
            return false;
    ...
    try {
      ...
      if (
        this.sessionReloadQueued.has(projectId) &&
        this.isWorkspaceHydrationCurrent(hydrationToken)
      ) {
  ...
  private async resolveGitDirPointer(
    pointerPath: string,
    ...
    try {
      ...
      const targetPath = isAbsolutePath(rawTarget)
        ? rawTarget
        : await resolve(parentReadPath, rawTarget);
  ...
  private async mapDirectoryEntries(
    readBasePath: string,
    ...
    const nodes = await Promise.all(
      entries.map(async (entry) => {
        ...
        if (!isDirectory && entry.name === '.git' && entry.isFile) {
          const gitTarget = await this.resolveGitDirPointer(
            actualPath,
            readBasePath,
          );
          if (gitTarget) {
  ...
  private async ensureFileTreeForProject(
    projectId: string,
    forceReload = false,
  ): Promise<void> {
    const project = this.projects.find((p) => p.id === projectId);
    ...
    try {
      const nodes = await this.mapDirectoryEntries(
        project.path,
        project.path,
        0,
      );
  ...
  private async toggleDirectory(
    ...
    try {
      node.children = await this.mapDirectoryEntries(
        node.path,
        node.displayPath,
        node.depth + 1,
      );
  ...
  private removeProject(projectId: string): void {
    ...
    this.fileTreeErrors.delete(projectId);
    this.openProjectMenuId =
      this.openProjectMenuId === projectId ? null : this.openProjectMenuId;
  ...
  private persistPinnedSessions(): void {
    try {
      ...
      localStorage.setItem(
        this.sessionPinsStorageKey(),
        JSON.stringify([...this.pinnedSessionPaths]),
      );
    } catch {
      // ignore
    }
  ...
  private loadPersistedProjects(): void {
    try {
      ...
      if (
        !raw &&
        this.storageKey === workspaceStorageKey('workspace_default')
      ) {
      ...
      this.projects = data
        ...
        .map((p, idx) => ({
          ...
          name: p.name,
          color:
            typeof p.color === 'string' && p.color.trim().length > 0
              ? p.color
              : stringToColor(p.name || pathBaseName(p.path)),
          emoji: normalizeProjectEmoji(p.emoji),
          expanded: idx === 0,
          sessions: [],
          loadingSessions: false,
  ...
  private filteredProjects(includeQuery = true): Project[] {
    ...
    if (includeQuery && q) {
      list = list.filter((project) => {
        if (`${project.name} ${project.path}`.toLowerCase().includes(q))
          return true;
        return project.sessions.some((session) =>
          `${session.name} ${session.path}`.toLowerCase().includes(q),
        );
      });
    }
  ...
  private sortedSessions(sessions: SidebarSession[]): SidebarSession[] {
    ...
    sorted.sort((a, b) => {
      const pinDelta =
        Number(this.isSessionPinned(b.path)) -
        Number(this.isSessionPinned(a.path));
  ...
  private visibleSessions(project: Project): SidebarSession[] {
    ...
    const transientDraft =
      this.transientSessionDraft &&
      this.transientSessionDraft.projectId === project.id
        ? ({
    ...
    if (transientDraft) {
      ...
      sessions = [
        ...
        ...sessions.filter(
          (session) =>
            normalizePath(session.path) !== transientPath || !transientPath,
        ),
      ];
    }
    if (q) {
    ...
    const pinned = sessions.filter((session) =>
      this.isSessionPinned(session.path),
    );
    const recent = sessions.filter(
      (session) =>
        session.transient ||
        (session.modifiedAt || session.createdAt || 0) >= cutoff,
    );
    const merged = [
      ...pinned,
      ...recent.filter((session) => !this.isSessionPinned(session.path)),
    ];
    if (merged.length > 0) return merged;
    return sessions.slice(0, 5);
  }

  private chronologicalSessions(
    projects: Project[],
  ): Array<{ project: Project; session: SidebarSession }> {
    ...
    list.sort((a, b) => {
      const pinDelta =
        Number(this.isSessionPinned(b.session.path)) -
        Number(this.isSessionPinned(a.session.path));
      if (pinDelta !== 0) return pinDelta;
      const aTs =
        this.sessionSortBy === 'created'
  ...
  private nodeMatchesQuery(node: FileNode, query: string): boolean {
    ...
    if (!node.children) {
      return (
        this.fileKind === 'files' || (this.fileKind === 'dirs' && textMatches)
      );
    }
  ...
  private primeFileDragPayload(
    node: FileNode,
    fileRenameActive: boolean,
  ...
  private handleFileDragStart(
    ...
    try {
      ...
      transfer.setData(
        'application/x-pi-file-paths-json',
  ...
  private renderFileNode(
    ...
    const activeFile =
      !node.isDirectory && normalizePath(node.path) === this.activeFilePath;
  ...
  private openProjectEmojiPicker(projectId: string, event: MouseEvent): void {
    ...
    requestAnimationFrame(() => {
      const input = this.container.querySelector<HTMLInputElement>(
        `.project-emoji-search[data-project-id="${projectId}"]`,
      );
      input?.focus();
      input?.select();
    });
  ...
  private readonly onProjectDragPointerMove = (event: PointerEvent): void => {
    ...
    if (!this.draggingProjectId) {
      if (
        Math.abs(event.clientY - this.projectDragStartY) <
    ...
    const hoveredProjectId = this.resolveProjectIdFromPoint(
      event.clientX,
      event.clientY,
      draggedProjectId,
    );
    if (!hoveredProjectId || hoveredProjectId === this.projectDragOverId)
      return;
  ...
  private beginProjectPointerDrag(
    event: PointerEvent,
    projectId: string,
  ): void {
    ...
    window.addEventListener(
      'pointercancel',
      this.onProjectDragPointerEnd,
      true,
    );
  }

  private isCompatibleProjectDropTarget(
    draggedProjectId: string,
    targetProjectId: string,
  ): boolean {
    const dragged =
      this.projects.find((project) => project.id === draggedProjectId) ?? null;
    const target =
      this.projects.find((project) => project.id === targetProjectId) ?? null;
    return Boolean(dragged && target);
  }

  private resolveProjectIdFromPoint(
    clientX: number,
    clientY: number,
    draggedProjectId: string,
  ): string | null {
    ...
    const directRow = target?.closest<HTMLElement>(
      '.sidebar-project-row[data-project-id]',
    );
    if (directRow?.dataset.projectId) {
      return this.isCompatibleProjectDropTarget(
        draggedProjectId,
    ...
    if (clientY > listRect.bottom) {
      return (
        compatibleRows[compatibleRows.length - 1]?.dataset.projectId ?? null
      );
    }
  ...
  private applyProjectReorderByIds(
    draggedProjectId: string,
    targetProjectId: string,
  ): void {
    const fromIndex = this.projects.findIndex(
      (project) => project.id === draggedProjectId,
    );
    const toIndex = this.projects.findIndex(
  ...
  private finishProjectPointerDrag(commitReorder: boolean): void {
    ...
    const hadDragState = Boolean(
      this.pendingProjectDragId ||
        this.draggingProjectId ||
        this.projectDragOverId,
    );
  ...
  private focusWorkspaceRenameInput(workspaceId: string): void {
    requestAnimationFrame(() => {
      const input = this.container.querySelector<HTMLInputElement>(
        `.sidebar-workspace-title-input[data-workspace-id="${workspaceId}"]`,
      );
      input?.focus();
      input?.select();
    });
  ...
  private startWorkspaceRename(workspaceId: string): void {
    const workspace =
      this.workspaces.find((entry) => entry.id === workspaceId) ?? null;
    if (!workspace) return;
    this.closeWorkspaceCreateDialog(false, false);
  ...
  private commitWorkspaceRename(): void {
    ...
    if (!draft) return;
    const workspace =
      this.workspaces.find((entry) => entry.id === draft.workspaceId) ?? null;
  ...
  private readonly onWorkspaceCreateDialogFocusIn = (
    event: FocusEvent,
  ): void => {
  ...
  private readonly onWorkspaceCreateDialogKeyDown = (
    event: KeyboardEvent,
  ): void => {
    if (!this.workspaceCreateDialogOpen) return;
    if (event.key === 'Escape') {
  ...
  private getWorkspaceCreateDialogElement(): HTMLElement | null {
    const host =
      ...
      document.body.contains(this.workspaceCreatePortalHost)
        ? this.workspaceCreatePortalHost
        : this.container;
  ...
  private focusWorkspaceCreateDialogPrimaryInput(selectText = false): void {
    ...
    const firstFocusable =
      this.getWorkspaceCreateDialogFocusableElements()[0] ?? null;
  ...
  private enableWorkspaceCreateDialogFocusTrap(): void {
    ...
    window.addEventListener(
      'keydown',
      this.onWorkspaceCreateDialogKeyDown,
      true,
    );
    document.addEventListener(
      'focusin',
      this.onWorkspaceCreateDialogFocusIn,
      true,
    );
  ...
  private openWorkspaceCreateDialog(): void {
    this.workspaceRenameDraft = null;
    this.workspaceCreateDialogRestoreFocus =
      document.activeElement instanceof HTMLElement
        ? document.activeElement
        : null;
    ...
    this.render();
    requestAnimationFrame(() =>
      this.focusWorkspaceCreateDialogPrimaryInput(true),
    );
  }

  private closeWorkspaceCreateDialog(
    shouldRender = true,
    restoreFocus = true,
  ): void {
  ...
  private createWorkspaceFromDialog(): void {
    const title =
      this.workspaceCreateName.trim() || this.nextWorkspaceDraftName();
  ...
  private readonly onWorkspaceDragPointerMove = (event: PointerEvent): void => {
    ...
    if (!this.draggingWorkspaceId) {
      const distance = Math.hypot(
        event.clientX - this.workspaceDragStartX,
        event.clientY - this.workspaceDragStartY,
      );
    ...
    const hoveredWorkspaceId = this.resolveWorkspaceIdFromPoint(
      event.clientX,
      event.clientY,
      draggedWorkspaceId,
    );
    if (!hoveredWorkspaceId || hoveredWorkspaceId === this.workspaceDragOverId)
      return;
  ...
  private beginWorkspacePointerDrag(
    event: PointerEvent,
    workspaceId: string,
  ): void {
    if (event.button !== 0) return;
    ...
    window.addEventListener(
      'pointermove',
      this.onWorkspaceDragPointerMove,
      true,
    );
    window.addEventListener('pointerup', this.onWorkspaceDragPointerEnd, true);
    window.addEventListener(
      'pointercancel',
      this.onWorkspaceDragPointerEnd,
      true,
    );
  }

  private isCompatibleWorkspaceDropTarget(
    ...
    const target =
      this.workspaces.find((workspace) => workspace.id === targetWorkspaceId) ??
      null;
    return Boolean(dragged && target);
  }

  private resolveWorkspaceIdFromPoint(
    clientX: number,
    clientY: number,
    draggedWorkspaceId: string,
  ): string | null {
    ...
    const directPill = target?.closest<HTMLElement>(
      '.sidebar-workspace-pill[data-workspace-id]',
    );
    if (directPill?.dataset.workspaceId) {
      return this.isCompatibleWorkspaceDropTarget(
        draggedWorkspaceId,
    ...
    if (clientX > listRect.right) {
      return (
        compatiblePills[compatiblePills.length - 1]?.dataset.workspaceId ?? null
      );
    }
  ...
  private applyWorkspaceReorderByIds(
    draggedWorkspaceId: string,
    targetWorkspaceId: string,
  ): void {
    const fromIndex = this.workspaces.findIndex(
      (workspace) => workspace.id === draggedWorkspaceId,
    );
    const toIndex = this.workspaces.findIndex(
    ...
    if (
      ...
      toIndex === -1 ||
      fromIndex === toIndex ||
      !this.isCompatibleWorkspaceDropTarget(
        draggedWorkspaceId,
        targetWorkspaceId,
      )
    ) {
  ...
  private shouldHandleWorkspaceSwipe(target: HTMLElement | null): boolean {
    ...
    if (
      target.closest(
        '.sidebar-context-menu, .workspace-emoji-picker, .sidebar-space-dialog, .sidebar-mode-filter-menu',
      )
    )
      return false;
    if (
      target.closest(
        '.sidebar-window-row, .sidebar-topbar, .sidebar-mode-row, .sidebar-footer, .sidebar-workspace-dock',
      )
    )
      return false;
  ...
  private switchWorkspaceByOffset(offset: 1 | -1): void {
    ...
    const activeIndex = this.workspaces.findIndex(
      (workspace) => workspace.id === this.activeWorkspaceId,
    );
  ...
  private handleSidebarWheel(event: WheelEvent): void {
    ...
    if (
      Math.abs(this.workspaceSwipeAccumulatorX) < WORKSPACE_SWIPE_THRESHOLD_PX
    ) {
      return;
    }
  ...
  private async invokeWindowControl(
  ...
  private renderWorkspaceHeader(): TemplateResult | typeof nothing {
    ...
    const isRenaming =
      this.workspaceRenameDraft?.workspaceId === activeWorkspace.id;
    return html`
			<div
              ...
              ? html`
                    ...
                    this.render();
								...
								<div class="sidebar-space-emoji-grid">
									${
                    filteredEmojis.length > 0
                      ? filteredEmojis.slice(0, 120).map(
                          (entry) => html`
											<button
  ...
  private renderProjectEmojiPicker(): TemplateResult | typeof nothing {
    ...
    const project =
      this.projects.find(
        (entry) => entry.id === this.projectEmojiPickerProjectId,
      ) ?? null;
  ...
  private ensureWorkspaceCreateDialogPortalHost(): HTMLElement | null {
    ...
    if (
      ...
      return this.workspaceCreatePortalHost;
  ...
  private ensureProjectEmojiPortalHost(): HTMLElement | null {
    if (typeof document === 'undefined') return null;
    if (
      this.projectEmojiPortalHost &&
      document.body.contains(this.projectEmojiPortalHost)
    )
      return this.projectEmojiPortalHost;
    const host = document.createElement('div');
  ...
  private renderChronologicalProjectsMode(projects: Project[]): TemplateResult {
    ...
    return html`
          ...
          const runningSession = this.runningSessionPaths.has(
            normalizedSessionPath,
          );
          const attentionMessage =
            this.attentionSessionMessages.get(normalizedSessionPath) ?? null;
          const pinnedSession = this.isSessionPinned(session.path);
          const prevPinned =
            index > 0
              ? this.isSessionPinned(rows[index - 1]?.session.path ?? '')
              : false;
  ...
  private renderProjectsMode(): TemplateResult {
    ...
    return html`
          ...
          const showBlockingSessionLoad =
            project.loadingSessions && sessions.length === 0;
          const showInlineSessionRefresh =
            project.loadingSessions && sessions.length > 0;
          const dragOver =
            project.id === this.projectDragOverId &&
          ...
          return html`
                      ...
                      this.toggleProject(project.id);
									...
									<button
										...
										@click=${() => {
                      if (this.shouldSuppressProjectMainClick(project.id))
                        return;
                      ...
                      showBlockingSessionLoad
                        ...
                        : sessions.length === 0
                          ...
                          : sessions.map((session, index) => {
                              const normalizedSessionPath = normalizePath(
                                session.path,
                              );
                              const activeSession = normalizedSessionPath
                                ? normalizedSessionPath ===
                              ...
                              const runningSession =
                                this.runningSessionPaths.has(
                                  normalizedSessionPath,
                                );
                              const attentionMessage =
                                this.attentionSessionMessages.get(
                                  normalizedSessionPath,
                                ) ?? null;
                              const pinnedSession = this.isSessionPinned(
                                session.path,
                              );
                              const prevPinned =
                                index > 0
                              ...
                              const showPinnedDivider =
                                prevPinned && !pinnedSession;
                              const sessionRenameActive =
                                Boolean(this.sessionRenameDraft) &&
                                this.sessionRenameDraft?.projectId ===
                                  project.id &&
                                this.sessionRenameDraft?.sessionPath ===
  ...
  private renderFilesMode(): TemplateResult {
    ...
    return html`
          ...
          const showInlineSessionRefresh =
            project.loadingSessions && project.sessions.length > 0;
  ...
  render(): void {
    ...
    const template = html`
          ...
          if (
            this.openProjectMenuId &&
            !target?.closest('.sidebar-project-menu') &&
          ...
          if (
            ...
            !target?.closest('.sidebar-mode-filter-menu') &&
            !target?.closest('.sidebar-mode-filter-btn')
          ) {
            this.modeFilterMenuOpen = false;
            changed = true;
          }
          ...
          if (
            (this.pendingProjectDragId || this.draggingProjectId) &&
            !target?.closest('.sidebar-project-list')
          ) {
            this.cancelProjectPointerDrag(false);
            changed = true;
          }
          ...
          if (
            (this.pendingWorkspaceDragId || this.draggingWorkspaceId) &&
            !target?.closest('.sidebar-workspace-dock')
          ) {
            this.cancelWorkspacePointerDrag(false);
            changed = true;
          }
