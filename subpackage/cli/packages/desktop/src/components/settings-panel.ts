...
export class SettingsPanel {
  ...
  setOnDesktopStatusChange(
    callback: (status: DesktopUpdateStatus | null) => void,
  ): void {
  ...
  setOnNavigationStateChange(
    callback: ((state: SettingsNavigationState) => void) | null,
  ): void {
  ...
  private normalizeActiveSection(
    runtimeControlsEnabled = this.isRuntimeControlsEnabled(),
  ): SettingsSectionId {
    const navItems = this.getSettingsNavItems();
    const requested =
      navItems.find((item) => item.id === this.activeSection) ?? navItems[0];
  ...
  private getProfile(theme: ThemeVariant): DesktopAppearanceProfile {
    const candidate =
      theme === 'light'
        ? this.appearanceProfiles?.light
        : this.appearanceProfiles?.dark;
  ...
  private applyColorDraftPreviewToRoot(theme: ThemeVariant): void {
    ...
    if (draft.accent) {
      ...
      root.style.setProperty(
        '--color-accent-soft',
        `color-mix(in srgb, ${draft.accent} 20%, transparent)`,
      );
    }
    ...
    if (draft.background) {
      ...
      root.style.setProperty(
        '--color-bg-elevated',
        `color-mix(in srgb, ${draft.background} 94%, ${neutralLift} 6%)`,
      );
      root.style.setProperty(
        '--color-bg-muted',
        `color-mix(in srgb, ${draft.background} 89%, ${neutralLift} 11%)`,
      );
      root.style.setProperty(
        '--color-bg-soft',
        `color-mix(in srgb, ${draft.background} 84%, ${neutralLift} 16%)`,
      );
      root.style.setProperty(
        '--color-bg-sidebar',
        `color-mix(in srgb, ${draft.background} 91%, ${neutralLift} 9%)`,
      );
      root.style.setProperty(
        '--color-bg-workspace-chrome',
        `color-mix(in srgb, ${draft.background} 92%, ${neutralLift} 8%)`,
      );
      root.style.setProperty(
        '--color-bg-workspace-chrome-soft',
        `color-mix(in srgb, ${draft.background} 86%, ${neutralLift} 14%)`,
      );
    }
    ...
    if (draft.foreground) {
      ...
      root.style.setProperty(
        ...
        `color-mix(in srgb, ${draft.foreground} 68%, ${effectiveBackground} 32%)`,
      );
      root.style.setProperty(
        '--color-text-tertiary',
        `color-mix(in srgb, ${draft.foreground} 52%, ${effectiveBackground} 48%)`,
      );
      root.style.setProperty(
        '--color-border-default',
        `color-mix(in srgb, ${draft.foreground} 12%, transparent)`,
      );
      root.style.setProperty(
        '--border',
        `color-mix(in srgb, var(--color-border-default) ${40 + Math.round((this.getProfile(theme).contrast / 100) * 60)}%, transparent)`,
      );
    }
  }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          const variant = this.extractThemeVariant(
            parsed,
            id,
            preview.background,
          );
  ...
  private extractThemePreview(theme: Record<string, unknown>): {
    accent: string;
    background: string;
    foreground: string;
  } {
  ...
  private extractThemeVariant(
    ...
    const directBackground =
      ...
      null;
    if (directBackground) {
      const byDirectBackground =
        this.inferVariantFromBackground(directBackground);
  ...
  private extractThemeDesktopDefaults(theme: Record<string, unknown>): {
    ...
    const contrast =
      typeof m.contrast === 'number' && Number.isFinite(m.contrast)
        ? Math.max(0, Math.min(100, Math.round(m.contrast)))
        : null;
    const fonts = m.fonts;
    const fontsObj =
      fonts && typeof fonts === 'object' && !Array.isArray(fonts)
        ? (fonts as Record<string, unknown>)
        : null;
    const uiFont =
      fontsObj && typeof fontsObj.ui === 'string' && fontsObj.ui.trim()
        ? fontsObj.ui.trim()
        : null;
    const codeFont =
      fontsObj && typeof fontsObj.code === 'string' && fontsObj.code.trim()
        ? fontsObj.code.trim()
        : null;
    const translucentSidebar =
      typeof m.opaqueWindows === 'boolean' ? !m.opaqueWindows : null;
  ...
  private lookupThemeOption(themeName: unknown): ThemeOption | null {
    ...
    return (
      this.availableThemes.find((entry) => {
        const id = typeof entry?.id === 'string' ? entry.id.toLowerCase() : '';
        const label =
          typeof entry?.label === 'string' ? entry.label.toLowerCase() : '';
  ...
  private themeBehaviorDefaults(
    ...
    const profileDefaults =
      theme === 'light'
        ? DEFAULT_APPEARANCE_PROFILES.light
        : DEFAULT_APPEARANCE_PROFILES.dark;
    return {
      ...
      translucentSidebar:
        selected?.translucentSidebar ?? profileDefaults.translucentSidebar,
    };
  }
  ...
  private baseThemePreview(theme: ThemeVariant): {
    accent: string;
    background: string;
    foreground: string;
  } {
  ...
  private profilePreview(theme: ThemeVariant): {
    accent: string;
    background: string;
    foreground: string;
  } {
  ...
  private setProfileTranslucentSidebar(
    theme: ThemeVariant,
    value: boolean,
  ): void {
    const profile = this.getProfile(theme);
  ...
  private async saveCreateThemeFromDialog(): Promise<void> {
    ...
    this.render();
    try {
      await this.persistThemeFromProfile(
        this.createThemeDialogTheme,
        normalizedName,
      );
      this.createThemeDialogOpen = false;
      this.createThemeDialogSaving = false;
  ...
  private async persistThemeFromProfile(
    theme: ThemeVariant,
    ...
    try {
      ...
      this.piPathActionMessage =
        err instanceof Error
          ? err.message
          : 'Failed to save CLI binary override.';
    }
    this.render();
  }
  ...
  private async loadState(): Promise<void> {
    const runtimeReady =
      Boolean(this.runtimeProjectPath) && rpcBridge.isConnected;
    if (runtimeReady) {
      try {
        const sessionState = await rpcBridge.getState();
        this.state.autoCompactionEnabled = Boolean(
          sessionState.autoCompactionEnabled,
        );
        this.state.steeringMode =
  ...
  private async openDesktopUpdateNow(): Promise<void> {
    ...
    this.desktopActionMessage = this.desktopStatus.assetUrl
      ? 'Opening desktop installer…'
    ...
    try {
      ...
      this.desktopActionMessage =
        err instanceof Error ? err.message : 'Failed to open desktop update.';
    } finally {
    ...
    try {
      ...
      this.compatibilityReport = {
        ...
        checks: Array.isArray(raw?.checks)
          ? raw.checks.filter((entry) => typeof entry === 'string')
          : [],
        error:
  ...
  private parseScopedModelOptions(
    rawModels: Array<Record<string, unknown>>,
  ): ScopedModelOption[] {
    const byId = new Map<string, ScopedModelOption>();
    for (const raw of rawModels) {
      const provider = this.pickStringPath(raw, [
        'provider',
        'target.provider',
        'model.provider',
      ]);
      const id = this.pickStringPath(raw, [
        'id',
        'modelId',
        'model_id',
        'model',
      ...
      const name =
        this.pickStringPath(raw, ['name', 'label', 'target.name']) ?? id;
      byId.set(key, {
        fullId,
        provider,
        id,
  ...
  private async readPiGlobalSettingsDoc(): Promise<{
    path: string;
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error(
        `Expected object in ${path}, but found ${Array.isArray(parsed) ? 'array' : typeof parsed}`,
      );
    }
  ...
  private setScopedModelsSelection(
    hasFilter: boolean,
    enabledIds: string[],
  ): void {
  ...
  private isScopedModelEnabled(fullId: string): boolean {
    return (
      !this.scopedModelsHasFilter ||
      this.scopedModelsEnabledIds.includes(fullId)
    );
  }
  ...
  private toggleScopedProvider(provider: string): void {
    ...
    const allIds = this.allScopedModelIds();
    const providerFullyEnabled = providerIds.every((id) =>
      this.isScopedModelEnabled(id),
    );
    if (!this.scopedModelsHasFilter) {
    ...
    try {
      ...
      const patterns = Array.isArray(doc.enabledModels)
        ? doc.enabledModels
            ...
            .filter(Boolean)
        : undefined;
      const resolved = this.resolveScopedModelIdsFromPatterns(
        patterns,
        this.scopedModels,
      );
      ...
      if (resolved.unknownPatterns.length > 0) {
        this.scopedModelsMessage =
          'Some saved model patterns are not currently available and were skipped in this view.';
      }
    } catch (err) {
      this.scopedModels = [];
      this.scopedModelsHasFilter = false;
  ...
  private async saveScopedModels(): Promise<void> {
    ...
    try {
      ...
      const allIds = this.allScopedModelIds();
      const enabledIds = this.scopedModelsHasFilter
        ? this.scopedModelsEnabledIds
        : allIds;
      const deduped = enabledIds.filter(
        (id, index) => enabledIds.indexOf(id) === index && allIds.includes(id),
      );
      const shouldClearFilter =
        !this.scopedModelsHasFilter || deduped.length >= allIds.length;
      ...
      this.scopedModelsSavedSnapshot = this.scopedModelsSnapshot();
      this.scopedModelsMessage =
        'Scoped models saved. Run /reload to apply this scope to the current runtime.';
    } catch (err) {
      this.scopedModelsError = err instanceof Error ? err.message : String(err);
      this.scopedModelsMessage = '';
    } finally {
  ...
  private renderScopedModelsSection(): TemplateResult {
    ...
    const enabledCount = this.scopedModelsHasFilter
      ? this.scopedModelsEnabledIds.length
      : totalModels;
    const query = this.scopedModelsSearch.trim().toLowerCase();
    const visibleModels = query
      ? this.scopedModels.filter((model) =>
    ...
    const providers = [...grouped.entries()].sort((a, b) =>
          ...
          !this.scopedModelsLoading && !this.scopedModelsError
            ? providers.length === 0
              ...
              : html`
                  ...
                  const providerEnabledCount = models.filter((model) =>
                    this.isScopedModelEnabled(model.fullId),
                  ).length;
                  const providerAllEnabled =
                    providerEnabledCount === models.length;
                  return html`
  ...
  private renderThemeProfileCard(theme: ThemeVariant): TemplateResult {
    ...
    const selectedValue =
      selectedOption && selectedOption.variant === theme
        ? selectedOption.id
  ...
  private renderCreateThemeDialog(): TemplateResult | typeof nothing {
    ...
    const variantLabel =
      this.createThemeDialogTheme === 'dark' ? 'Dark' : 'Light';
    return html`
  ...
  private renderBasicSettingsShell(
    runtimeMessage: string,
    options: { showAddProject: boolean; warning?: string } = {
              ...
              this.desktopLoading
                ...
                : html`
                  ...
                  this.desktopStatus
					...
					<div class="settings-section">
						<div class="settings-section-title">CLI binary path</div>
              ...
              runtimeControlsEnabled
                ? html`
								<details class="settings-advanced">
									...
									<div class="settings-desc">Discovery: <code>${rpcBridge.discoveryInfo || 'unknown'}</code></div>
									<div class="settings-actions">
  ...
  render(): void {
    ...
    const activeItem =
      navigation.items.find((item) => item.id === activeSection) ??
      navigation.items[0];
    this.emitNavigationState(runtimeControlsEnabled);
      ...
      try {
        this.renderBasicSettingsShell(
          'Advanced runtime settings are temporarily unavailable. You can still use appearance settings.',
          {
            showAddProject: false,
            warning:
              'Open another session or reopen settings once runtime stabilizes.',
          },
        );
      } catch {
        render(
