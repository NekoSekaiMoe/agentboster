...
export const EMOJI_CATALOG: EmojiEntry[] = EMOJI_CATALOG_RAW.map(
  ([emoji, name, keywords]) => ({
    emoji,
    name,
    keywords,
...
export class WorkspaceTabs {
  ...
  setTabs(tabs: WorkspaceTabItem[], activeId: string | null): void {
    ...
    if (
      ...
      !this.tabs.some((tab) => tab.id === this.contextWorkspaceId)
    ) {
      this.contextWorkspaceId = null;
    }
    if (
      this.emojiPickerWorkspaceId &&
      !this.tabs.some((tab) => tab.id === this.emojiPickerWorkspaceId)
    ) {
      this.emojiPickerWorkspaceId = null;
      this.emojiSearchQuery = '';
    }
    if (
      this.renamingWorkspaceId &&
      !this.tabs.some((tab) => tab.id === this.renamingWorkspaceId)
    ) {
    ...
    if (
      ...
      !this.tabs.some((tab) => tab.id === this.draggingWorkspaceId)
    ) {
      this.clearDragState();
    }
    this.render();
  ...
  private startRenameWorkspace(workspaceId: string): void {
    ...
    requestAnimationFrame(() => {
      const input = this.container.querySelector<HTMLInputElement>(
        `.workspace-tab-inline-input[data-workspace-id="${workspaceId}"]`,
      );
      input?.focus();
      input?.select();
    });
  ...
  private beginWorkspacePointerInteraction(
    workspaceId: string,
    event: PointerEvent,
  ): void {
    ...
    const tabElement =
      ...
      this.container.querySelector<HTMLElement>(
        `.workspace-tab[data-workspace-id="${workspaceId}"]`,
      );
  ...
  private updatePreviewOrderFromPointer(): void {
    ...
    const sameGroupTabs = previewTabs.filter(
      (tab) => Boolean(tab.pinned) === Boolean(draggedTab.pinned),
    );
    const otherTabs = sameGroupTabs.filter((tab) => tab.id !== draggedId);
    let insertIndex = 0;
    for (const tab of otherTabs) {
      const element = this.container.querySelector<HTMLElement>(
        `.workspace-tab[data-workspace-id="${tab.id}"]`,
      );
      const rect = element?.getBoundingClientRect();
      if (!rect) continue;
    ...
    const pinnedGroup = draggedTab.pinned
      ? reorderedGroup
      : previewTabs.filter((tab) => tab.pinned);
    const unpinnedGroup = draggedTab.pinned
      ? previewTabs.filter((tab) => !tab.pinned)
      : reorderedGroup;
  ...
  private finishWorkspacePointerInteraction(): void {
    ...
    const orderChanged = Boolean(
      draggingId && previewTabs && finalIds !== initialIds,
    );
  ...
  private updateLayoutMetrics(): void {
    ...
    const left = this.container.querySelector<HTMLElement>(
      '.workspace-tabs-left',
    );
    if (!root || !left) return;
    ...
    const gap =
      Number.parseFloat(rootStyles.columnGap || rootStyles.gap || '0') || 0;
    const baseStart = Math.max(
      0,
      Math.round(leftRect.left - rootRect.left + leftRect.width + gap),
    );
    this.container.style.setProperty(
      '--workspace-topbar-main-start-base',
      `${baseStart}px`,
    );
  }
  ...
  render(): void {
    ...
    const emojiPickerWorkspace = this.getWorkspaceById(
      this.emojiPickerWorkspaceId,
    );
    ...
    const dragGhostLeft = this.draggingWorkspaceId
      ? this.getDragGhostLeft()
      : 0;
    const template = html`
