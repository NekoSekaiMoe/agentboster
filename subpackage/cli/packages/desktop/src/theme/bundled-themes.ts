import {
  buildPiThemeDocument,
  isThemeDocumentSchemaCompatible,
} from './pi-theme-document.js';
...
const BUNDLED_THEME_ID_SET = new Set<string>(
  BUNDLED_THEME_SPECS.flatMap((spec) => {
    ...
    if (spec.legacyFileName)
      ids.push(spec.legacyFileName.replace(/\.json$/i, '').toLowerCase());
    return ids;
  }),
);
...
async function hasBundledThemeRepairCandidate(
  ...
  for (const spec of BUNDLED_THEME_SPECS) {
    ...
    const legacyPath = spec.legacyFileName
      ? joinFsPath(themesRoot, spec.legacyFileName)
      : null;
...
async function hasBundledThemesMarker(
  ...
  for (const markerName of markerNames) {
    if (await existsSafe(exists, joinFsPath(themesRoot, markerName)))
      return true;
  }
  return false;
}

async function installBundledThemes(options: {
  respectMarker: boolean;
}): Promise<BundledThemeInstallResult> {
  const { exists, mkdir, writeTextFile, readTextFile, rename, remove } =
  ...
  if (
    options.respectMarker &&
    (await hasBundledThemesMarker(themesRoot, exists))
  ) {
    const hasRepairCandidate = await hasBundledThemeRepairCandidate(
      themesRoot,
      exists,
      readTextFile,
    );
