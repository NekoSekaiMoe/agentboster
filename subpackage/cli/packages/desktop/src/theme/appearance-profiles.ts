...
export const DESKTOP_APPEARANCE_PROFILES_STORAGE_KEY =
  'pi-desktop.appearance.profiles.v1';
export const DESKTOP_APPEARANCE_PROFILE_CHANGED_EVENT =
  'pi-desktop:appearance-profile-changed';
...
function sanitizeProfile(
  value: unknown,
  fallback: DesktopAppearanceProfile,
  ...
  return {
    ...
    contrast: clampContrast(
      typeof input.contrast === 'number' ? input.contrast : fallback.contrast,
    ),
  };
}
...
export function saveDesktopAppearanceProfiles(
  profiles: DesktopAppearanceProfiles,
): void {
  try {
    localStorage.setItem(
      DESKTOP_APPEARANCE_PROFILES_STORAGE_KEY,
...
export function applyDesktopAppearanceProfileToRoot(
  ...
  if (profile.accent) {
    ...
    root.style.setProperty(
      '--color-accent-soft',
      `color-mix(in srgb, ${profile.accent} 20%, transparent)`,
    );
  }
  ...
  if (profile.background) {
    ...
    root.style.setProperty(
      '--color-bg-elevated',
      `color-mix(in srgb, ${profile.background} 94%, ${neutralLift} 6%)`,
    );
    root.style.setProperty(
      '--color-bg-muted',
      `color-mix(in srgb, ${profile.background} 89%, ${neutralLift} 11%)`,
    );
    root.style.setProperty(
      '--color-bg-soft',
      `color-mix(in srgb, ${profile.background} 84%, ${neutralLift} 16%)`,
    );
    root.style.setProperty(
      '--color-bg-sidebar',
      `color-mix(in srgb, ${profile.background} ${sidebarBase}, black ${sidebarShade})`,
    );
    root.style.setProperty(
      '--color-bg-workspace-chrome',
      `color-mix(in srgb, ${profile.background} 92%, ${neutralLift} 8%)`,
    );
    root.style.setProperty(
      '--color-bg-workspace-chrome-soft',
      `color-mix(in srgb, ${profile.background} 86%, ${neutralLift} 14%)`,
    );
  }
  ...
  if (profile.foreground) {
    ...
    root.style.setProperty(
      ...
      `color-mix(in srgb, ${profile.foreground} 68%, ${bgForMix} 32%)`,
    );
    root.style.setProperty(
      '--color-text-tertiary',
      `color-mix(in srgb, ${profile.foreground} 52%, ${bgForMix} 48%)`,
    );
    root.style.setProperty(
      '--color-border-default',
      `color-mix(in srgb, ${profile.foreground} 12%, transparent)`,
    );
  }
  ...
  root.style.setProperty(
    '--desktop-sidebar-opacity',
    profile.translucentSidebar ? '88%' : '100%',
  );
  root.style.setProperty(
    '--desktop-sidebar-blur',
    profile.translucentSidebar ? '14px' : '0px',
  );
  root.style.setProperty(
    '--desktop-sidebar-tint-color',
    profile.foreground || 'var(--color-text-primary)',
  );
  root.style.setProperty(
    '--desktop-sidebar-tint-strength',
    profile.translucentSidebar ? '8%' : '0%',
  );
  root.style.setProperty(
    '--desktop-chrome-opacity',
    profile.translucentSidebar ? '92%' : '100%',
  );
  root.style.setProperty(
    '--desktop-chrome-blur',
    profile.translucentSidebar ? '12px' : '0px',
  );
  root.style.setProperty(
    '--desktop-chrome-tint-strength',
    profile.translucentSidebar ? '5%' : '0%',
  );
...
export function notifyDesktopAppearanceProfileChanged(): void {
  window.dispatchEvent(
    new CustomEvent(DESKTOP_APPEARANCE_PROFILE_CHANGED_EVENT),
  );
}
