import {
  getAppearanceProfileForResolvedTheme,
  loadDesktopAppearanceProfiles,
} from './appearance-profiles.js';
...
const PI_THEME_BACKGROUND_TOKENS = [
  'selectedBg',
  'userMessageBg',
  'customMessageBg',
  'toolPendingBg',
...
const PI_THEME_FOREGROUND_TOKENS = [
  'text',
  'userMessageText',
  'customMessageText',
  'toolOutput',
...
function resolveThemeColorValue(
  ...
  if (Object.hasOwn(vars, ref)) {
    ...
    seenVars.add(ref);
    const resolved = resolveThemeColorValue(
      vars[ref],
      theme,
      seenVars,
      seenColorRefs,
...
function resolveThemeColorToken(
  ...
  const colors = theme.colors ?? {};
  if (!Object.hasOwn(colors, token)) return null;
...
function buildPiThemeOverlay(
  ...
  const result: Partial<
    Record<(typeof PI_THEME_OVERLAY_VARS)[number], string>
  > = {};
  ...
  if (background) {
    ...
    result['--color-bg-elevated'] =
      `color-mix(in srgb, ${background} 94%, ${neutralLift} 6%)`;
    result['--color-bg-muted'] =
      `color-mix(in srgb, ${background} 89%, ${neutralLift} 11%)`;
    result['--color-bg-soft'] =
      `color-mix(in srgb, ${background} 84%, ${neutralLift} 16%)`;
    result['--color-bg-sidebar'] =
      `color-mix(in srgb, ${background} ${sidebarBase}, black ${sidebarShade})`;
    result['--color-bg-workspace-chrome'] =
      `color-mix(in srgb, ${background} 92%, ${neutralLift} 8%)`;
    result['--color-bg-workspace-chrome-soft'] =
      `color-mix(in srgb, ${background} 86%, ${neutralLift} 14%)`;
  }
  ...
  if (foreground) {
    ...
    if (background) {
      result['--color-text-secondary'] =
        `color-mix(in srgb, ${foreground} 68%, ${background} 32%)`;
      result['--color-text-tertiary'] =
        `color-mix(in srgb, ${foreground} 52%, ${background} 48%)`;
    } else {
      result['--color-text-secondary'] =
        `color-mix(in srgb, ${foreground} 68%, transparent)`;
      result['--color-text-tertiary'] =
        `color-mix(in srgb, ${foreground} 52%, transparent)`;
    }
    // Keep border subtle and neutral-ish relative to foreground.
    result['--color-border-default'] =
      `color-mix(in srgb, ${foreground} 12%, transparent)`;
  }
...
function applyPiThemeOverlay(
  root: HTMLElement,
  overlay: Partial<Record<(typeof PI_THEME_OVERLAY_VARS)[number], string>>,
  themeName: string,
): void {
  for (const cssVar of PI_THEME_OVERLAY_VARS) {
...
async function resolveSelectedPiThemeName(
  projectPath: string | null | undefined,
  home: string,
): Promise<string | null> {
  const globalSettingsPath = joinFsPath(
    joinFsPath(joinFsPath(home, '.pi'), 'agent'),
    'settings.json',
  );
  ...
  if (normalizedProjectPath) {
    const projectSettingsPath = joinFsPath(
...
async function resolvePiThemeFilePath(
  themeName: string,
  projectPath: string | null | undefined,
  home: string,
): Promise<string | null> {
  ...
  if (isLikelyAbsolutePath(raw) || raw.startsWith('~/')) {
    ...
    if (normalizedProjectPath) {
      candidates.push(
        joinFsPath(
          joinFsPath(joinFsPath(normalizedProjectPath, '.pi'), 'themes'),
          `${raw}.json`,
        ),
      );
    }
    candidates.push(
      joinFsPath(
        joinFsPath(joinFsPath(joinFsPath(home, '.pi'), 'agent'), 'themes'),
        `${raw}.json`,
      ),
    );
  }
...
export async function syncDesktopThemeWithPiTheme(
  projectPath?: string | null,
  preferredThemeName?: string | null,
): Promise<void> {
  const normalizedProjectPath = normalizeFsPath(projectPath);
  ...
  try {
    ...
    if (!selectedThemeName) {
      try {
        ...
        selectedThemeName = getAppearanceProfileForResolvedTheme(
          profiles,
          mode,
        ).themeName.trim();
      } catch {
    ...
    if (!selectedThemeName) {
      selectedThemeName =
        (await resolveSelectedPiThemeName(normalizedProjectPath, home)) ?? '';
    }
    ...
    const themeFilePath = await resolvePiThemeFilePath(
      selectedThemeName,
    ...
    const theme: PiThemeFile = {
      name:
        typeof themeJson.name === 'string' ? themeJson.name : selectedThemeName,
