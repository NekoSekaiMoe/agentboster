...
const workspaceTabsBar: WorkspaceTabs | null = null;
...
const runtimeRunHadError = new Map<string, boolean>();
const runtimeRunNotifyObserved = new Map<string, boolean>();
...
function clampTerminalDockHeight(value: number): number {
  return Math.min(
    ...
    Math.max(TERMINAL_DOCK_MIN_HEIGHT, Math.round(value)),
  );
}
...
function clampFileSplitWidth(value: number): number {
  return Math.min(
    resolveFileSplitMaxWidth(),
    Math.max(FILE_SPLIT_MIN_WIDTH, Math.round(value)),
  );
}
  ...
  const onPointerDown = (event: PointerEvent) => {
    const target =
      event.target instanceof Element
        ? event.target.closest('.terminal-resize-handle')
        : null;
    if (!target) return;
    event.preventDefault();
...
function isCliMissingError(message: string | null | undefined): boolean {
  ...
  if (
    text.includes('could not find the pi cli') ||
    text.includes('npm install -g @mariozechner/pi-coding-agent')
  ) {
    return true;
  }

  const referencesPiBinary =
    /\bpi(?:\.cmd|\.exe|\.bat)?\b/.test(text) || text.includes('pi process');
  if (
    text.includes("'pi' is not recognized as an internal or external command")
  ) {
    return true;
  }
  ...
  if (
    text.includes('failed to spawn pi process') &&
...
function getOrCreateRuntimeForTab(
  workspaceId: string,
  tabId: string,
  projectPath: string,
): SessionRuntime {
...
function resolveRuntimeNotifyTarget(runtime: SessionRuntime): {
  ...
  const workspace =
    workspaces.find((entry) => entry.id === runtime.workspaceId) ?? null;
  const tab = workspace
    ? (workspace.sessionTabs.find((entry) => entry.id === runtime.tabId) ??
      null)
    : null;
  const sessionPath =
    runtime.lastKnownSessionPath ?? tab?.sessionPath ?? undefined;
  const workspaceLabel = workspace?.title?.trim() || undefined;
  const sessionLabel =
    tab?.title?.trim() || (sessionPath ? baseName(sessionPath) : undefined);
  return {
...
function consumeRuntimeRunState(runtimeKey: string): {
  hadError: boolean;
  hadNotify: boolean;
} {
...
function nextSyntheticRuntimeNotifyRequestId(runtimeKey: string): string {
  syntheticRuntimeNotifyCounter =
    syntheticRuntimeNotifyCounter >= 2_100_000_000
      ? 1
      : syntheticRuntimeNotifyCounter + 1;
...
function dispatchSyntheticRunEndNotify(
  runtime: SessionRuntime,
  source: 'active' | 'background',
): void {
...
function handleBackgroundRuntimeNotifyEvent(
  runtimeKey: string,
  ...
  recordDebugTrace(
    `rpc:event type=${type} source=background runtime=${runtime.instanceId}`,
  );
  recordDebugTrace(
    `extension_ui_request method=${method} message=${message.slice(0, 80)} source=background runtime=${runtime.instanceId}`,
  );
  ...
  if (!normalizedRequest) {
    ...
    recordDebugTrace(
      `extension_ui_request unsupported method=${unsupportedMethod} source=background runtime=${runtime.instanceId}`,
    );
    if (requestId) {
...
function setRuntimeRunning(
  runtime: SessionRuntime | null,
  running: boolean,
  _options: { suppressNotify?: boolean } = {},
): void {
  if (!runtime) return;
...
function ensureRunningSessionPoller(): void {
  const hasRunning = [...sessionRuntimes.values()].some(
...
async function pollBackgroundRuntimeState(): Promise<void> {
  ...
  try {
    const runtimes = [...sessionRuntimes.values()].filter(
      (runtime) => runtime.running && runtime.key !== activeSessionRuntimeKey,
    );
...
function updateRuntimeFromState(
  runtime: SessionRuntime | null,
...
function listRuntimeKeysForWorkspace(workspaceId: string): string[] {
  return [...sessionRuntimes.keys()].filter((key) =>
...
function getSessionTabProjectPath(
  tab: WorkspaceSessionTab | null | undefined,
): string | null {
  return normalizeStoredPath(tab?.projectPath ?? null);
}

function getFileTabProjectPath(
  tab: WorkspaceFileTab | null | undefined,
): string | null {
  return normalizeStoredPath(tab?.projectPath ?? null);
}

function getSessionTabProjectId(
  tab: WorkspaceSessionTab | null | undefined,
): string | null {
  return normalizeStoredId(tab?.projectId ?? null);
}

function getFileTabProjectId(
  tab: WorkspaceFileTab | null | undefined,
): string | null {
  return normalizeStoredId(tab?.projectId ?? null);
}

function getWorkspaceActiveProjectPath(
  workspace: WorkspaceState,
): string | null {
  const activeFile =
    workspace.fileTabs.find((tab) => tab.id === workspace.activeFileTabId) ??
    null;
  const activeSession =
    workspace.sessionTabs.find(
      (tab) => tab.id === workspace.activeSessionTabId,
    ) ??
    workspace.sessionTabs[0] ??
    null;
  return workspace.pane === 'file'
    ? (getFileTabProjectPath(activeFile) ??
        getSessionTabProjectPath(activeSession) ??
        normalizeStoredPath(workspace.activeProjectPath))
    : (getSessionTabProjectPath(activeSession) ??
        getFileTabProjectPath(activeFile) ??
        normalizeStoredPath(workspace.activeProjectPath));
}
...
function getWorkspaceActiveProjectId(workspace: WorkspaceState): string | null {
  ...
  const activeSession =
    ...
    null;
  return workspace.pane === 'file'
    ? (getFileTabProjectId(activeFile) ??
        getSessionTabProjectId(activeSession) ??
        normalizeStoredId(workspace.activeProjectId))
    : (getSessionTabProjectId(activeSession) ??
        getFileTabProjectId(activeFile) ??
        normalizeStoredId(workspace.activeProjectId));
}
...
function isEphemeralSessionTab(
  tab: WorkspaceSessionTab | null | undefined,
): boolean {
...
function ensureWorkspaceContentState(workspace: WorkspaceState): void {
  ...
  workspace.activeProjectPath = normalizeStoredPath(
    workspace.activeProjectPath,
  );

  const incomingSessionTabs = Array.isArray(workspace.sessionTabs)
  ...
  workspace.sessionTabs = incomingSessionTabs
    ...
    .map((tab) => {
      ...
      const storedMessageCount = (tab as Partial<WorkspaceSessionTab>)
        .messageCount;
      const needsAttentionRaw = (tab as Partial<WorkspaceSessionTab>)
        .needsAttention;
      const attentionMessageRaw = (tab as Partial<WorkspaceSessionTab>)
        .attentionMessage;
      return {
        id: tab.id,
        projectId: normalizeStoredId(
          (tab as Partial<WorkspaceSessionTab>).projectId,
        ),
        projectPath: normalizeStoredPath(
          (tab as Partial<WorkspaceSessionTab>).projectPath,
        ),
        sessionPath,
        title:
        ...
        messageCount:
          ...
          Number.isFinite(storedMessageCount)
            ...
            : sessionPath
              ...
              : 0,
        ephemeral:
          typeof (tab as Partial<WorkspaceSessionTab>).ephemeral === 'boolean'
            ? Boolean((tab as Partial<WorkspaceSessionTab>).ephemeral)
            : !sessionPath,
        needsAttention:
          typeof needsAttentionRaw === 'boolean' ? needsAttentionRaw : false,
        attentionMessage:
          typeof attentionMessageRaw === 'string' &&
          attentionMessageRaw.trim().length > 0
  ...
  if (workspace.sessionTabs.length === 0) {
    const fallbackTitle =
      typeof workspace.sessionTitle === 'string' &&
      workspace.sessionTitle.trim().length > 0
  ...
  const incomingFileTabs = Array.isArray(workspace.fileTabs)
    ? workspace.fileTabs
    : [];
  workspace.fileTabs = incomingFileTabs
    ...
    .map((tab) => {
      ...
      const projectPath = normalizeStoredPath(
        (tab as Partial<WorkspaceFileTab>).projectPath,
      );
      const draftDirectoryPath = path
        ? null
        : (normalizeStoredPath(
            (tab as Partial<WorkspaceFileTab>).draftDirectoryPath,
          ) ?? projectPath);
      const draftAnchorPath = path
        ? null
        : normalizeStoredPath(
            (tab as Partial<WorkspaceFileTab>).draftAnchorPath,
          );
      return {
        id: tab.id,
        projectId: normalizeStoredId(
          (tab as Partial<WorkspaceFileTab>).projectId,
        ),
        projectPath,
        path,
        title:
  ...
  if (
    ...
    !workspace.fileTabs.some((tab) => tab.id === workspace.activeFileTabId)
  ) {
  ...
  if (
    activeSession &&
    isDraftSessionTab(activeSession) &&
    !activeSession.projectPath &&
    workspace.activeProjectPath
  ) {
    setSessionTabProject(
      activeSession,
      workspace.activeProjectId,
      workspace.activeProjectPath,
    );
  ...
  if (
    activeFile &&
    isDraftFileTab(activeFile) &&
    !activeFile.draftDirectoryPath
  ) {
    activeFile.draftDirectoryPath =
      activeFile.projectPath ?? workspace.activeProjectPath;
  }
...
function isSessionTabRuntimeRunning(
  workspaceId: string,
  tabId: string,
): boolean {
  ...
  if (
    ...
    runtime.phase === 'creating_session'
  ) {
    return true;
  }
  return false;
...
function markSessionAttentionTarget(target: {
  ...
  if (target.tabId) {
    tab =
      workspace.sessionTabs.find((entry) => entry.id === target.tabId) ?? null;
  }
  if (!tab && target.sessionPath) {
    const normalizedPath = normalizeSessionPath(target.sessionPath);
    tab =
      workspace.sessionTabs.find(
        (entry) => normalizeSessionPath(entry.sessionPath) === normalizedPath,
      ) ?? null;
  }
  ...
  recordDebugTrace(
    `notify-attention:set workspace=${workspace.id} tab=${tab.id} message=${tab.attentionMessage}`,
  );
  persistWorkspaces();
...
function clearVisibleActiveSessionAttention(): void {
  ...
  recordDebugTrace(
    `notify-attention:cleared workspace=${workspace.id} tab=${tab.id}`,
  );
  persistWorkspaces();
  syncContentTabsBar(workspace);
  syncSidebarSelectionFromWorkspace(workspace);
...
function setActiveSessionTab(
  ...
  setWorkspaceActiveProject(workspace, {
    id: tab.projectId,
...
function openOrActivateSessionTab(
  ...
  let tab = workspace.sessionTabs.find(
    (entry) => normalizeSessionPath(entry.sessionPath) === normalized,
  );
  const nextTitle = (preferredTitle || baseName(sessionPath)).trim() || 'Chat';
  if (!tab) {
    const activeTab =
      workspace.sessionTabs.find(
        (entry) => entry.id === workspace.activeSessionTabId,
      ) ?? null;
    const preferredTab = options.preferredTabId
      ? (workspace.sessionTabs.find(
          (entry) => entry.id === options.preferredTabId,
        ) ?? null)
      : null;
    const onlyTab =
      workspace.sessionTabs.length === 1 ? workspace.sessionTabs[0] : null;
    const onlyTabLooksLikeSeed =
      Boolean(onlyTab) &&
      ['chat', 'new session', ''].includes(
        (onlyTab?.title || '').trim().toLowerCase(),
      );
    const reusableCandidates: WorkspaceSessionTab[] = [];
    const pushReusableCandidate = (
      candidate: WorkspaceSessionTab | null | undefined,
    ) => {
      if (!candidate) return;
    ...
    const reusableTab = allowCreateTab
      ? null
      : (reusableCandidates.find(
          (candidate) =>
            !isSessionTabRuntimeRunning(workspace.id, candidate.id),
        ) ?? null);
...
function openOrActivateFileTab(
  ...
  const allowCreateTab = options.allowCreateTab ?? false;
  let tab = workspace.fileTabs.find(
    (entry) => normalizeProjectPath(entry.path) === normalized,
  );
  if (!tab) {
    const preferredTab = options.preferredTabId
      ? (workspace.fileTabs.find(
          (entry) => entry.id === options.preferredTabId,
        ) ?? null)
      : null;
    const activeTab =
      workspace.fileTabs.find(
        (entry) => entry.id === workspace.activeFileTabId,
      ) ?? null;
    const reusableTab = allowCreateTab
      ? null
      : (preferredTab ?? activeTab ?? workspace.fileTabs[0] ?? null);
    if (reusableTab) {
...
function createAndActivateEmptyFileTab(
  ...
  const normalizedDraftDirectoryPath =
    normalizeStoredPath(draftDirectoryPath) ?? normalizeStoredPath(projectPath);
...
function resetWorkspaceContentTabs(
  ...
  project: { id?: string | null; path?: string | null } | null = {
    ...
    path: workspace.activeProjectPath,
  },
): void {
...
function createAndActivateEmptySessionTab(
  ...
  if (
    activeSessionTab &&
    !forceNewTab &&
    !isSessionTabRuntimeRunning(workspace.id, activeSessionTab.id)
  ) {
    if (
      isEphemeralSessionTab(activeSessionTab) &&
      activeSessionTab.sessionPath &&
      (activeSessionTab.messageCount ?? 0) <= 0
    ) {
    ...
    setWorkspaceActiveProject(workspace, {
      id: activeSessionTab.projectId,
      path: activeSessionTab.projectPath,
    });
    workspace.pane = 'chat';
...
function collectEphemeralSessionPaths(
  tabs: Array<WorkspaceSessionTab | null | undefined>,
): string[] {
...
async function discardEphemeralSessionPaths(
  sessionPaths: string[],
): Promise<void> {
  ...
  await Promise.all(
    sessionPaths.map(async (sessionPath) => {
      ...
      try {
        ...
        console.warn(
          'Failed to discard empty draft session:',
          sessionPath,
          err,
        );
      }
...
function scheduleDiscardEphemeralSessionTabs(
  tabs: Array<WorkspaceSessionTab | null | undefined>,
): void {
...
function pruneInactiveEphemeralSessionTabs(
  workspace: WorkspaceState,
  keepTabIds: string[] = [],
): boolean {
  ensureWorkspaceContentState(workspace);
  ...
  workspace.sessionTabs = workspace.sessionTabs.filter(
    (tab) => !removedIds.has(tab.id),
  );
...
function pruneEphemeralTabsWhenLeavingDraft(
  workspace: WorkspaceState,
  keepTabIds: string[] = [],
): boolean {
  if (
    !workspace.sessionTabs.some(
      (tab) => !isEphemeralSessionTab(tab) || keepTabIds.includes(tab.id),
    )
  ) {
    return false;
  }
...
function queueProjectTask(
  ...
  const version = invalidatePending
    ? ++projectSwitchVersion
    : projectSwitchVersion;
  recordDebugTrace(
    `queue ${label} v=${version}${invalidatePending ? '' : ' (keep-version)'}`,
  );
...
function isRpcTimeoutError(err: unknown): boolean {
  const message = err instanceof Error ? err.message : String(err);
  return (
    /timeout waiting for response/i.test(message) || /timed out/i.test(message)
  );
}
...
async function withRpcRetry<T>(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = err;
      recordDebugTrace(
        `rpc ${label} failed attempt=${attempt}: ${err instanceof Error ? err.message : String(err)}`,
      );
...
async function renameSessionFromWorkspace(
  projectId: string,
  sessionPath: string,
  nextName: string,
): Promise<boolean> {
  ...
  ensureWorkspaceContentState(workspace);
  const targetTab = workspace.sessionTabs.find(
    (tab) =>
      normalizeSessionPath(tab.sessionPath) ===
  ...
  await queueProjectTask(
    async () => {
      ...
      const openTargetTab =
        workspace.sessionTabs.find(
          (tab) => normalizeSessionPath(tab.sessionPath) === normalizedTarget,
        ) ?? null;
      const activeTarget = Boolean(
        openTargetTab && workspace.activeSessionTabId === openTargetTab.id,
      );

      if (openTargetTab) {
        const targetRuntime = await ensureRuntimeForSessionTab(
          workspace,
          openTargetTab,
          project.path,
          activeTarget,
...
async function reloadActiveWorkspaceRuntime(): Promise<boolean> {
  ...
  const projectPath =
    getSessionTabProjectPath(activeSession) ??
    getWorkspaceActiveProjectPath(workspace);
  if (!projectPath) return false;

  syncActiveChatRuntimeBinding(workspace, {
    forceReset: true,
    statusText: 'Reloading runtime…',
  });
  ...
  await queueProjectTask(
    async (version) => {
      ...
      assertProjectTaskCurrent(version);
      await ensureRuntimeForSessionTab(
        workspace,
        activeSession,
        projectPath,
        true,
...
function queueAuthConfigDrivenReload(
  trigger: string,
  delayMs = AUTH_CONFIG_RELOAD_DEBOUNCE_MS,
): void {
...
async function runAuthConfigDrivenReload(trigger: string): Promise<void> {
  ...
  if (isActiveRuntimeStreamingForAuthReload()) {
    ...
    recordDebugTrace(
      `auth-reload deferred trigger=${trigger} reason=streaming`,
    );
    return;
  }
...
async function startAuthConfigChangeMonitor(): Promise<void> {
  ...
  try {
    ...
    recordDebugTrace(
      `auth-watch fallback=poll reason=${err instanceof Error ? err.message : String(err)}`,
    );
  }
...
function startSidebarSessionsWarmRefresh(
  durationMs = 90_000,
  intervalMs = 1_200,
): void {
  scheduleSidebarSessionsRefresh(0);
...
function getCwd(): string {
  try {
    ...
    if (defaultWorkspaceRaw) {
      const projects = JSON.parse(defaultWorkspaceRaw) as Array<{
        path?: string;
      }>;
...
const WORKSPACE_DEFAULT_EMOJIS = [
  '💻',
  '🧠',
  '🚀',
  '📝',
...
function ensureWorkspaceEmoji(workspace: WorkspaceState): boolean {
  const normalized =
    typeof workspace.emoji === 'string' ? workspace.emoji.trim() : '';
...
function loadWorkspaces(): void {
      ...
      workspaces = parsed
        ...
        .map((w, idx) => {
          const fallbackSessionTitle =
            typeof w.sessionTitle === 'string' &&
            w.sessionTitle.trim().length > 0
              ? w.sessionTitle.trim()
              : NEW_SESSION_TAB_TITLE;
          const rawSessionTabs = Array.isArray(w.sessionTabs)
            ? (w.sessionTabs as Array<Partial<WorkspaceSessionTab>>)
            : [];
          const rawFileTabs = Array.isArray(w.fileTabs)
            ? (w.fileTabs as Array<Partial<WorkspaceFileTab>>)
            : [];
                ...
                messageCount:
                  ...
                  Number.isFinite(storedMessageCount)
                    ...
                    : sessionPath
                      ...
                      : 0,
                ephemeral:
                  typeof tab.ephemeral === 'boolean'
                    ? Boolean(tab.ephemeral)
                    : !sessionPath,
                needsAttention:
                  typeof needsAttentionRaw === 'boolean'
                    ? needsAttentionRaw
                    : false,
                attentionMessage:
                  typeof attentionMessageRaw === 'string' &&
                ...
                title:
                  typeof tab.title === 'string' && tab.title.trim().length > 0
                    ...
                    : path
                      ? baseName(path)
                      : NEW_FILE_TAB_TITLE,
                draftDirectoryPath: path
                  ? null
                  : (normalizeStoredPath(tab.draftDirectoryPath) ??
                    projectPath),
                draftAnchorPath: path
                  ? null
                  : normalizeStoredPath(tab.draftAnchorPath),
              };
            });

          if (
            fileTabs.length === 0 &&
            typeof w.filePath === 'string' &&
            w.filePath.trim().length > 0
          ) {
            fileTabs.push({
              id: uid('filetab'),
          ...
          const workspace: WorkspaceState = {
            ...
            title:
              typeof w.title === 'string' && w.title.trim().length > 0
                ...
                : `Workspace ${idx + 1}`,
            color:
              typeof w.color === 'string' && w.color.trim().length > 0
                ? w.color
                : null,
            emoji:
              typeof w.emoji === 'string' && w.emoji.trim().length > 0
                ? w.emoji.trim()
                : null,
            pinned: false,
            ...
            sessionTabs,
            activeSessionTabId:
              typeof w.activeSessionTabId === 'string' &&
              sessionTabs.some((tab) => tab.id === w.activeSessionTabId)
                ? w.activeSessionTabId
                : (sessionTabs[0]?.id ?? null),
            fileTabs,
            activeFileTabId:
              typeof w.activeFileTabId === 'string' &&
              fileTabs.some((tab) => tab.id === w.activeFileTabId)
                ? w.activeFileTabId
                : (fileTabs[0]?.id ?? null),
          };
  ...
  for (const workspace of workspaces) {
    ensureWorkspaceContentState(workspace);
    mutatedWorkspaceMetadata =
      ensureWorkspaceEmoji(workspace) || mutatedWorkspaceMetadata;
  }
...
function syncDesktopUpdateUiHint(): void {
  sidebar?.setDesktopUpdateStatus(
    ...
    desktopUpdateStatus?.latestVersion ?? null,
  );
}
...
function syncSidebarSelectionFromWorkspace(
  workspace: WorkspaceState | null = getActiveWorkspace(),
): void {
  if (!sidebar) {
  ...
  chatView?.setWelcomeProjects(
    sidebar.listProjects(),
  ...
  const attentionEntries = workspace.sessionTabs
    ...
    .map((tab) => ({
      path: tab.sessionPath as string,
      message: tab.attentionMessage,
    }));
...
function syncActiveChatRuntimeBinding(
  ...
  const projectPath =
    getSessionTabProjectPath(activeSessionTab) ??
    getWorkspaceActiveProjectPath(workspace);
  ...
  if (runtimeChanged) {
    recordDebugTrace(
      `syncActiveChatRuntimeBinding runtime=${expectedRuntimeKey ?? '-'} tab=${activeSessionTab.id}`,
    );
...
function listVisibleSessionTabsForContentBar(
  workspace: WorkspaceState,
): WorkspaceSessionTab[] {
  ensureWorkspaceContentState(workspace);
...
function syncContentTabsBar(
  ...
  const hasProject = Boolean(
    workspace && getWorkspaceActiveProjectPath(workspace),
  );
...
function setPaneVisibility(
  ...
  const fileSplitResizeHandle = document.getElementById(
    'file-split-resize-handle',
  );
...
function resolveSettingsRuntimeProjectPath(
...
async function applyWorkspacePane(
  ...
  if (showFileSplit && activeFileTab) {
    const draftBasePath = isDraftFileTab(activeFileTab)
      ...
      : null;
    fileViewer?.setProjectPath(
      draftBasePath ??
        getFileTabProjectPath(activeFileTab) ??
...
async function ensureRuntimeForSessionTab(
  ...
  setSessionTabProject(
    sessionTab,
    sessionTab.projectId ?? workspace.activeProjectId,
    projectPath,
  );
  if (makeActive) {
    setWorkspaceActiveProject(workspace, {
      id: sessionTab.projectId,
      path: projectPath,
    });
  ...
  const runtime = getOrCreateRuntimeForTab(
    workspace.id,
    sessionTab.id,
    projectPath,
  );
  ...
  const projectChanged =
    normalizeProjectPath(runtime.projectPath) !==
  ...
  try {
    ...
    if (sessionTab.sessionPath) {
      ...
      if (
        normalizeSessionPath(targetSessionPath) !==
        normalizeSessionPath(runtime.lastKnownSessionPath)
      ) {
        runtime.phase = 'switching_session';
    ...
    const state = await withRpcRetry(`get_state ${runtime.instanceId}`, () =>
      bridge.getState(),
    );
    if (typeof taskVersion === 'number') {
      assertProjectTaskCurrent(taskVersion);
    ...
    if (
      sessionTab.sessionPath &&
      normalizeSessionPath(state.sessionFile) !==
        normalizeSessionPath(sessionTab.sessionPath)
    ) {
      throw new Error(
    ...
    runtime.phase = 'ready';
    recordDebugTrace(
      `ensureRuntime:ready instance=${runtime.instanceId} session=${runtime.lastKnownSessionPath ?? '-'}`,
    );
    if (
      makeActive &&
      workspace.activeSessionTabId === sessionTab.id &&
      normalizeProjectPath(
        getSessionTabProjectPath(sessionTab) ??
          getWorkspaceActiveProjectPath(workspace),
      ) === normalizeProjectPath(projectPath)
    ) {
    ...
    recordDebugTrace(
      `ensureRuntime:failed instance=${runtime.instanceId}: ${runtime.lastError}`,
    );
...
async function ensureRpcForProject(
  projectPath: string,
  taskVersion?: number,
  ...
  setSessionTabProject(
    activeSessionTab,
    activeSessionTab.projectId ?? workspace.activeProjectId,
    projectPath,
  );
  setWorkspaceActiveProject(workspace, {
    id: activeSessionTab.projectId,
  ...
  return ensureRuntimeForSessionTab(
    workspace,
    activeSessionTab,
    projectPath,
    true,
...
async function activateWorkspace(
  ...
  recordDebugTrace(
    `activateWorkspace:project id=${workspaceId} path=${activeProjectPath ?? '-'}`,
  );
...
async function focusNotificationTarget(
  ...
  const workspace =
    workspaces.find((entry) => entry.id === targetWorkspace.id) ??
    targetWorkspace;
  ensureWorkspaceContentState(workspace);

  let resolution = target.tabId ? 'tab-id' : 'none';
  let focusedTab = target.tabId
    ? setActiveSessionTab(workspace, target.tabId)
    : null;
  if (!focusedTab && target.sessionPath) {
    ...
    const existingTab =
      workspace.sessionTabs.find(
        (tab) =>
          normalizeSessionPath(tab.sessionPath) === normalizedSessionPath,
      ) ?? null;
    if (existingTab) {
...
async function initialize(): Promise<void> {
  ...
  if (compatInstall.error && !compatInstall.skipped) {
    console.warn(
      'Failed to install desktop compatibility extension:',
      compatInstall.error,
    );
  }
  const notifyBridgeInstall =
    await ensureDesktopNotifyBridgeExtensionInstalled();
  if (notifyBridgeInstall.error && !notifyBridgeInstall.skipped) {
    console.warn(
      'Failed to install desktop notify bridge extension:',
      notifyBridgeInstall.error,
    );
  }
  const smartVoiceNotifyHostMode =
    await ensureSmartVoiceNotifyDesktopHostMode();
  if (smartVoiceNotifyHostMode.error && !smartVoiceNotifyHostMode.skipped) {
    console.warn(
      'Failed to enforce smart voice notify desktop host mode:',
      smartVoiceNotifyHostMode.error,
    );
  }
  ...
  try {
    ...
    chatView.setOnStateChange((state) => {
      ...
      if (!runtime) {
        recordDebugTrace(
          `state-change ignored: no active runtime session=${state.sessionFile ?? '-'}`,
        );
        return;
      }
      ...
      if (state.sessionFile) {
        ...
        const keepNewSessionLabel =
          (incomingName.length === 0 ||
            incomingName.toLowerCase() === 'chat') &&
          ['chat', 'new session', ''].includes(currentTitle);
        activeSession.title = keepNewSessionLabel
          ? NEW_SESSION_TAB_TITLE
          : nextTitle;
    ...
    chatView.setOnOpenExtensionConfig(async (commandName, args) => {
      ...
      return await packagesView.openExtensionConfigByCommand(
        normalizedName,
        args,
      );
    });
    chatView.setOnOpenProviderConfig(async (provider) => {
      ...
      if (!packagesView) return false;
      await packagesView.refreshPackages(false);
      return await packagesView.openExtensionConfigByProvider(
        normalizedProvider,
      );
    });
    chatView.setOnBeginRenameCurrentSession(() => {
      ...
      const projectId =
        getSessionTabProjectId(activeSession) ??
        getWorkspaceActiveProjectId(workspace);
      const sessionPath = normalizeSessionPath(
        chatView?.getState()?.sessionFile ?? activeSession.sessionPath ?? '',
      );
    ...
    chatView.setOnRenameCurrentSession(async (nextName) => {
      ...
      if (projectId && sessionPath) {
        return await renameSessionFromWorkspace(
          projectId,
          sessionPath,
          nextName,
        );
    ...
    chatView.setOnCreateFreshSession(async () => {
      ...
      if (!getWorkspaceActiveProjectPath(workspace)) {
        chatView?.notify(
          'Add/select a project before creating a new session',
          'info',
        );
        return false;
      }
    ...
    chatView.setOnReloadRuntime(async () => {
      ...
      if (!workspace || !getWorkspaceActiveProjectPath(workspace)) {
        chatView?.notify(
          'Add/select a project before reloading runtime',
          'info',
        );
        return false;
      }
    ...
    chatView.setOnRunStateChange((running) => {
      ...
      if (runtime) {
        ...
        const currentSessionPath =
          chatView?.getState()?.sessionFile ?? runtime.lastKnownSessionPath;
    ...
    extensionUiHandler?.setEditorTextHandler((text) =>
    ...
    if (startupWorkspaceId) {
      void queueProjectTask(
        ...
        (err) => {
          ...
          recordDebugTrace(
            `startup-activation-error: ${err instanceof Error ? err.message : String(err)}`,
          );
...
function mountSettingsPanel(): SettingsPanel {
  ...
  panel.setOnPiBinaryPathChange((path) => {
    ...
    if (previous !== preferredPiBinaryPath && preferredPiBinaryPath) {
      chatView?.notify(
        'Saved CLI binary path override. Use /reload to reconnect active runtimes.',
        'info',
      );
    }
  });
...
function initializeComponents(): void {
  ...
  rpcBridge.onEvent((event) => {
    ...
    if (type === 'extension_ui_request') {
      ...
      const message = typeof event.message === 'string' ? event.message : '';
      recordDebugTrace(
        `extension_ui_request method=${method} message=${message.slice(0, 80)}`,
      );

      const request = { ...(event as Record<string, unknown>) } as Record<
        string,
        unknown
      >;
      if (method === 'notify') {
        if (runtime) {
          ...
          const targetSessionLabel =
            ...
            (targetSessionPath ? baseName(targetSessionPath) : undefined);
      ...
      if (!normalizedRequest) {
        ...
        const unsupportedMethod =
          typeof request.method === 'string' ? request.method : 'unknown';
        recordDebugTrace(
          `extension_ui_request unsupported method=${unsupportedMethod} source=active`,
        );
        if (requestId) {
          if (extensionUiHandler) {
            void extensionUiHandler.respondUnsupportedRequest(
              requestId,
              unsupportedMethod,
              'active',
            );
...
async function startFreshSessionTab(
  ...
  await queueProjectTask(
    ...
    {
      label: options.forceNewTab
        ? 'fresh-session-tab-explicit'
        : 'fresh-session-tab',
    },
  );
}
...
async function recoverSettingsPaneIfBlank(reason: string): Promise<void> {
  ...
  if (settingsContainer.childElementCount > 0 && settingsPanel?.isVisible())
    return;
  ...
  void applyWorkspacePane(workspace)
    .catch((err) => {
      ...
      try {
        ...
        setPaneVisibility('settings');
        const panel = mountSettingsPanel();
        panel.setRuntimeProjectPath(
          resolveSettingsRuntimeProjectPath(workspace),
        );
        if (targetSection) panel.setActiveSection(targetSection);
        void panel.open();
      } catch (innerErr) {
...
function renderApp(): void {
  ...
  if (
    activeWorkspace?.pane === 'settings' &&
    settingsPaneContainer &&
    settingsPaneContainer.childElementCount === 0
  ) {
  ...
  const contentTabsContainer = document.getElementById(
  ...
  if (contentTabsContainer) {
    ...
    contentTabsBar.setOnSelect((tabId) => {
      ...
      syncActiveChatRuntimeBinding(workspace, {
        forceReset: true,
        statusText: sessionTab.sessionPath
          ? 'Loading session…'
          : 'Starting new session…',
      });
      void applyWorkspacePane(workspace);
    ...
    contentTabsBar.setOnClose((tabId) => {
      ...
      const sessionIndex = workspace.sessionTabs.findIndex(
        (tab) => tab.id === tabId,
      );
      ...
      if (workspace.sessionTabs.length === 0) {
        ...
        nextSession =
          workspace.sessionTabs[sessionIndex] ??
          workspace.sessionTabs[sessionIndex - 1] ??
          workspace.sessionTabs[0] ??
          null;
        workspace.activeSessionTabId = nextSession?.id ?? null;
        workspace.sessionTitle = nextSession?.title ?? NEW_SESSION_TAB_TITLE;
      ...
      syncActiveChatRuntimeBinding(workspace, {
        forceReset: true,
        statusText: nextSession?.sessionPath
          ? 'Loading session…'
          : 'Starting new session…',
      });
      void applyWorkspacePane(workspace);
      ...
      const nextProjectPath =
        getSessionTabProjectPath(nextSession) ??
        getWorkspaceActiveProjectPath(workspace);
      void queueProjectTask(
        async (version) => {
  ...
  if (filePane) {
    ...
    fileViewer.setOnDraftFileCreated((filePath) => {
      ...
      const activeFileTab =
        workspace.fileTabs.find(
          (tab) => tab.id === workspace.activeFileTabId,
        ) ?? null;
      if (activeFileTab && !activeFileTab.path) {
        ...
        setFileTabProject(
          activeFileTab,
          activeFileTab.projectId ?? workspace.activeProjectId,
          activeFileTab.projectPath ?? workspace.activeProjectPath,
        );
        if (anchorPath && activeFileTab.projectId) {
          sidebar?.setNewFilePlacementHint(
            activeFileTab.projectId,
            filePath,
            anchorPath,
          );
        }
      } else {
        openOrActivateFileTab(
          workspace,
          filePath,
          workspace.activeProjectId,
          workspace.activeProjectPath,
  ...
  sidebar.setOnProjectSelect((project) => {
    ...
    const selectingSameProject =
      ...
      normalizeProjectPath(currentProjectPath) ===
    ...
    const preferredSession =
      sidebar?.getPreferredSessionForProject(project.id) ?? null;
    const autoTabCountBefore = getVisibleContentTabCount(workspace);
    const canAutoCreateTab =
      autoTabCountBefore < DEFAULT_AUTO_CONTENT_TAB_LIMIT;
    setWorkspaceActiveProject(workspace, project);

    if (preferredSession) {
      const sessionTab = openOrActivateSessionTab(
        workspace,
        preferredSession.path,
        project.id,
        project.path,
        preferredSession.name,
        {
  ...
  sidebar.setOnSessionMarkUnread((_projectId, sessionPath, _sessionName) => {
    ...
    targetTab.attentionMessage = pickSessionAttentionMessage(
      targetTab.attentionMessage,
    );
    persistWorkspaces();
    syncContentTabsBar(workspace);
    syncSidebarSelectionFromWorkspace(workspace);
  ...
  sidebar.setOnSessionRename(
    (projectId, sessionPath, _currentName, nextName) => {
  ...
  sidebar.setOnSessionDelete((projectId, sessionPath) => {
    ...
    const activeWasRemoved = Boolean(
      activeTab &&
        normalizeSessionPath(activeTab.sessionPath) === normalizedTarget,
    );
    ...
    if (workspace.sessionTabs.length === 0) {
      ...
      nextSession =
        workspace.sessionTabs[firstRemovedIndex] ??
        workspace.sessionTabs[firstRemovedIndex - 1] ??
        workspace.sessionTabs[0] ??
        null;
      workspace.activeSessionTabId = nextSession?.id ?? null;
      workspace.sessionTitle = nextSession?.title ?? NEW_SESSION_TAB_TITLE;
  ...
  sidebar.setOnFileDelete((projectId, filePath) => {
    ...
    const activeWasRemoved = Boolean(
      activeFileTab &&
        normalizeProjectPath(activeFileTab.path) === normalizedTarget,
    );
    workspace.fileTabs = workspace.fileTabs.filter(
      (tab) => normalizeProjectPath(tab.path) !== normalizedTarget,
    );
    ...
    if (workspace.fileTabs.length === 0) {
      ...
      const nextFile =
        workspace.fileTabs[firstRemovedIndex] ??
        workspace.fileTabs[firstRemovedIndex - 1] ??
        workspace.fileTabs[0] ??
        null;
