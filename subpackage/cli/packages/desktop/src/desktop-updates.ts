...
function archHints(name: string): {
  mentionsArm64: boolean;
  mentionsX64: boolean;
} {
...
export async function fetchDesktopUpdateStatus(): Promise<DesktopUpdateStatus> {
  ...
  const latestVersion = latestVersionRaw
    ? sanitizeVersion(latestVersionRaw)
    : null;
  ...
  if (updateAvailable && !asset) {
    note =
      'No installer matched this platform automatically. Open the release page to choose an asset manually.';
  } else if (runtime.platform === 'unknown') {
    note =
      'Could not detect desktop platform reliably; using generic asset matching.';
  }
...
export async function openDesktopUpdate(
