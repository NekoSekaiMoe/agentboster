...
import { Agent, type AgentMessage, type StreamFn, type ThinkingLevel } from "@agentboster-cli/agent";
import { clampThinkingLevel, type Message, type Model } from "@agentboster-cli/ai/compat";
...
export interface CreateAgentSessionOptions {
	...
	/**
	 * Override the LLM streaming function. Default: build one from the
	 * model registry + provider SDKs. When set, the agent session uses
	 * this function verbatim and skips model registry auth lookup.
	 */
 ...
 *
 * // With explicit model
 * import { getModel } from '@agentboster-cli/ai';
 * const { session } = await createAgentSession({
