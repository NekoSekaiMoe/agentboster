...
export class InteractiveThemeController {
  ...
  constructor(
    ...
    this.ui.onTerminalColorSchemeChange((terminalTheme) =>
  ...
  async applyFromSettings(): Promise<void> {
    ...
    if (autoTheme) {
      this.terminalTheme = await detectTerminalThemeForAuto({
        ui: this.ui,
        timeoutMs: 100,
      });
      this.setAutoSync(true);
      this.applyThemeName(
        this.terminalTheme === 'light'
  ...
  preview(themeSettingOrName: string): void {
    const themeName =
      resolveThemeSetting(themeSettingOrName, this.terminalTheme) ??
  ...
  private applyTerminalTheme(terminalTheme: TerminalTheme): void {
    ...
    const themeName =
      terminalTheme === 'light' ? autoTheme.lightTheme : autoTheme.darkTheme;
