...
export class FirstTimeSetupComponent extends Container {
  ...
  constructor(options: FirstTimeSetupOptions) {
    ...
    this.themeIndex = Math.max(
      ...
      THEME_OPTIONS.findIndex(
        (option) => option.value === options.detectedTheme,
      ),
    );
    this.update();
  }
  ...
  private update(): void {
    ...
    this.addChild(
      new Text(theme.fg('accent', SETUP_LOGO_LINES.join('\n')), 1, 0),
    );
    this.addChild(new Spacer(1));
    this.addChild(
      new Text(
        theme.fg(
          'accent',
          theme.bold(`Welcome to ${APP_NAME}, the minimal coding agent.`),
        ),
        1,
        0,
      ),
    ...
    if (this.step === 'theme') {
      ...
      this.addChild(
        new Text(
          theme.fg(
            'muted',
            `Detected system appearance: ${this.options.detectedTheme}`,
          ),
          1,
          0,
        ),
    ...
    this.addChild(
      new Text(
        rawKeyHint('↑↓', 'navigate') +
          ...
          keyHint(
            'tui.select.confirm',
            this.step === 'theme' ? 'continue' : 'finish',
          ) +
          '  ' +
  ...
  private moveSelection(delta: number): void {
    if (this.step === 'theme') {
      const next = Math.max(
        0,
        Math.min(THEME_OPTIONS.length - 1, this.themeIndex + delta),
      );
