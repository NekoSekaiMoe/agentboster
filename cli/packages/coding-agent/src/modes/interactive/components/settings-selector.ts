...
import type { Transport } from '@agentboster-cli/ai';
import {
  type Component,
  Container,
  getCapabilities,
...
const DEFAULT_PROJECT_TRUST_BY_LABEL = new Map(
  Object.entries(DEFAULT_PROJECT_TRUST_LABELS).map(([value, label]) => [
    label,
    value as DefaultProjectTrust,
  ]),
);
    ...
    const items: SettingItem[] = [
      {
        ...
        currentValue:
          (this.state.anthropicExtraUsage ?? true) ? 'true' : 'false',
        values: ['true', 'false'],
      },
    ];
      ...
      (id, newValue) => {
        switch (id) {
          case 'anthropic-extra-usage':
            this.state = {
              ...this.state,
              anthropicExtraUsage: newValue === 'true',
            };
            onChange({ ...this.state });
            break;
        }
...
function defaultAutomaticThemes(
  ...
  const currentFixedTheme = currentThemeSetting.includes('/')
    ? undefined
    : currentThemeSetting;
...
class ThemeSubmenu extends Container {
  ...
  constructor(
    ...
    const autoTheme = parseAutoThemeSetting(currentThemeSetting);
    const automaticThemes = defaultAutomaticThemes(
      currentThemeSetting,
      availableThemes,
    );
    const fixedTheme =
      autoTheme || currentThemeSetting.includes('/')
        ? undefined
        : currentThemeSetting;
  ...
  private setContent(
    ...
    inputComponent: Component = renderComponent,
  ): void {
    this.clear();
  ...
  private showSingleMenu(): void {
    ...
    const menu = new SelectSubmenu(
      ...
      () => this.cancel(),
      (value) => {
        this.callbacks.onThemePreview?.(
          value === AUTOMATIC_THEME_VALUE
            ? this.getAutomaticThemeSetting()
            : value,
        );
      },
    );
  ...
  constructor(config: SettingsConfig, callbacks: SettingsCallbacks) {
    ...
    const items: SettingItem[] = [
      ...
      },
      {
        ...
        currentValue: str(
          DEFAULT_PROJECT_TRUST_LABELS[config.defaultProjectTrust],
        ),
    ...
    const clearOnShrinkIndex = items.findIndex(
      (item) => item.id === 'clear-on-shrink',
    );
        ...
        switch (id) {
          ...
          case 'default-project-trust': {
            const defaultProjectTrust =
              DEFAULT_PROJECT_TRUST_BY_LABEL.get(newValue);
