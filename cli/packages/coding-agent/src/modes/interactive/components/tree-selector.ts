...
function renderHorizontalViewport(
  ...
  const maxBodyWidth = rows.reduce(
    (max, row) => Math.max(max, row.bodyWidth),
    0,
  );
  ...
  if (selectedRow && maxHorizontalScroll > 0) {
    ...
    if (selectedRow.anchorCol > viewportWidth - minVisibleAnchorContentWidth) {
      ...
      horizontalScroll = Math.min(
        maxHorizontalScroll,
        selectedRow.anchorCol - anchorContextWidth,
      );
    }
  }
...
class TreeList implements Component {
      ...
      case 'message': {
        ...
        const versionSuffix =
          remoteMeta?.versions && remoteMeta.versions.length > 1
            ? theme.fg(
                'dim',
                ` (${(remoteMeta.currentVersionIndex ?? 0) + 1}/${remoteMeta.versions.length})`,
              )
            : '';
        if (role === 'user') {
          ...
          if (textContent) {
            result =
              theme.fg('success', 'assistant: ') + textContent + versionSuffix;
  ...
  handleInput(keyData: string): void {
    ...
    if (kb.matches(keyData, 'tui.select.up')) {
      ...
      kb.matches(keyData, 'app.tree.versionPrev') ||
      kb.matches(keyData, 'app.tree.versionNext')
    ) {
      const selected = this.filteredNodes[this.selectedIndex];
...
function compactRawKeys(keys: string[]): string {
  ...
  const parts = keys.map((key) => {
    ...
    return separatorIndex === -1
      ? { prefix: '', suffix: key }
      : {
          prefix: key.slice(0, separatorIndex + 1),
...
export class TreeSelectorComponent extends Container implements Focusable {
  ...
  constructor(
    ...
    this.treeList.onVersionChange = (entryId, newVersionIndex) =>
      this.onVersionChange?.(entryId, newVersionIndex);
