...
function emitStart(stream: AssistantMessageEventStream, state: StreamState): void {
...
async function driveStream(
	...
	if (!response.ok) {
		...
		stream.push({
			type: "error",
			reason: "error",
			error: errorMessage(`HTTP ${response.status}: ${body || response.statusText}`),
		});
		return;
	}
	...
	try {
		...
		const text = err instanceof Error ? err.message : String(err);
		stream.push({ type: "error", reason: "error", error: errorMessage(`Stream read error: ${text}`) });
	}
}
