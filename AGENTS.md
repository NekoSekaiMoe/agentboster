...
- **Workflow DevKit sandbox needs `__dirname` polyfilled**: the DevKit runs the bundled workflow code in a `vm.Script.runInContext` sandbox that doesn't define CommonJS globals. Several ncc-compiled bundles under `node_modules/next/dist/compiled/...` (notably `ua-parser-js`, pulled in by `next/server`) read `__dirname` at init and crash with `ReferenceError: __dirname is not defined`. The polyfill is in `lib/workflow/agent/workflow-polyfills.ts` and must be the first import of any module that calls `start()` from `workflow/api`. Don't remove it.
