...
- **KV**: Dual-backend. Vercel uses Upstash Redis HTTP; self-hosted uses a Postgres-backed shim (`lib/core/kv/pg-backend.ts`, tables `kv_store`/`kv_sets`). Selection is automatic via `lib/deploy/index.ts` `isVercel`.
...
- **Deployment detection**: `lib/deploy/index.ts` exports `isVercel` / `isSelfHosted` / `getPublicAppUrl()`. All backend-selection decisions funnel through this module — never inline `process.env.VERCEL` checks elsewhere.
