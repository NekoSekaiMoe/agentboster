...
- **Linting**: Biome (not ESLint/Prettier). Config: `biome.jsonc`. Run `yarn check:lint` before committing. However, `yarn check:lint` does NOT auto-fix code (`--write` flag removed). It only reports issues because it cause many unsafe changes. Use `yarn format` or `yarn lint:fix` for auto-fixing, but review the changes carefully before committing. Many Biome auto-fixes can be incorrect (e.g., changing `let` to `const` when the variable is mutated later).
...
- **DB**: Dual-driver Postgres via Drizzle ORM. On Vercel/Neon it uses the fetch-based `neon-http` driver; self-hosted it uses `pg` (node-postgres) over TCP. Driver selection is automatic by DATABASE_URL hostname (override with `DB_DRIVER=neon|postgres`). Schema in `lib/core/db/schema/`. Migrations output to `lib/core/db/migrations/`. See `lib/core/db/index.ts` (wrapper) and `lib/core/db/pg-driver.ts` (host-only pg warm-up via `instrumentation.ts`).
- **KV**: Dual-backend. Vercel uses Upstash Redis HTTP; self-hosted uses a Postgres-backed shim (`lib/core/kv/pg-backend.ts`, tables `kv_store`/`kv_sets`). Selection is automatic via `lib/extra/deploy/index.ts` `isVercel`.
- **Blob**: Dual-backend. Vercel uses `@vercel/blob`; self-hosted uses S3/MinIO (`lib/core/blob/s3-backend.ts`). Self-hosted blob URLs are signed proxy URLs (`/api/blob/...`) so the LLM can fetch them without S3 credentials.
- **Deployment detection**: `lib/extra/deploy/index.ts` exports `isVercel` / `isSelfHosted` / `getPublicAppUrl()`. All backend-selection decisions funnel through this module — never inline `process.env.VERCEL` checks elsewhere.
- **Self-hosted migrations**: `yarn db:migrate` (or container entrypoint) runs `scripts/self-host-migrate.ts` → ensure-vector + drizzle-kit push + message-version data migration. Vercel uses `scripts/vercel-postbuild.ts` (unchanged).
- **Auth**: Cookie-based. Requires `AUTH_SECRET`, `USERNAME`, `PASSWORD` env vars. Middleware (Next.js 16+: `proxy.ts`, formerly `middleware.ts`) protects all routes except `/login`, `/api/auth/login`, `/.well-known/workflow/*`, and public assets (`.*` file extensions).
- **Bot webhooks**: Auth secret is embedded in the callback URL path (`/api/bot/{AUTH_SECRET}/{adapter}/callback`). CI uses Yarn (`yarn check:lint`).
...
- **Top-level `node:*` imports break the workflow bundle even in non-step exports.** The workflow DevKit bundler (`@workflow/core`) walks imports **statically per file**: as soon as any file under `lib/workflow/**` is reachable from a workflow body, every top-level `import ... from 'node:fs'` / `'node:path'` / `'node:crypto'` / etc. in that file becomes part of the steps bundle, and the `workflow-node-module-error` esbuild plugin fails the **whole `yarn build`** with `ERROR: You are attempting to use "node:fs" which is a Node.js module. Node.js modules are not available in workflow functions.` This is **not** caught by `yarn check:lint` or `yarn test` — only `yarn build` runs the workflow bundler, and `next.config.ts` is configured to ignore build errors, so the failure surfaces as a hard stop only at build time. The trap is that a function doesn't have to be `'use step'` to trigger this — promoting a `readFileSync` call from inside a `'use step'` function into a non-step sibling export (while keeping the top-level `import { readFileSync } from 'node:fs'`) silently breaks the build. **Rule: in any file that is (or might transitively become) part of the workflow bundle, never use top-level `node:*` imports — use `const { x } = await import('node:fs')` inside the function body.** The dynamic import is invisible to the static bundle analyzer and resolves at runtime, where these functions always run on the host (step body or route handler, both Node). See commit 3803c20 for the regression that established this rule.
  ...
  - **Third-party packages that depend on `node:*` (e.g. `pg`, `@aws-sdk/client-s3`) are NOT hidden by `await import('pg')`.** Unlike `await import('node:fs')` which works because the bundler treats `node:*` specifiers as external, `await import('pg')` causes the bundler to resolve and walk into `pg`'s source, discover its `node:net`/`node:tls` deps, and fail the build. The `await import()` trick ONLY hides Node built-in specifiers. For third-party packages: (a) move all references into a file that is NEVER statically reachable from the workflow bundle (e.g. `pg-driver.ts`, imported only from `instrumentation.ts`), and (b) add the package to `serverExternalPackages` in `next.config.ts` so webpack leaves it as a runtime `require()` rather than bundling it. See the dual-deployment-mode work for the canonical example: `lib/core/db/pg-driver.ts` holds all `pg` references, imported only from `instrumentation.ts`; `pg` and `@aws-sdk/client-s3` are in `serverExternalPackages`.
...
- `yarn build` runs `next build`; it does **not** enforce type or lint correctness (see `next.config.ts`). However, it **does** run the workflow DevKit bundler, which is the only place that catches workflow-bundle violations (top-level `node:*` imports, etc. — see the bullet above). So: `yarn check:lint` is the gate for types/format, build is the gate for workflow-bundle compatibility. After touching anything under `lib/workflow/**` (or files transitively reachable from a workflow body, like `lib/extra/agent/agentd-tools-client.ts`), run `yarn build` before committing — `yarn check:lint` + `yarn test` passing is NOT enough.
- `yarn check:lint` is the real gate before shipping. Exact form: `tsc --noEmit && biome check app components lib hooks proxy.ts next.config.ts drizzle.config.ts` — note the scoped path list, not `biome check .`..
...
- `yarn publish` runs `yarn run check:lint` first (i.e. `tsc --noEmit && biome check ...`), then `yarn build` and `git push`.
...
- DB commands need `DATABASE_URL`: `yarn db:generate`, `yarn db:push`, `yarn db:studio`, `yarn db:ensure-vector`. Self-hosted schema setup: `yarn db:migrate` (`scripts/self-host-migrate.ts` → ensure-vector + drizzle-kit push + message-version data migration); the Docker container runs it at start via `scripts/docker-entrypoint.sh`. Vercel still uses `postbuild` (`scripts/vercel-postbuild.ts`).
...
- `postbuild` (`scripts/vercel-postbuild.ts`) only runs migrations when `VERCEL=1` **and** `VERCEL_ENV=production` (or `VERCEL_TARGET_ENV=production`); local builds never touch the DB. Self-hosted deployments run migrations separately via `scripts/self-host-migrate.ts` (`yarn db:migrate`), invoked at container start by `scripts/docker-entrypoint.sh` (skip with `SKIP_DB_MIGRATE=1`).
- `proxy.ts` (Next.js 16+ middleware convention, formerly `middleware.ts`) protects all routes unless explicitly bypassed; new `app/api/**` routes are session-gated by default. Bypassed paths now include `/api/blob/*` (self-hosted blob proxy — cookie-less, authenticated by an HMAC `t`/`s` query signature verified in the route via `lib/core/blob/proxy-link.ts`, same pattern as `/api/l2`).
...
- **`agentd_nodes.status` is advisory — never trust it for liveness.** The column is only ever written `'online'` (by the register and heartbeat handlers); nothing in the write path sets `'offline'`. The reaper (`reapStaleNodes()` in `lib/extra/agent/node-liveness.ts`, piggybacked on each heartbeat) does flip stale rows to `'offline'` and deletes 24h+ zombies, but reads must still compute effective status from `lastHeartbeat` freshness via `computeNodeStatus()` (2-minute cutoff, matching `dispatch.ts`'s `selectBestNode`). Both `/api/agentd/v1/nodes/status` and `/api/config/monitoring/nodes` already do this; any new read endpoint must too. Do not add a raw `select(... agentdNodes.status ...)` and hand it to the UI.
- **Node registration dedups by `(node_id)` then falls back to `(ip, port)`.** agentd persists its `node_id` to `node_id_file` (default `/var/lib/agentd/node_id` — deliberately NOT under `/var/run/`, which is tmpfs and wiped on reboot, causing duplicate rows). If a daemon restarts with a fresh `node_id` (e.g. the persisted file was lost), the register route (`app/api/agentd/v1/nodes/register/route.ts`) reclaims the stale row by matching `(ip, port)` and rewriting its `node_id` rather than inserting a duplicate. Do not "simplify" this to `node_id`-only dedup — host-reboot scenarios will reintroduce the duplicate-node bug.
...
- `next.config.ts` keeps these as `serverExternalPackages` to avoid bundle breakage: `@chat-adapter/discord`, `@discordjs/ws`, `@vercel/queue`, `discord-interactions`, `discord.js`, `zlib-sync`, `pg`, `@aws-sdk/client-s3`. The last two were added for self-hosting: they are third-party packages that require `node:*`, so webpack must leave them as runtime `require()` (bundling them fails with "Can't resolve 'readline'/'path'" from `pg`→`pgpass`). Adding any new server-only package that pulls in `node:*` needs the same treatment.
