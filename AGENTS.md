...
- `yarn check:lint` is the real gate before shipping. Exact form: `tsc --noEmit && biome check app components lib hooks middleware.ts next.config.ts drizzle.config.ts` — note the scoped path list, not `biome check .`.
